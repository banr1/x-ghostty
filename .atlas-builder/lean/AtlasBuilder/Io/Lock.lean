...
/-!
`AtlasBuilder.Io.Lock` — I-018 の単一実行スロット(canonical write window)。
...
- Python は取得時に `ATLAS_BUILDER_LOCK_TOKEN` を os.environ へ輸出するが、
  Lean 版の ensure / validate は mutating transition を子プロセスとして
  起動しないため輸出しない(継承側 = 環境変数とロックファイルの照合は
  同一挙動)。
-/

namespace AtlasBuilder.Io.Lock

open AtlasBuilder.State.Model (pyIntOfString?)
...
private def randomHex (n : Nat) : IO String := AtlasBuilder.Io.Rand.randomHex n
...
private def heldMessage (owner : Int) : String :=
  s!"another Atlas Builder mutating transition holds the single-flight lock (pid {owner}); state mutation refused."
  ...
  | n + 1 => do
    ...
    | some e =>
      ...
      | none =>
        ...
        | some age =>
          if age ≤ 10 then
            return .error ("another Atlas Builder mutating transition is acquiring "
...
def acquire (controlRoot : System.FilePath) : IO (Except String Bool) := do
  ...
  let supplied := (← IO.getEnv "ATLAS_BUILDER_LOCK_TOKEN").getD ""

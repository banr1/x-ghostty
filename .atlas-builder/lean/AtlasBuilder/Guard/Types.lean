...
/-!
`AtlasBuilder.Guard.Types` — PreToolUse guard の型群(LEAN_MIGRATION_PLAN.md §3.3-2、
META.md §16・§23)。
...
open AtlasBuilder.Core.Classify (DeclaredPackages Profile)
...
structure GuardEnv where
  ...
  /-- `ATLAS_BUILDER_SESSION_MODE` の strip 済み値(未設定は `""`)。 -/
  sessionMode : String
  /-- `ATLAS_BUILDER_SESSION_ESSENCE_MODE` の strip 済み値(essence セッションの
  ...
  以外では未使用。 -/
  handoffRoot : String
  /-- `ATLAS_BUILDER_SESSION_PROJECT_ROOT` の解決結果(未設定・空・解決不能は
  `none`)。 -/
  expectedProject? : Option String
  /-- `(CONTROL_ROOT / "bin" / "atlas-builder").resolve()`(triage の
...
/-- 併走モード(`ATLAS_BUILDER_GUARD_SHADOW`)の判定ログ

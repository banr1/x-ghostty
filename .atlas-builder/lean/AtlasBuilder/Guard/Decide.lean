...
/-!
`AtlasBuilder.Guard.Decide` — PreToolUse guard の純粋決定核(META.md §16・§23、
§31.2-19)。
...
namespace AtlasBuilder.Guard

open AtlasBuilder.Core.Classify
open AtlasBuilder.Core.Prim (dropLit?)
open AtlasBuilder.Core.Text (isPySpace pyStrip)
open AtlasBuilder.Hook.PostTool (pyPathStr childPath relativeTo? bashCandidateTokens)
open Rules
...
def cdFirstTokens (command : String) : List String :=
  (cdRawCaptures command).filterMap fun rawCap =>
    match AtlasBuilder.Core.ShellLex.split (pyStrip rawCap) with
...
def resolvesToEssenceAssetInstall (env : GuardEnv) (raw : String) : Bool :=
  ...
  | some root =>
    ...
    | some resolved =>
      ...
      | some rel => AtlasBuilder.Core.Text.validAssetPath rel
      | none => false
    | none => false
  | none => false
...
private def env0 : GuardEnv :=
  { controlRoot := "/ws/.atlas-builder"
    projectRoots := ["/ws/proj"]
    sessionMode := ""
    base := "/ws/.atlas-builder"
    handoffRoot := "/ws/.atlas-builder/.agent/tmp/triage"
    expectedProject? := some "/ws/proj"
    stateScript := "/ws/.atlas-builder/bin/atlas-builder"
    declared := {}
    resolved :=
      [ ("", some "/ws/proj"),
        ...
        ("scripts/state.py", some "/ws/.atlas-builder/scripts/state.py"),
        ("recipes/asl/files/loop/lean/lakefile.lean",
         some "/ws/.atlas-builder/recipes/asl/files/loop/lean/lakefile.lean"),
        ("templates/project/package.json",
         some "/ws/.atlas-builder/templates/project/package.json"),
        ("../proj", some "/ws/proj"),
  ...
  { env0 with
    ...
    resolved := env0.resolved ++
      [ ("README.md", some "/ws/proj/README.md"),
        ...
        ("../.atlas-builder/README.md", some "/ws/.atlas-builder/README.md") ] }

private def triageEnv : GuardEnv :=
  { env0 with
    sessionMode := "triage"
    settingsFile := "/ws/.atlas-builder/.claude/settings.json"
    resolved := env0.resolved ++
      [ ("/ws/.atlas-builder/.agent/tmp/triage/resume_note.txt",
         some "/ws/.atlas-builder/.agent/tmp/triage/resume_note.txt"),
        ("/ws/.atlas-builder/.agent/tmp/triage",
         some "/ws/.atlas-builder/.agent/tmp/triage"),
        ("/ws/.atlas-builder/bin/atlas-builder",
         some "/ws/.atlas-builder/bin/atlas-builder"),
        ("/ws/.atlas-builder/.claude/settings.json",
         some "/ws/.atlas-builder/.claude/settings.json"),
        ("/ws/.atlas-builder/../proj", some "/ws/proj") ] }
...
private def essenceNewEnv : GuardEnv :=
  { env0 with
    ...
    essenceMode := "new"
    settingsFile := "/ws/.atlas-builder/.claude/settings.json"
    handoffRoot := "/ws/.atlas-builder/.agent/tmp/essence"
    resolved := env0.resolved ++
      [ ("/ws/.atlas-builder/.agent/tmp/essence/ESSENCE.draft.md",
         some "/ws/.atlas-builder/.agent/tmp/essence/ESSENCE.draft.md"),
        ("/ws/.atlas-builder/.claude/settings.json",
         some "/ws/.atlas-builder/.claude/settings.json"),
...
#guard decide env0 (.bash "echo x > .atlas-builder/state/todo.json")
  == ⟨[], some ⟨.deny, protectedWriteDenyReason⟩⟩
...
#guard decide env0 (.bash "bin/atlas-builder state --project ../proj resume")
  == ⟨[], some ⟨.deny, humanOnlyDenyReason⟩⟩
#guard decide env0 (.bash "bin/atlas-builder state resume --project ../proj")
  == ⟨[], some ⟨.deny, humanOnlyDenyReason⟩⟩
#guard decide env0 (.bash "just state --project x record-progress")
  == ⟨[], some ⟨.deny, loopOnlyDenyReason⟩⟩
#guard decide env0 (.bash "bin/atlas-builder state --project x record-progress")
  == ⟨[], some ⟨.deny, loopOnlyDenyReason⟩⟩
...
#guard decide projectCwdEnv (.bash "cp note.md ../.atlas-builder/README.md")
  == ⟨[], some ⟨.deny, controlSurfaceBashDenyReason⟩⟩
...
#guard decide triageEnv
    (.bash "cat > '/ws/.atlas-builder/.agent/tmp/triage/resume_note.txt' <<'EOF'\nhi\nEOF")
...
-- ask 面に先行する。
#guard decide triageEnv (.write "/ws/.atlas-builder/.claude/settings.json")
  == ⟨[], some ⟨.ask, liveSettingsAskReason "triage"⟩⟩
#guard decide essenceNewEnv (.write "/ws/.atlas-builder/.claude/settings.json")
...
#guard decide essenceNewEnv
    (.write "/ws/.atlas-builder/.agent/tmp/essence/ESSENCE.draft.md")
...
#guard decide essenceNewEnv (.write "/ws/proj/essences")
  == ⟨[], some ⟨.deny,
    readOnlyWriteDenyReason "essence" "/ws/.atlas-builder/.agent/tmp/essence" true⟩⟩
...
#guard decide { essenceNewEnv with essenceMode := "update" }
  ...
  == ⟨[], some ⟨.deny,
    readOnlyWriteDenyReason "essence" "/ws/.atlas-builder/.agent/tmp/essence"⟩⟩
...
#guard decide { triageEnv with settingsFile := "" }
    (.write "/ws/.atlas-builder/.claude/settings.json")
  == ⟨[], some ⟨.deny,
    readOnlyWriteDenyReason "triage" "/ws/.atlas-builder/.agent/tmp/triage"⟩⟩
-- resume は read-only の state 読み取り面をすり抜けない
#guard (decide triageEnv
    (.bash "bin/atlas-builder state resume --project ../proj")).decision?
  == some ⟨.deny, readOnlyDenyReason "triage" "/ws/.atlas-builder/.agent/tmp/triage"⟩
...
#guard decide relaxedEnv
    (.bash "bin/atlas-builder state resume --project ../proj")
  == ⟨[], some ⟨.deny, humanOnlyDenyReason⟩⟩

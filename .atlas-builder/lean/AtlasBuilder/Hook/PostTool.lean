...
/-!
`AtlasBuilder.Hook.PostTool` — PostToolUse hook(`atlas-builder hook post-tool`)の
...
「stderr 1 行 + 該当レコードを書かない」に落とす(best-effort、§3.4)。
...
namespace AtlasBuilder.Hook.PostTool

open AtlasBuilder.Core.Json (Value)
open AtlasBuilder.Core.Text (splitLinesPy)
open AtlasBuilder.Core.Classify (isControlPlaneHighRisk isHighRiskPath
...
export AtlasBuilder.Core.PathAlg (childPath)
...
private def pendingBeforeAux (targetStr : String) (sessionId : Value) :
  ...
  | [] => .ok (.str "UNKNOWN")
  | line :: rest =>
    match AtlasBuilder.Core.Json.parse line with
...
#guard stateRootCandidates "/ws/proj/hooks/h.py" "/ws/.atlas-builder"
  == [("/ws/proj/hooks/h.py", "/ws/proj/hooks/h.py/.atlas-builder/state"),
      ("/ws/proj/hooks", "/ws/proj/hooks/.atlas-builder/state"),
      ("/ws/proj", "/ws/proj/.atlas-builder/state"),
      ("/ws", "/ws/.atlas-builder/state"),
      ("/", "/.atlas-builder/state")]
#guard stateRootCandidates "/cr/scripts/x" "/cr"
  == [("/cr/scripts/x", "/cr/scripts/x/.atlas-builder/state"),
      ("/cr/scripts", "/cr/scripts/.atlas-builder/state"),
      ("/", "/.atlas-builder/state")]
...
#guard (AtlasBuilder.Core.Json.Value.render <$> auditRecord "T"
...
end AtlasBuilder.Hook.PostTool

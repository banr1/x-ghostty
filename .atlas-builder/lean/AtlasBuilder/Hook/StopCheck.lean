...
/-!
`AtlasBuilder.Hook.StopCheck` — Stop hook(`atlas-builder hook stop`)の純粋決定核。

移行元: `.claude/hooks/stop_check.py`(移行完了まで凍結)。Atlas Builder のターン終了時、
ディスク状態がクリーンな停止と矛盾していれば(validation.json がまだ error を報告
していれば)、停止をブロックせずに警告だけ出す。

IO シェル(`AtlasBuilder.Cli.Hook`)は project_index.json と validation.json を読んで
...
namespace AtlasBuilder.Hook.StopCheck
...
def outputLine (warnings : List String) : Option String :=
  ...
  | [] => none
  | ws =>
    let message := "[Atlas Builder stop_check] " ++ String.intercalate " / " ws
    some (Value.render (.obj [("systemMessage", .str message)]))

/-! ## コンパイル時テスト -/

private def parsed (s : String) : Option Value := (AtlasBuilder.Core.Json.parse s).toOption
...
#guard outputLine ["X: validation.json still reports errors (2)."]
    == some "{\"systemMessage\": \"[Atlas Builder stop_check] X: validation.json still reports errors (2).\"}"

end AtlasBuilder.Hook.StopCheck

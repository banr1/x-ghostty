...
/-!
`AtlasBuilder.Hook.PreCompact` — PreCompact hook(`atlas-builder hook pre-compact`)の
純粋決定核(META.md §14.1)。
...
**I-021 の型設計(§3.3-5 の初出)**: 既存 canonical file の読み取り結果を
...
namespace AtlasBuilder.Hook.PreCompact
...
def noteText : String :=
  "Context compacted. Recover from disk: read project_index.json, then the active project's .atlas-builder/state/context.json and validation.json."

private def entryOf (b : Breadcrumb) : Value :=
...
(`notes` キーを持つ)との形状差は意図的凍結 — 裁定待ち(昇格設計書 §8-1)。 -/
...
private def appended (data : Value) (b : Breadcrumb) : Action :=
  let render (v : Value) : Action := .write (v.renderPretty ++ "\n")
  match data.get? "compactions" with
  | some (.arr existing) =>
    render (data.set "compactions" (.arr (lastN 50 (existing ++ [entryOf b]))))
  | none => render (data.set "compactions" (.arr [entryOf b]))
  | some _ => .skip corruptCompactionsSkip

/-- 判定本体。書き込み内容が生成されるのは `absent`(初期スキーマ)と
`readable` な object だけ。それ以外はすべて skip(I-021)。 -/
def decide (read : CanonicalRead) (b : Breadcrumb) : Action :=
  match read with
  | .unreadable reason => .skip (unreadableSkip reason)
  | .absent => appended freshBase b
  | .readable (.obj entries) => appended (.obj entries) b
...
#guard actionText (decide (.readable
...
end AtlasBuilder.Hook.PreCompact

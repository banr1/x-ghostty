...
/-!
`AtlasBuilder.State.Predicates` — state エンジンの read-only 判定核(純粋層)。
...
namespace AtlasBuilder.State.Predicates
...
executor.mode == "atlas-builder" かつ(承認済み phase または must)。executor が
...
def todoIsRunnable (t : Json.Value) (phase : String := defaultPhase) : Bool :=
  ...
  (status == some (.str "pending") || status == some (.str "in_progress"))
    && ((t.get? "executor").bind (·.get? "mode")) == some (.str "atlas-builder")
...
def isPhaseGateRecommendation (rec : Json.Value) : Bool :=
  rec.get? "gate" == some (.str "must_complete_awaiting_phase_approval")
    && rec.get? "raised_by" == some (.str "atlas-builder-loop")
...
def stopAssemble (i : StopInputs) (phaseApprovalStop placeholderEssence : Bool)
  ...
  let parts : List String := List.flatten [
    ...
    if placeholderEssence then
      ["ESSENCE.md is still the Atlas Builder placeholder (empty, the "
...
private def runnableMust : Json.Value := .obj [
  ...
  ("executor", .obj [("mode", .str "atlas-builder")])
]
...
#guard todoIsRunnable (runnableMust.set "executor" (.str "atlas-builder")) == false
...
#guard (completionPayload
    { todo := .text "{\"items\": [{\"status\": \"done\", \"priority\": \"must\"}, {\"status\": \"blocked\", \"priority\": \"should\", \"executor\": {\"mode\": \"atlas-builder\"}}]}",
      spec := .absent, context := .text "{\"phase\": \"extended_approved\"}",
      essence := .absent }).toOption.map (·.complete) == some false
#guard (completionPayload
...
private def structStop : StopInputs := { cleanStop with
  ...
  project := .text ("{\"essence_sha256\": \""
    ++ AtlasBuilder.Core.Sha256.hexDigest (String.toUTF8 malformedEssence) ++ "\"}")
}
...
private def assetStop : StopInputs := { cleanStop with
  ...
  project := .text ("{\"essence_sha256\": \""
    ++ AtlasBuilder.Core.Sha256.hexDigest (String.toUTF8 assetEssenceText)
...
end AtlasBuilder.State.Predicates

...
/-!
`AtlasBuilder.State.Watch` — `atlas-builder state watch-render`(META.md §15.3)の純粋核。
...
画面制御は driver(`atlas-builder-watch.sh`)の責務。SGR(色)だけを `color` の
とき埋め、`--no-color` では ESC バイトゼロ(W-013)。色は状態語のみ(D7):
STOP=赤 / COMPLETE=緑 / drain=黄 / EVAL ERROR=赤反転。
-/

namespace AtlasBuilder.State.Watch
...
structure HeartbeatView where
  ...
  lock 保持者のものか」の判定材料になる(§19.1-8-e — heartbeat は loop 終了時に
...
`—` になる。canonical でない入力に fail-closed は要らない(§19.1-8-a)。 -/
  ...
  | .ok stop =>
    match Predicates.shouldStopPayload stop,
        Predicates.completionPayload
          { todo := stop.todo, spec := stop.spec,
            context := stop.context, essence := stop.essence } with
    | .ok sv, .ok cv =>
      let gate :=
        if sv.stop then
...
private def headerLine (i : WatchInputs) : Line :=
  ...
  let left := s!"atlas-builder watch — {sanitizeDisplay i.projectTitle}"
...
private def cycleLine (i : WatchInputs) (hb : HeartbeatRead) : Line :=
  ...
  | .ok h =>
    -- heartbeat は loop 終了時に削除されない(§19.1-8-e)ため、lock の生存
...
private def guardInputs (stop : Except String Predicates.StopInputs) : WatchInputs := {
  projectTitle := "sample", nowEpoch := 0,

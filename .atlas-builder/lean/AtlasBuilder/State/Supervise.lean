...
/-!
`AtlasBuilder.State.Supervise` — human-supervised High-Risk session の read-only
preflight。
...
namespace AtlasBuilder.State.Supervise
...
def preflight (i : Inputs) : Except String Json.Value := do
  ...
  if (todo.get? "executor").bind (·.get? "mode") != some (.str "atlas-builder") then
    throw s!"Todo {i.todoId} must have executor.mode=atlas-builder"
...
private def agrees (todoRaw recRaw : RawFile) : Bool :=
  ...
  let viaPredicate := recItems.any fun r =>
    Predicates.superviseAuthorizedTodo? r todoItems == some "T-HR"
...
end AtlasBuilder.State.Supervise

...
/-!
`AtlasBuilder.State.Status` — state.py `cmd_status`(L3077–3156)の純粋核
。
...
- float の `str()` は Python `repr` の最短表現を再現しない(§4.4 —
  現行出力に float は現れない。出現時は裁定キュー)。
-/

namespace AtlasBuilder.State.Status

open AtlasBuilder.Core
open AtlasBuilder.Core.Prim (hex2)
open AtlasBuilder.State.Model
...
mutual
  ...
  (int は一致。float の最短 repr は非再現 — §4.4)。 -/
...
def acceptanceEvidenceMap (conditions : List String)
  ...
  pure <| conditions.map fun cond =>
    let hits := (candidates.filter fun c => Text.textsRelate cond c.2).map (·.1)
...
def successConditions : EssenceRead → Except String (List String)
  ...
  | .file bytes =>
    ...
    | some text => .ok (Text.essenceSuccessItems text)
...
def statusRender (i : StatusInputs) : List String × Option String := Id.run do
  ...
  let (_, d7) := readDiag "context.json" g.context
  ...
  let completion ← match Predicates.completionPayload
      { todo := g.todo, spec := g.spec, context := g.context,
        essence := g.essence } with
    | .error e => return (lines, some e)
    | .ok v => pure v
  lines := lines ++ [
...
-- `essence_structure` で停止するため、フィクスチャも正準 9 節で書く
private def canonicalEssence (title motivation extra : String) : String :=
  "# ESSENCE — " ++ title ++ "\n"
    ++ "## モチベーション\n- " ++ motivation ++ "\n"
...
private def cleanStatus : StatusInputs := {
  ...
  stop := {
    ...
    project := .text ("{\"essence_sha256\": \""
      ++ AtlasBuilder.Core.Sha256.hexDigest (String.toUTF8 realEssence) ++ "\"}")
    spec := .text "{\"items\": [{\"id\": \"S-001\"}]}"
    todo := .text ("{\"items\": [{\"id\": \"T-001\", \"status\": \"pending\", "
      ++ "\"priority\": \"must\", \"executor\": {\"mode\": \"atlas-builder\"}}], "
      ++ "\"active_batch\": []}")
...
#guard (statusRender cleanStatus).1 == [
  ...
  "Active batch : none",
  "Blockers     : 0",
  "Validation   : ok (2026-07-16T00:00:00+09:00)",
  "Runs logged  : 0",
  "Phase        : must",
  "Must todos   : 0/1 done",
  "Idle cycles  : 0/3",
...
private def assetEssence : String :=
  canonicalEssence "Asset Test" "See essences/logo.png for the mark." ""
private def assetStatus : StatusInputs := cleanWith fun s => { s with
  essence := .file (String.toUTF8 assetEssence)
  project := .text ("{\"essence_sha256\": \""
    ++ AtlasBuilder.Core.Sha256.hexDigest (String.toUTF8 assetEssence)
    ++ "\", \"essences_manifest\": {\"logo.png\": \""
    ++ AtlasBuilder.Core.Sha256.hexDigest (String.toUTF8 "IMG") ++ "\"}}")
...
private def essenceWithSuccess : String :=
  canonicalEssence "Success Test" "A deterministic test project."
...
private def successStatus : StatusInputs := cleanWith fun s => { s with
  ...
  project := .text ("{\"essence_sha256\": \""
    ++ AtlasBuilder.Core.Sha256.hexDigest (String.toUTF8 essenceWithSuccess) ++ "\"}")

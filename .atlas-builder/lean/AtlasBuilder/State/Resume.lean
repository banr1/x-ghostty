...
/-!
`AtlasBuilder.State.Resume` — `state.py cmd_resume`(L2767–3074)の純粋決定核。
...
namespace AtlasBuilder.State.Resume
...
def preconditionError? (stateInitialized : Bool) (stateDir : String)
  ...
  if ← Predicates.essenceIsPlaceholder essence then
    return some ("ESSENCE.md is still the Atlas Builder placeholder (empty, the guide "
...
ため、must 未完了(direct = false)でも `raised_by: "atlas-builder-loop"` を詐称した
...
#guard match transition { probeInputs with
    ...
    blockersRaw := .text "{\"items\": []}"
    recsRaw := .text (gateRec (some "atlas-builder-loop"))
    resolveIds := []
...
-- raised_by=atlas-builder-loop を詐称した gate rec があっても --approve-should は
...
#guard match preconditionError? true "/x/state"
  ...
  | .ok (some msg) => msg.startsWith "ESSENCE.md is still the Atlas Builder placeholder"
  | _ => false
...
private def authorizingTodoRaw : RawFile := .text
  ("{\"items\": [{\"id\": \"T-HR\", \"title\": \"instantiate\", "
    ...
    ++ "\"executor\": {\"mode\": \"atlas-builder\"}, \"risk_level\": \"high\", "

...
/-!
`AtlasBuilder.State.Validate` — `cmd_validate` / `cmd_ensure` /
...
namespace AtlasBuilder.State.Validate

open AtlasBuilder.Core
open AtlasBuilder.State.Model
open AtlasBuilder.State.Status (pyStr pyRepr pyReprStr acceptanceEvidenceMap successConditions)
...
private def executorModes : List String := ["atlas-builder", "human", "blocked"]
...
def initialState (now title : String) (essenceSha : Option String)
  ...
  ("state_index.json", .obj [
    ...
    ("priority_order", .arr [
      .str "ESSENCE.md",
      .str ".atlas-builder/state/spec.json",
      .str ".atlas-builder/state/todo.json",
      .str "implementation"])]),
  ("project.json", .obj [
    ("schema_version", .str schemaVersion),
    ("id", .str "P-001"),
...
def wontViolationWarnings (wontItems : List String)
  ...
  if wontItems.isEmpty then []
    ...
    | some [] => []
    | some allChanges =>
      let changes := allChanges.filter (fun p => !p.startsWith ".atlas-builder/")
...
def essenceErrors (essence : EssenceRead) : Except String (List String) := do
  ...
  | _ =>
    if ← Predicates.essenceIsPlaceholder essence then
      pure ["ESSENCE.md is still the Atlas Builder placeholder "
...
#guard ((ensureIndexUpdate .absent false "demo" "../demo" "../demo/.atlas-builder"
    ...
    == some ("{\"schema_version\": \"1.0.0\", \"project\": {\"id\": \"P-001\", "
      ...
      ++ "\"project_root\": \"../demo\", \"project_state_root\": \"../demo/.atlas-builder\", "
      ++ "\"status\": \"active\"}}")
#guard (ensureIndexUpdate (.text ("{\"schema_version\": \"1.0.0\", \"project\": "
    ++ "{\"id\": \"P-001\", \"created_at\": \"t\", \"title\": \"demo\", "
    ++ "\"project_root\": \"../demo\", \"project_state_root\": \"../demo/.atlas-builder\", "
    ++ "\"status\": \"active\"}}")) true "demo" "../demo" "../demo/.atlas-builder" "now")
    matches .ok none
...
#guard wontViolationWarnings ["依存を増やさない"]
    (some ["package.json", ".atlas-builder/state/todo.json"])
...
#guard wontViolationWarnings ["`legacy/` は触らない"] (some [".atlas-builder/state/x.json"]) == []
...
#guard (todoTraceChecks [] (.obj [("id", .str "T-1"), ("title", .str "t"),
      ...
      ("spec", .arr [.str "S-1"]),
      ("executor", .obj [("mode", .str "atlas-builder")])])).1
...
#guard (todoTraceChecks [(.str "S-1", .obj [])] (.obj [("id", .str "T-1"),
      ...
      ("spec", .arr [.str "S-1"]),
      ("executor", .obj [("mode", .str "atlas-builder")])])).2
...
end AtlasBuilder.State.Validate

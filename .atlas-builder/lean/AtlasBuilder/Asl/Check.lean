...
/-!
`AtlasBuilder.Asl.Check` — agentic-state-loop レシピの schema 駆動ミニバリデータ
...
- `pattern` キーワードは Python regex(`re.search`)ではなく §29.6 の制限
  グロブ方言(`AtlasBuilder.Core.Glob`、search 意味論)で判定する。
...
namespace AtlasBuilder.Asl.Check

open AtlasBuilder.Core
open AtlasBuilder.Core.Prim (hex2)
open AtlasBuilder.State.Model (pyGE? numEq)

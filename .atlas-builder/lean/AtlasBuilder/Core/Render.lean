...
/-!
`AtlasBuilder.Core.Render` — バインドテンプレート描画とシード置換の純粋核。

置換元は `atlas-builder-init.sh` の `render()` インライン Python(トークンの
文脈依存置換: settings.json は JSON 文字列エスケープ、justfile は Just
文字列エスケープ、Markdown は生のまま)と、`atlas-builder-bootstrap.sh` の
`seed()` / init の workspace README シード(単純な逐次トークン置換)。

バインドトークンは `__ATLAS_BUILDER_PROJECT_TITLE__`(basename)と
`__ATLAS_BUILDER_PROJECT_PATH__`(CONTROL_ROOT からの相対パス)の 2 種のみ
...
namespace AtlasBuilder.Core.Render
...
/-- バインドテンプレート描画(`atlas-builder-init.sh` `render()` の純粋核)。
...
def renderBind (mode : Mode) (b : Binding) (text : String) : Except String String := do
  ...
  let out := substitute text [
    ("__ATLAS_BUILDER_PROJECT_TITLE__", e.title),
    ("__ATLAS_BUILDER_PROJECT_PATH__", e.rel)]
  if mode == .json then
...
#guard (renderBind .raw {title := "t", rel := "../t"}
    "T=__ATLAS_BUILDER_PROJECT_TITLE__ P=__ATLAS_BUILDER_PROJECT_PATH__").toOption
    == some "T=t P=../t"
#guard (renderBind .json {title := "t\"x", rel := "../t"}
    "{\"title\": \"__ATLAS_BUILDER_PROJECT_TITLE__\"}").toOption
    == some "{\"title\": \"t\\\"x\"}"
#guard (renderBind .json {title := "t", rel := "../t"}
    "not json __ATLAS_BUILDER_PROJECT_TITLE__").isOk == false
#guard (match renderBind .raw {title := "bad\ntitle", rel := "../t"} "x" with
  | .error e => e == "project title contains a control character; refusing to render templates"
  | .ok _ => false)
#guard (renderBind .just {title := "t", rel := "p\\q"}
    "\"__ATLAS_BUILDER_PROJECT_PATH__\"").toOption == some "\"p\\\\q\""

end AtlasBuilder.Core.Render

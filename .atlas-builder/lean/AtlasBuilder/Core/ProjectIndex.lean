...
/-!
`AtlasBuilder.Core.ProjectIndex` — §8.1 registry(project_index.json の単一
`project` オブジェクト)の形状判定の純粋核。

置換元は `atlas-builder-init.sh` の 2 つのインライン Python:
...
namespace AtlasBuilder.Core.ProjectIndex
...
end AtlasBuilder.Core.ProjectIndex

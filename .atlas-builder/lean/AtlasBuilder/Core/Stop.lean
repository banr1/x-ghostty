...
/-!
`AtlasBuilder.Core.Stop` — stop-reason 優先順の唯一の定義箇所(D-005)。

シェル側は `atlas-builder util stop-status` 経由で本モジュールを実体として使い、
...
namespace AtlasBuilder.Core.Stop

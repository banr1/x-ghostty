...
/-!
`AtlasBuilder.Core.Classify` — guard の分類カタログ・第 1 部: パス分類 4 群と
...
open AtlasBuilder.Core.Text (isPySpace pySplitWs containsSub runsOf dedup)
...
`(^|/)\.atlas-builder/state/project\.json$`。`essences/` は ESSENCE.md と同格の
...
機械強制): これらは `bin/atlas-builder state apply-projection` の事前検証を通して
のみ書かれるべきで、Edit ツールでの直接上書きは検証を迂回して不正な投影を
書ける。Bash mutator 経路(`isProtectedWriteTarget` が `.atlas-builder/state/` を
...
def isDenyPath (cand : String) : Bool :=
  atBoundaryEndCI "ESSENCE.md" cand
    ...
    || atBoundaryEndCI ".atlas-builder/state/project.json" cand
    || atBoundaryEndCI ".atlas-builder/state/spec.json" cand
    || atBoundaryEndCI ".atlas-builder/state/todo.json" cand
    || atBoundaryEndCI ".atlas-builder/state/recommendations.json" cand
    || atBoundaryEndCI ".atlas-builder/state/blockers.json" cand
...
`atlas-builder state --todo T-1 resume` のような flag-first 綴りが resume /
...
/-- `state.py` 直後、`atlas-builder state` 直後(agent 向け配線面。`state` が
...
private def afterStateEntry : List String → Option (List String)
  ...
  | tok :: rest =>
    ...
    else if lastComponent tok == "atlas-builder" then
      match rest with
...
(`bash -c 'bin/atlas-builder state ... resume'` — **実行される**文字列)も同じ
...
#guard isDenyPath ".atlas-builder/state/project.json" == true
#guard isDenyPath "p/.atlas-builder/state/project.json" == true
#guard isDenyPath ".atlas-builder/state/project.jsonx" == false
-- apply-projection 正本 4 ファイルの Edit 経路 deny(§20.2-8 機械強制)
#guard isDenyPath ".atlas-builder/state/spec.json" == true
#guard isDenyPath ".atlas-builder/state/todo.json" == true
#guard isDenyPath ".atlas-builder/state/recommendations.json" == true
#guard isDenyPath ".atlas-builder/state/blockers.json" == true
#guard isDenyPath "p/.atlas-builder/state/todo.json" == true
-- agent が直接書く面は deny しない(context / reflection / high_risk_changes)
#guard isDenyPath ".atlas-builder/state/context.json" == false
#guard isDenyPath ".atlas-builder/state/reflection.jsonl" == false
#guard isDenyPath ".atlas-builder/state/high_risk_changes.jsonl" == false
#guard isDenyPath ".atlas-builder/state/validation.json" == false
...
#guard isControlPlaneHighRisk "bin/atlas-builder" == true
#guard isControlPlaneHighRisk "lean/AtlasBuilder/Guard/Decide.lean" == true
#guard isControlPlaneHighRisk "lean/lakefile.lean" == true
#guard isControlPlaneHighRisk "xbin/atlas-builder" == false
...
-- atlas-builder state 綴り(agent 向け配線面)
#guard statePySubcommands "bin/atlas-builder state resume --project x" == ["resume"]
#guard statePySubcommands "bin/atlas-builder state --project X resume" == ["resume"]
#guard statePySubcommands "/cr/bin/atlas-builder state start-run" == ["start-run"]
#guard statePySubcommands "atlas-builder state status" == ["status"]
#guard statePySubcommands "bin/atlas-builder trust ensure" == []
#guard statePySubcommands "atlas-builder util token state resume" == []
-- shell の `-c` 引数(= 実行される文字列)は同じ遮断に載る。実行されない
-- 文字列(`echo '...'`・heredoc 本文)は対象外という境界も同時に凍結する。
#guard statePySubcommands "bash -c 'bin/atlas-builder state --project x resume'"
  == ["resume"]
#guard statePySubcommands "sh -c 'just state resume'" == ["resume"]
#guard statePySubcommands "bash -lc 'atlas-builder state start-run'" == ["start-run"]
#guard statePySubcommands "env bash -c 'atlas-builder state resume'" == ["resume"]
#guard statePySubcommands "echo 'state.py resume'" == []
...
#guard statePySubcommands "env /cr/bin/atlas-builder state resume" == ["resume"]
#guard statePySubcommands "command atlas-builder state validate" == ["validate"]
#guard statePySubcommands "echo atlas-builder; state.py validate" == ["validate"]
#guard statePySubcommands "atlas-builder; atlas-builder state validate" == ["validate"]
...
#guard statePySubcommands "bin/atlas-builder state --todo T-001 resume --project x"
  == ["resume"]
#guard statePySubcommands "bin/atlas-builder state --recommendation R-001 resume"
  == ["resume"]
#guard statePySubcommands "atlas-builder state --todo T-1 start-run" == ["start-run"]
#guard statePySubcommands "atlas-builder state --todo T-1 raise-loop-gates"
  == ["raise-loop-gates"]
#guard statePySubcommands "env atlas-builder state resume" == ["resume"]
#guard statePySubcommands "bin/atlas-builder state" == []

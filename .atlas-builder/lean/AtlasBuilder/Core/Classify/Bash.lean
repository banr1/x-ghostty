...
/-!
`AtlasBuilder.Core.Classify.Bash` — guard の分類カタログ・第 2 部: Bash コマンド
...
namespace AtlasBuilder.Core.Classify

open AtlasBuilder.Core.Prim (dropLit?)
open AtlasBuilder.Core.Text (isPySpace runsOf dedup containsSub)
open AtlasBuilder.Core.Frag
...
/-- `(?:loop|stop|once|init|doctor|trust(?:-check)?|triage|supervise|new-essence|update-essence|watch)`
...
def isHumanOnlyBash (command : String) : Bool :=
  search (lit "state.py" >=> ws1 >=> lit "resume" >=> wordEnd) command
    ...
    || searchAtBoundary
        (lit "atlas-builder" >=> ws1 >=> lit "state" >=> ws1 >=> lit "resume" >=> wordEnd) command
    || containsSub command "atlas-builder-resume.sh"
    || searchJustPrefix (lit "resume" >=> wordEnd) command
    || searchJustPrefix (lit "state" >=> ws1 >=> lit "resume" >=> wordEnd) command
    || ["atlas-builder-loop.sh", "atlas-builder-stop.sh", "atlas-builder-once.sh",
        "atlas-builder-init.sh", "atlas-builder-doctor.sh", "atlas-builder-triage.sh",
        "atlas-builder-essence.sh", "atlas-builder-supervise.sh",
        "atlas-builder-watch.sh"].any (containsSub command)
    ...
    || searchAtBoundary
        (lit "atlas-builder" >=> ws1 >=> lit "trust" >=> ws1 >=> lit "ensure" >=> wordEnd) command
...
def isLoopOnlyBash (command : String) : Bool :=
  search (lit "state.py" >=> ws1 >=> loopOnlySubTail) command
    || searchAtBoundary
        (lit "atlas-builder" >=> ws1 >=> lit "state" >=> ws1 >=> loopOnlySubTail) command
...
def isAskBash (command : String) : Bool :=
  containsSub command "atlas-builder-bootstrap.sh"
...
/-- `atlas-builder-[A-Za-z-]+\.sh`。クラスに `.` が無いため貪欲一致が唯一の分割。 -/
private def atlasBuilderShFrag : Frag := fun cs =>
  (dropLit? cs "atlas-builder-".toList).bind fun rest =>
...
private def controlSurfaceTail : Frag :=
  alts
    [ lit "scripts/"
        >=> alts [lit "state.py", lit "_lib.sh", atlasBuilderShFrag, claudeTrustFrag],
...
/-- `ESSENCE\.md|essences/|\.atlas-builder/state/|\.env($|\.|\s)|secrets/|config/credentials\.json|essence_attestations\.jsonl|\.agent/state/`
...
private def protectedTargetText (text : String) : Bool :=
  containsSub text "ESSENCE.md"
    ...
    || containsSub text ".atlas-builder/state/"
    || search
        (lit ".env"
...
外せた(`cp a ../p/'ESSENCE'.md`、`echo x > ../p/.atlas-builder/'state'/todo.json`)。
...
#guard isHumanOnlyBash "atlas-builder-loop.sh" == true
#guard isHumanOnlyBash "bash atlas-builder-loop.sh" == true
#guard isHumanOnlyBash "aatlas-builder-loop.sh" == true
#guard isHumanOnlyBash "atlas-builder-loopx.sh" == false
#guard isHumanOnlyBash "claude_trust.py ensure" == true
#guard isHumanOnlyBash "claude_trust.py status" == false
#guard isHumanOnlyBash "atlas-builder trust ensure" == true
#guard isHumanOnlyBash "bin/atlas-builder trust ensure" == true
#guard isHumanOnlyBash "xatlas-builder trust ensure" == false
#guard isHumanOnlyBash "atlas-builder trust status" == false
#guard isHumanOnlyBash "just doctor" == true
#guard isHumanOnlyBash "atlas-builder-doctor.sh" == true
...
-- `atlas-builder-stop.sh` は human-only。ただし state 述語の読み取り
-- (`should-stop`、G-008 の triage allow 面を含む)は侵食しない —
-- `stop` の代替は `\bjust\s+…stop\b` の位置でしか照合せず、
-- `atlas-builder-stop.sh` は基底名の部分文字列照合だからである。
#guard isHumanOnlyBash "just stop" == true
#guard isHumanOnlyBash "just --unstable stop" == true
#guard isHumanOnlyBash "just stop --cancel" == true
#guard isHumanOnlyBash "just stopx" == false
#guard isHumanOnlyBash "atlas-builder-stop.sh" == true
#guard isHumanOnlyBash "bash scripts/atlas-builder-stop.sh --project ../x --cancel" == true
#guard isHumanOnlyBash "bin/atlas-builder state should-stop --project ../x" == false
#guard isHumanOnlyBash "just state should-stop" == false
#guard isHumanOnlyBash "atlas-builder-status.sh" == false
-- 常駐監視 TUI(§15.3)は human-only: `just watch` / `atlas-builder-watch.sh` は
-- deny。単発描画の読み取り面 `state watch-render` は非侵食(D1)。
#guard isHumanOnlyBash "just watch" == true
#guard isHumanOnlyBash "just --unstable watch" == true
#guard isHumanOnlyBash "just watchx" == false
#guard isHumanOnlyBash "atlas-builder-watch.sh" == true
#guard isHumanOnlyBash "bash scripts/atlas-builder-watch.sh --project ../x" == true
#guard isHumanOnlyBash "atlas-builder-watchx.sh" == false
#guard isHumanOnlyBash "bin/atlas-builder state watch-render --project ../x" == false
#guard isHumanOnlyBash "just state watch-render" == false
...
#guard isHumanOnlyBash "atlas-builder state resume" == true
#guard isHumanOnlyBash "bin/atlas-builder state resume" == true
#guard isHumanOnlyBash "./bin/atlas-builder state resume --project ../p" == true
#guard isHumanOnlyBash "xatlas-builder state resume" == false
#guard isHumanOnlyBash "atlas-builder state resumex" == false
#guard isHumanOnlyBash "atlas-builder state validate" == false
...
#guard isLoopOnlyBash "bin/atlas-builder state start-run" == true
#guard isLoopOnlyBash "atlas-builder state record-progress --project ../p" == true
#guard isLoopOnlyBash "atlas-builder state resume" == false
#guard isLoopOnlyBash "xatlas-builder state end-run" == false
#guard isLoopOnlyBash "atlas-builder state --project x end-run" == false
...
-- isAskBash
#guard isAskBash "atlas-builder-bootstrap.sh" == true
#guard isAskBash "bash scripts/atlas-builder-bootstrap.sh" == true
...
#guard isProtectedWriteTarget ".atlas-builder/state/x" == true
...
#guard isControlSurfaceBashMutation "rm scripts/atlas-builder-foo.sh" == true
#guard isControlSurfaceBashMutation "rm scripts/atlas-builder-foo2.sh" == true
...
end AtlasBuilder.Core.Classify

...
/-!
`AtlasBuilder.Core.Classify.ReadOnly` — read-only セッション(I-022 triage /
...
5. **triage の state 読み取り形状**(`triageStateShape?` /
   ...
   <args...>`(argv 3 個以上、`bin/atlas-builder state` 綴り)
   から binary 生パスと唯一の `--project` 生値を取る。バイナリ実体照合
   (CONTROL_ROOT/bin/atlas-builder への解決一致)と project 解決照合は IO。
...
open AtlasBuilder.Core.Text (isPySpace splitLinesBash pyStrip)
...
(CONTROL_ROOT/bin/atlas-builder への解決一致)は IO 側の表引きが行う。 -/
...
def triageStateSubcommandsOk (command : String) : Bool :=
...
-- triageStateShape?
#guard triageStateShape? ["bin/atlas-builder", "state", "--project", "p", "status"]
  == some ("bin/atlas-builder", "p")
#guard triageStateShape? ["bin/atlas-builder", "state", "--project=p", "status"]
  == some ("bin/atlas-builder", "p")
#guard triageStateShape? ["/cr/bin/atlas-builder", "state", "--project", "p", "status"]
  == some ("/cr/bin/atlas-builder", "p")
#guard triageStateShape? ["bin/atlas-builder", "status", "--project", "p"] == none
#guard triageStateShape? ["bin/atlas-builder", "state"] == none
#guard triageStateShape? ["bin/atlas-builder", "state", "status"] == none
...
#guard triageStateSubcommandsOk "bin/atlas-builder state --project p should-stop"
  == true
#guard triageStateSubcommandsOk "bin/atlas-builder state --project p status" == true
#guard triageStateSubcommandsOk "bin/atlas-builder state --project p validate"
  == false
#guard triageStateSubcommandsOk "bin/atlas-builder state --project p" == false
#guard triageStateSubcommandsOk
  "bin/atlas-builder state status; bin/atlas-builder state should-stop" == true
#guard triageStateSubcommandsOk "python3 scripts/state.py --project p status" == true

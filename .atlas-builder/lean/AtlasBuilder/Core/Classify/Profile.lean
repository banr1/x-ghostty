...
/-!
`AtlasBuilder.Core.Classify.Profile` — ESSENCE.md の第 3 の機械可読 directive
...
`_atlas_builder_profile` 行の値としてだけ焼き込まれる — relaxed の実効
...
open AtlasBuilder.Core.Text (pyStrip semanticLines)
...
def profileTemplateLine (p : Profile) : String :=
  "  \"_atlas_builder_profile\": \"" ++ p.tag ++ "\",\n"
...
派生は `_atlas_builder_profile` 行の exactly-once 置換 1 本に縮小されている。
...
def deriveProfileTemplate (p : Profile) (template : String) : Option String :=
  replaceExactlyOnce template (profileTemplateLine .standard)
    (profileTemplateLine p)
...
#guard deriveProfileTemplate .autoApprove sampleTemplateFragment
  == some ("  \"model\": \"best\",\n"
    ++ "  \"_atlas_builder_profile\": \"auto-approve\",\n"
    ++ "  \"permissions\": {\n")
#guard deriveProfileTemplate .unsandboxed sampleTemplateFragment
  == some ("  \"model\": \"best\",\n"
    ++ "  \"_atlas_builder_profile\": \"unsandboxed\",\n"
    ++ "  \"permissions\": {\n")
-- fail-closed: profile 行が欠けた・重複したテンプレは standard 含む全派生を
-- 拒否する(存在検証は 3 態で対称)
#guard deriveProfileTemplate .standard "no profile line here" == none
#guard deriveProfileTemplate .autoApprove "no profile line here" == none
#guard deriveProfileTemplate .unsandboxed
    (sampleTemplateFragment ++ sampleTemplateFragment) == none
-- インデントや引用の崩れた行は一致しない(綴りは profileTemplateLine の
-- 1 定義点)
#guard deriveProfileTemplate .autoApprove
    "\"_atlas_builder_profile\": \"standard\",\n" == none

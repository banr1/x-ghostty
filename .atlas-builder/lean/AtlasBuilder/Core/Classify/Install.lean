...
/-!
`AtlasBuilder.Core.Classify` 第 3 部 — 依存インストールゲート(META.md §11.2)の
...
4. `install_allow_reason` の純粋部分 — `installGateShape`(`&&` 分割・
   ...
   許可ルート照合は IO の責務**(§3.3-4): 呼び出し側が `installGateShape` の
...
open AtlasBuilder.Core.Text (isPySpace pyStrip containsSub)
...
def trustedInstallPackage (declared : DeclaredPackages) :
  ...
  | .lean, token =>
    ...
    | some name => trustedLean.contains name || declared.lean.contains name
    | none => false
...
def installAllowSegReason? (declared : DeclaredPackages) (seg : String) :
    Option String :=
  ...
  | .ok rawArgv =>
    ...
    | none =>
      ...
      | some (eco, rest) =>
        ...
        else if packages.all (trustedInstallPackage declared eco) then
...
#guard trustedInstallPackage declTest .npm "react" == true
#guard trustedInstallPackage declTest .npm "leftpad@1.0" == true
#guard trustedInstallPackage declTest .npm "@types/node@20" == true
#guard trustedInstallPackage declTest .npm "unknown" == false
#guard trustedInstallPackage declTest .pypi "PyTest" == true
#guard trustedInstallPackage declTest .pypi "My_Pkg==1.0" == true
#guard trustedInstallPackage declTest .pypi "my_pkg[x]" == true
#guard trustedInstallPackage declTest .crates "serde_json" == true
#guard trustedInstallPackage declTest .crates "foo_bar@1" == true
#guard trustedInstallPackage declTest .go "golang.org/x/tools" == true
#guard trustedInstallPackage declTest .go "example.com/m@v1" == true
#guard trustedInstallPackage declTest .go "example.com/mm" == false
#guard trustedInstallPackage declTest .lean "mathlib" == true
#guard trustedInstallPackage declTest .lean "my-lake-dep" == true
#guard trustedInstallPackage declTest .lean "Mathlib" == false  -- 大小文字有意
#guard trustedInstallPackage declTest .lean "unknown-dep" == false
...
#guard installAllowSegReason? declTest "npm install react"
...
#guard installAllowSegReason? declTest "npm install react vue"
...
#guard installAllowSegReason? declTest "npm install unknownpkg" == none
#guard installAllowSegReason? declTest "npm install 'react'"
  == installAllowSegReason? declTest "npm install react"
#guard installAllowSegReason? declTest "npm install -D typescript"
  == some ("Trusted dependency install (META.md §11.2): typescript (npm) are "
    ++ "all on the curated list or declared in the project's ESSENCE.md; "
    ++ "anything else still requires the human.")
#guard installAllowSegReason? declTest "npm install --unknown react" == none
#guard installAllowSegReason? declTest "npm install react@18"
  == some ("Trusted dependency install (META.md §11.2): react@18 (npm) are "
    ++ "all on the curated list or declared in the project's ESSENCE.md; "
    ++ "anything else still requires the human.")
#guard installAllowSegReason? declTest "npm install" -- materialization が先勝ち
...
#guard installAllowSegReason? declTest "go mod download"
...
#guard installAllowSegReason? declTest "pip install -r requirements.txt"
  == some ("Materialization (META.md §11.2): installs requirements file(s): "
    ...
    ++ "edits ask).")
#guard installAllowSegReason? declTest "uv add my_pkg"
  == some ("Trusted dependency install (META.md §11.2): my_pkg (pypi) are all "
    ++ "on the curated list or declared in the project's ESSENCE.md; anything "
    ++ "else still requires the human.")
#guard installAllowSegReason? declTest "npm install 'react" == none
-- post-M3 追加ツール(理由文字列は既存形をそのまま流用)
#guard installAllowSegReason? declTest "lake update mathlib"
  == some ("Trusted dependency install (META.md §11.2): mathlib (lean) are all "
    ++ "on the curated list or declared in the project's ESSENCE.md; anything "
    ++ "else still requires the human.")
#guard installAllowSegReason? declTest "lake update my-lake-dep"
  == some ("Trusted dependency install (META.md §11.2): my-lake-dep (lean) are "
    ++ "all on the curated list or declared in the project's ESSENCE.md; "
    ++ "anything else still requires the human.")
#guard installAllowSegReason? declTest "lake update unknown-dep" == none
#guard installAllowSegReason? declTest "lake update"  -- bare は materialization
  == some ("Materialization (META.md §11.2): installs the manifest/lockfile "
    ++ "already in the repository, naming no package of its own. The "
    ++ "dependency set it expands was already gated when the manifest was "
    ++ "written (manifest edits ask).")
#guard installAllowSegReason? declTest "cargo install --locked ripgrep" == none
#guard installAllowSegReason? declTest "poetry add pytest"
  == some ("Trusted dependency install (META.md §11.2): pytest (pypi) are all "
    ++ "on the curated list or declared in the project's ESSENCE.md; anything "
    ++ "else still requires the human.")
#guard installAllowSegReason? declTest "bun add react"
  == some ("Trusted dependency install (META.md §11.2): react (npm) are all "
    ++ "on the curated list or declared in the project's ESSENCE.md; anything "
    ++ "else still requires the human.")

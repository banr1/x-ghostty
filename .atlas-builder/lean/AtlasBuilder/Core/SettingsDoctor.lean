...
/-!
`AtlasBuilder.Core.SettingsDoctor` — doctor の settings 安全配線検査の純粋核。

settings の base/overlay 分割(§16.1)に対応して検査も 2 面である:

- `checkBase` — git 追跡の `.claude/settings.json`(マシン非依存)。B 項目
  (permission 床・hooks 配線)に加え、`_atlas_builder_profile` の存在と ESSENCE
...
namespace AtlasBuilder.Core.SettingsDoctor
...
private def expectedHooks : List (String × String) :=
  [("SessionStart", "/bin/atlas-builder hook session-start"),
   ("PreToolUse", "/bin/atlas-builder hook pre-tool"),
   ("PostToolUse", "/bin/atlas-builder hook post-tool"),
   ("PreCompact", "/bin/atlas-builder hook pre-compact"),
   ("Stop", "/bin/atlas-builder hook stop")]

/-- base(git 追跡の `.claude/settings.json`)の検査。B 項目 = マシン非依存の
permission 床と hooks 配線。加えて (a) `_atlas_builder_profile` の存在・閉集合・
...
なので re-run init(§16.1)。旧形式検出は `_atlas_builder_profile` キーが存在する
...
def checkBase (settings : Value) (essenceProfile : Profile) :
  ...
  -- §16.1 / §11.5: profile は base の `_atlas_builder_profile` 1 行が正本
  -- (launcher の overlay 生成が読む実使用値)。ESSENCE 宣言との突合で
  -- 「宣言だけ変えて init を忘れた」配備を検出する。
  let profileKey := settings.get? "_atlas_builder_profile"
  match profileKey with
  | none =>
    issues := issues.push <|
      "settings.json carries no _atlas_builder_profile: this control plane is " ++
      "unbound or pre-split legacy; run `just init` to bind it (§16.1)"
  | some v =>
    match v.asStr? with
    | none =>
      issues := issues.push
        "_atlas_builder_profile must be a string (standard | auto-approve | unsandboxed, §11.5); re-run init"
    | some tag =>
      match Profile.parse? tag with
      | none =>
        issues := issues.push <|
          s!"_atlas_builder_profile {reprStr tag} is not in the closed set " ++
          "standard | auto-approve | unsandboxed (§11.5); re-run init"
      | some p =>
        if p != essenceProfile then
          issues := issues.push <|
            s!"_atlas_builder_profile {reprStr p.tag} does not match the " ++
  ...
  --
  -- `_atlas_builder_profile` キーが存在するとき **だけ** 走らせる。キー不在の base
  ...
  -- 本切替後の配線(META.md §31.2-42): 全 5 イベントが `bin/atlas-builder hook <event>`。
  ...
  for (_, groupList) in hookEntries do
    for command in groupCommands groupList do
      if containsSubstr command "python3" || containsSubstr command "ATLAS_BUILDER_GUARD_SHADOW" then
...
ここに独立列挙する。`profile` は base の `_atlas_builder_profile`(launcher の
...
共通である(§11.5)。 -/
def checkOverlay (overlay : Value) (project control cwd : String)
...
end AtlasBuilder.Core.SettingsDoctor

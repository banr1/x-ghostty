...
/-!
`AtlasBuilder.Core.Runs` — runs.jsonl の未終了 run 走査の純粋核。

置換元は `atlas-builder-doctor.sh` のインライン Python: `start`/`end` イベントを
...
namespace AtlasBuilder.Core.Runs

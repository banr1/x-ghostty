...
/-!
`atlas-builder state watch-render` — watch の 1 フレーム描画(IO シェル、META.md
§15.3)。
...
validation / tails — driver `atlas-builder-watch.sh` が観測してフラグ・パスで渡す)。
...
namespace AtlasBuilder.Cli.Watch

open AtlasBuilder.Core
open AtlasBuilder.State

private def usage : String :=
  "usage: atlas-builder state watch-render --project <PROJECT_ROOT> --cols <n> --rows <n> --now <epoch> [--no-color] [--lock-pid <pid>] [--lock-alive yes|no] [--drain-pending] [--heartbeat <path>] [--tool-audit-tail <path>] [--transcript-tail <path>]"
...
def run (args : List String) : IO UInt32 := do
  let some parsed := parseArgs args {}
    | return ← die usage
  match ← Cli.State.resolveCtx parsed.project with
  | .error msg => die msg
  | .ok ctx =>

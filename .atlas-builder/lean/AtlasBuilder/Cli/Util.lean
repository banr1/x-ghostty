...
/-!
`AtlasBuilder.Cli.Util` — シェルスクリプトが依存する小さな判定・抽出サブコマンド群
(R-7: シェル内に判定ロジックを書かず `atlas-builder util <subcmd>` へ収容する)。
...
- `bound-settings <project> <control>`  2 パスを実体解決し、base
  (`{control}/.claude/settings.json`)の `_atlas_builder_profile` を読んで
  loop / supervise の bound overlay JSON を 1 行出力する(構築は
  `Core.Settings.boundSession`、§16.1)。base が読めない・パース不能・
  `_atlas_builder_profile` が欠落/不正(= unbound か旧形式)は stderr に
...
- `loop-heartbeat <path> --project T --cycle N --max-cycles N --run-id R
  ...
  §19.1-8 の表示専用 heartbeat(スキーマ v1)を atomic に書く(tmp→rename —
  watch の 1s ポーリングに破れた読みを見せない)。`started_at`(+`_epoch`)は
  書き込み時点の現在時刻で、ISO 整形は `ATLAS_BUILDER_TZ` のオフセット方言
...
- `settings-doctor <settings.json> <project> <control> [<profile>]`  doctor の
  ...
  `_atlas_builder_profile` の存在・ESSENCE 宣言との突合 + 旧形式検出)にかけ、
...
namespace AtlasBuilder.Cli.Util

open AtlasBuilder

private def usage : String :=
  "usage: atlas-builder util {json-get|stop-status|token|file-age-exceeds|read-only-settings|bound-settings|loop-heartbeat|relpath|render|seed|project-index|json-check|dangling-run|settings-doctor|essence-profile|static-config} ..."
...
private def runJsonGet (args : List String) : IO UInt32 := do
  let some parsed := parseJsonGetArgs args
    | do
      IO.eprintln "usage: atlas-builder util json-get <keypath> [--file <path>] [--default <value>] [--join <sep>] [--lenient]"
      pure 2
  ...
  | .error e =>
    if parsed.lenient then
      ...
      IO.eprintln s!"atlas-builder: json-get: {e}"
      pure 2
...
private def runStopStatus : IO UInt32 := do
  let text ←
    try
      ...
      IO.eprintln "atlas-builder: stop-status: cannot read stdin"
      return 2
  ...
  | .ok status =>
    ...
    pure 0
  | .error e =>
    IO.eprintln s!"atlas-builder: stop-status: {e}"
    pure 2
...
private def runToken : IO UInt32 := do
  let bytes ←
    try
      ...
      IO.eprintln s!"atlas-builder: token: cannot read /dev/urandom: {e}"
      return 2
  if bytes.size != 32 then
    IO.eprintln s!"atlas-builder: token: short read from /dev/urandom ({bytes.size} bytes)"
    return 2
...
private def runFileAgeExceeds (path maxAgeStr : String) : IO UInt32 := do
  let some maxAge := maxAgeStr.toNat?
    | do
      IO.eprintln "usage: atlas-builder util file-age-exceeds <path> <seconds>"
      pure 2
...
private def runReadOnlySettings (project control handoff : String)
  ...
  let resolve (label p : String) : IO (Option String) := do
    ...
    | none => do
      IO.eprintln s!"atlas-builder: read-only-settings: cannot resolve {label}: {p}"
      pure none
...
private def runBoundSettings (project control : String) : IO UInt32 := do
  let resolve (label p : String) : IO (Option String) := do
    match ← Io.Fs.realPath? p with
    | some rp => pure (some rp.toString)
    | none => do
      IO.eprintln s!"atlas-builder: bound-settings: cannot resolve {label}: {p}"
      pure none
  let some projectR ← resolve "project root" project | pure 2
  let some controlR ← resolve "control root" control | pure 2
  let settingsPath := controlR ++ "/.claude/settings.json"
  -- §16.1 / §11.5: profile は init が base に焼いた `_atlas_builder_profile` だけ
  ...
  let some text ← Io.Fs.readFile? settingsPath
    | do
      IO.eprintln s!"atlas-builder: bound-settings: cannot read {settingsPath}; {initHint}"
      pure 2
  match Core.Json.parse text with
  | .error e =>
    IO.eprintln s!"atlas-builder: bound-settings: {settingsPath} is not valid JSON: {e}"
    pure 2
  | .ok settings =>
    match Core.Settings.baseProfile? settings with
    | none =>
      IO.eprintln <|
        s!"atlas-builder: bound-settings: {settingsPath} records no valid " ++
        s!"_atlas_builder_profile (unbound or pre-split legacy base); {initHint}"
      pure 2
    | some profile =>
      IO.println (Core.Settings.boundSession projectR controlR profile).renderCompactAscii
      pure 0
...
private def renderUsage : String :=
  "usage: atlas-builder util render <src> <dst> {json|just|raw} --title <t> --rel <r> [--profile standard|auto-approve|unsandboxed]"
...
private def runRender (src dst mode : String) (flags : List String) : IO UInt32 := do
  ...
  let profile? ← match profileStr? with
    ...
    | some s =>
      ...
      | some p =>
        if mode != .json then
          IO.eprintln
            "atlas-builder: render: --profile applies only to the json mode (settings.json.tmpl, §16.1)"
          return 2
        pure (some p)
      | none => do
        IO.eprintln s!"atlas-builder: render: unknown profile '{s}' (standard|auto-approve|unsandboxed)"
        return 2
  let some text ← Io.Fs.readFile? src
    | do IO.eprintln s!"atlas-builder: render: cannot read {src}"; pure 2
  -- §11.5: profile 派生(`_atlas_builder_profile` 行の exactly-once 置換)は
  ...
  let text ← match profile? with
    | none => pure text
    | some profile =>
      match Core.Classify.deriveProfileTemplate profile text with
      | some t => pure t
      | none => do
        IO.eprintln <|
          s!"atlas-builder: render: template does not match the profile derivation " ++
          s!"patterns (profile '{profile.tag}', §11.5); refusing to render"
        return 2
  match Core.Render.renderBind mode binding text with
  | .error e =>
    IO.eprintln s!"atlas-builder: render: {e}"
    pure 2
  | .ok out =>
    try
      ...
      IO.eprintln s!"atlas-builder: render: cannot write {dst}: {e}"
      pure 2
...
private def runSeed (src dst : String) (flags : List String) : IO UInt32 := do
  let some replacements := parseSeedReplacements flags []
    | do
      IO.eprintln "usage: atlas-builder util seed <src> <dst> [--replace TOKEN=VALUE]..."
      pure 2
  let some text ← Io.Fs.readFile? src
    | do IO.eprintln s!"atlas-builder: seed: cannot read {src}"; pure 2
  try
    Io.Fs.writeFileAtomicTagged dst "seed" (Core.Render.substitute text replacements)
    pure 0
  catch e =>
    IO.eprintln s!"atlas-builder: seed: cannot write {dst}: {e}"
    pure 2
...
private def runProjectIndex (action index target control : String) : IO UInt32 := do
  ...
  let some targetR ← Io.Fs.realPath? target
    | do IO.eprintln s!"atlas-builder: project-index: cannot resolve target: {target}"; pure 2
  let some controlR ← Io.Fs.realPath? control
    | do IO.eprintln s!"atlas-builder: project-index: cannot resolve control root: {control}"; pure 2
  ...
  | .absent => pure 0
  | .malformed =>
    IO.eprintln "atlas-builder: project-index: project.project_root has a non-string shape; repair project_index.json (§8.1)"
    pure 2
  | .registered title rel? =>
    ...
    if pointsAtTarget then
      ...
      | _ => do  -- prune
        ...
        IO.eprintln s!"[atlas-builder] pruned stale project_index entries: {title}"
        pure 0
...
private def runJsonCheck (path : String) : IO UInt32 := do
  let some text ← Io.Fs.readFile? path
    | do IO.eprintln s!"atlas-builder: json-check: cannot read {path}"; pure 1
  match Core.Json.parse text with
  | .ok _ => pure 0
  | .error e =>
    IO.eprintln s!"atlas-builder: json-check: {path}: {e}"
    pure 1
...
private def runSettingsDoctor (path project control profileStr : String) :
  ...
  let some essenceProfile := Core.Classify.Profile.parse? profileStr
    | do
      IO.eprintln s!"atlas-builder: settings-doctor: unknown profile '{profileStr}' (standard|auto-approve|unsandboxed)"
      pure 2
  let some text ← Io.Fs.readFile? path
  ...
  | .error e =>
    ...
    pure 0
  | .ok settings =>
    for issue in Core.SettingsDoctor.checkBase settings essenceProfile do
      IO.println issue
    ...
    | some profile =>
      let cwd := (← IO.currentDir).toString
      for issue in Core.SettingsDoctor.checkOverlay
          (Core.Settings.boundSession project control profile)
...
#guard [Core.Classify.Profile.standard, .autoApprove, .unsandboxed].all fun p =>
  Core.SettingsDoctor.checkOverlay
    (Core.Settings.boundSession "/w/proj" "/w/.atlas-builder" p)
    "/w/proj" "/w/.atlas-builder" "/w/.atlas-builder" p == []

-- standard overlay を relaxed 宣言で検査: ask 3 面の残存だけが issue
#guard Core.SettingsDoctor.checkOverlay
    (Core.Settings.boundSession "/w/proj" "/w/.atlas-builder" .standard)
    "/w/proj" "/w/.atlas-builder" "/w/.atlas-builder" .autoApprove
  == [
...
#guard Core.SettingsDoctor.checkOverlay
    (Core.Settings.boundSession "/w/proj" "/w/.atlas-builder" .autoApprove)
    "/w/proj" "/w/.atlas-builder" "/w/.atlas-builder" .unsandboxed
  == ["sandbox.enabled must be false (recorded profile 'unsandboxed', §11.5)"]

-- unsandboxed overlay を standard 宣言で検査: ask 3 面の欠落 + sandbox
#guard Core.SettingsDoctor.checkOverlay
    (Core.Settings.boundSession "/w/proj" "/w/.atlas-builder" .unsandboxed)
    "/w/proj" "/w/.atlas-builder" "/w/.atlas-builder" .standard
  == [
...
#guard Core.SettingsDoctor.checkOverlay (.obj [])
    "/w/proj" "/w/.atlas-builder" "/w/.atlas-builder" .standard
  == [
    "permissions.deny must contain 'Edit(//w/.atlas-builder/CLAUDE.md)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.deny must contain 'Edit(//w/.atlas-builder/.claude/agents/**)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.deny must contain 'Edit(//w/.atlas-builder/.claude/commands/**)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.deny must contain 'Edit(//w/.atlas-builder/.claude/rules/**)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.deny must contain 'Edit(//w/.atlas-builder/.claude/skills/**)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.deny must contain 'Edit(//w/.atlas-builder/scripts/**)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.deny must contain 'Edit(//w/.atlas-builder/templates/**)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.deny must contain 'Edit(//w/.atlas-builder/recipes/**)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.deny must contain 'Edit(//w/.atlas-builder/META.md)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.deny must contain 'Edit(//w/.atlas-builder/README.md)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.deny must contain 'Edit(//w/.atlas-builder/justfile)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.deny must contain 'Edit(//w/.atlas-builder/.agent/prompts/**)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.deny must contain 'Edit(//w/.atlas-builder/.agent/state/**)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.deny must contain 'Edit(//w/.atlas-builder/bin/**)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.deny must contain 'Edit(//w/.atlas-builder/lean/**)' (immutable CONTROL_ROOT, §11.3)",
    "permissions.ask must contain 'Edit(//w/.atlas-builder/.claude/settings.json)' (human-approved live-settings repair surface for read-only sessions, §28.5-3)",
    "permissions.ask must contain 'Edit(//w/proj/CLAUDE.md)' (project agent-runtime ask floor, §16.1; recorded profile 'standard')",
    "permissions.ask must contain 'Edit(//w/proj/.claude/**)' (project agent-runtime ask floor, §16.1; recorded profile 'standard')",
    "permissions.ask must contain 'Edit(//w/proj/.mcp.json)' (project agent-runtime ask floor, §16.1; recorded profile 'standard')",
    ...
    "sandbox.filesystem.allowWrite must permit CONTROL_ROOT/.agent/tmp (/w/.atlas-builder/.agent/tmp) for state and single-flight locks",
...
private def runEssenceProfile (path : String) : IO UInt32 := do
  ...
  | some text =>
    ...
    if Core.Text.containsSub text State.Model.placeholderMarker
      ...
      | .invalid raw =>
        IO.eprintln <|
          s!"atlas-builder: essence-profile: invalid profile value '{raw}' — " ++
          "the closed set is standard | auto-approve | unsandboxed (§11.5)"
        pure 2
      | .conflict =>
        IO.eprintln <|
          "atlas-builder: essence-profile: conflicting profile declarations — " ++
...
private def runStaticConfig (name : String) : IO UInt32 := do
  ...
  | some rendered =>
    ...
    pure 0
  | none =>
    IO.eprintln "usage: atlas-builder util static-config {workspace|project-index-seed}"
    pure 2

/-! ## loop-heartbeat -/

private def heartbeatUsage : String :=
  "usage: atlas-builder util loop-heartbeat <path> --project <title> --cycle <n> --max-cycles <n> --run-id <id> --session-mode {fresh|continue} --loop-pid <pid> --loop-started-at-epoch <epoch>"

/-- §19.1-8 の heartbeat の確定引数(全欄必須)。 -/
...
/-- atlas-builder-loop.sh の SESSION_MODE の閉集合(§19.1)。 -/
...
/-- heartbeat payload の純粋構築(スキーマ v1、§19.1-8 のキー順)。epoch 併記は
...
private def runLoopHeartbeat (path : String) (flags : List String) : IO UInt32 := do
  let usageErr : IO UInt32 := do
    IO.eprintln heartbeatUsage
    pure 2
  ...
  try
    ...
    IO.eprintln s!"atlas-builder: loop-heartbeat: cannot write {path}: {e}"
    pure 2
...
def run : List String → IO UInt32
  ...
  | ["bound-settings", project, control] =>
    runBoundSettings project control
  | "loop-heartbeat" :: path :: flags => runLoopHeartbeat path flags
  | ["loop-heartbeat"] => do
    IO.eprintln heartbeatUsage
    pure 2

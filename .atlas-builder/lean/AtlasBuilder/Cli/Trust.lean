...
/-!
`AtlasBuilder.Cli.Trust` — `atlas-builder trust status|ensure [--quiet] <paths...>` の
...
namespace AtlasBuilder.Cli.Trust

open AtlasBuilder

/-- usage(`prog` はマルチコールバイナリ名 — live `atlas-builder` / レシピ
`asl-loop` の両方から共有される)。 -/
private def usage (prog : String) : String :=
  s!"usage: {prog} trust \{status|ensure} [--quiet] <paths...>"
...
def run (args : List String) (prog : String := "atlas-builder") : IO UInt32 := do

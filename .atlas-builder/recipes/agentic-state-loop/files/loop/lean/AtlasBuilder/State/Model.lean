...
/-!
`AtlasBuilder.State.Model` — state 判定核の入力型と寛容デコードプリミティブ(純粋)。
...
(§3.3-2 注入原則)。診断文言は state.py と同一プレフィクスに揃える —
括弧内の例外詳細のみ Python 例外文字列と一致しない(§5.3 の
「人間向け文言のみの差」として許容し、消費側はプレフィクスと reasons
だけを読む)。
-/

namespace AtlasBuilder.State.Model
...
再帰的に(`Text.maxAssetDepth` = 3 階層まで)観測し、パス昇順・dot 始まり
...
/-- state.py `PLACEHOLDER_MARKER` / `FILL_MARKER`(§13.1-8)。 -/
def placeholderMarker : String := "ATLAS-BUILDER-TEMPLATE-PLACEHOLDER"

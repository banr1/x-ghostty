/-!
`AtlasBuilder.Core.Path` — POSIX パス文字列の純粋正規化(Python `posixpath` 互換)。
...
namespace AtlasBuilder.Core.Path
...
引くのに対し常に無展開で返す(意図差: Atlas Builder の trust 候補は絶対パスのみ
なので実行上到達しない)。 -/
...
#guard relpath "/ws/.atlas-builder" "/ws/my-project" == "../my-project"

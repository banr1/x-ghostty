...
/-!
`AtlasBuilder.Core.Trust` — Claude Code workspace trust 判定の純粋核
(`claude_trust.py` 対応)。

`~/.claude.json` の読み書きそのものは `AtlasBuilder.Cli.Trust`(IO シェル)が行い、
ここは「payload とパス列から判定・更新後 payload を計算する」純粋関数のみを
置く(LEAN_MIGRATION_PLAN.md §3.3-2 の注入原則)。
-/

namespace AtlasBuilder.Core.Trust

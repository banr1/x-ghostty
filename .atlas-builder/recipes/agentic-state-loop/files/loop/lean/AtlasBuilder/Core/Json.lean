...
/-!
`AtlasBuilder.Core.Json` — 挿入順序を保持する純粋 JSON コア。

Lean 標準の `Lean.Data.Json` はオブジェクトをキー整列の RBNode で持ち挿入順序を
失うため(LEAN_MIGRATION_PLAN.md §4.4)、Atlas Builder は入出力とも本モジュールの
...
互換を保証しない。出力に float が現れた時点で裁定キューへ(§4.4)。
-/

namespace AtlasBuilder.Core.Json

open AtlasBuilder.Core.Prim (hexDigit digitsToNat padLeftZeros)
...
コンパクト直列化(progress signature の安定バイト列が使う)。 -/

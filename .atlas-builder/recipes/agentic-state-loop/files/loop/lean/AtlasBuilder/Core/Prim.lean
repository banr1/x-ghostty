/-!
`AtlasBuilder.Core.Prim` — モジュール横断で共有する言語プリミティブ級の小関数。

Python 原本の写しではなく Lean 側の実装都合で生じる文字・数値プリミティブの
単一定義点。従来は各モジュールが同一定義を private に重複させていた
(`dropLit?` は 7 箇所)。ここに置くのはドメイン知識を含まない自明な
全域関数に限り、各モジュールは選択 open(`open AtlasBuilder.Core.Prim (...)`)で
借用する。
-/

namespace AtlasBuilder.Core.Prim

/-!
`AtlasBuilder.Core.ShellLex` — POSIX shlex 等価の字句解析器(LEAN_MIGRATION_PLAN.md §4.2)。
...
namespace AtlasBuilder.Core.ShellLex
...
/-! ## 全域性の観測的帰結(証明層 `proofs/` の土台 — §4.6 B-T2)

`go` は `List Char` の構造的再帰による全域関数であり(`partial` なし —
純度ゲート §3.3-1 が機械検査)、停止性は定義の型検査そのものが保証する。
...
end AtlasBuilder.Core.ShellLex

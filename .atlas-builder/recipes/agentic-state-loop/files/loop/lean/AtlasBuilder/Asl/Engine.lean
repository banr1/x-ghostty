...
/-!
`AtlasBuilder.Asl.Engine` — agentic-state-loop レシピの schema 駆動 canonical
...
ロック・時刻は `AtlasBuilder.Cli.Asl`(IO シェル)が注入する。
...
namespace AtlasBuilder.Asl.Engine

open AtlasBuilder.Core
open AtlasBuilder.State.Model (RawFile pyInt?)
open AtlasBuilder.Asl.Check

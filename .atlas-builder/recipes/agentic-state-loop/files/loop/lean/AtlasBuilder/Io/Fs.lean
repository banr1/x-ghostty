...
/-!
`AtlasBuilder.Io.Fs` — ファイル読み取りの薄い IO シェル(信頼境界)。

「読めない」を `none` へ写像するのは best-effort(fail-open)な読み取り専用。
gate 判定に使う canonical state の読み取り失敗を停止側へ倒す fail-closed 系
(I-021)は、対応フェーズで別の明示的なコンストラクタとして導入する(§3.4)。
-/

namespace AtlasBuilder.Io.Fs
...
def readJson? (path : System.FilePath) : IO (Option AtlasBuilder.Core.Json.Value) := do
  pure ((← readFile? path).bind fun s => (AtlasBuilder.Core.Json.parse s).toOption)

/-- `IO.FS.realPath` の失敗(不存在・権限等)を `none` へ(§4.3)。
...
def writeJsonPretty (path : System.FilePath) (payload : AtlasBuilder.Core.Json.Value) :
    IO Unit := do
...
/-- Python `Path.resolve()`(strict=False)対応(§4.3)。CPython 3.12
...
`resolveNonStrict`(§4.3 の既知差はそちらの頭注のとおり許容領域)。guard の
呼び出し側は `none` を「どのルールにもマッチしない」に写す(§3.4 —
...
def appendJsonl (path : System.FilePath) (record : AtlasBuilder.Core.Json.Value) :
...
def sha256OrAbsent (path : System.FilePath) : IO (Except String String) := do
  ...
  if !isFile then
    return .ok "ABSENT"
  try
    pure (.ok (AtlasBuilder.Core.Sha256.hexDigest (← IO.FS.readBinFile path)))
  catch e =>
    pure (.error (toString e))
...
def sha256File (path : System.FilePath) : IO String := do
  pure (AtlasBuilder.Core.Sha256.hexDigest (← IO.FS.readBinFile path))
...
`Cli/Recipe`(`atlas-builder recipe hash`)と `atlas-builder state validate` の
...
def recipeSourceHash (recipeDir : System.FilePath) : IO String := do
  ...
  for rel in sorted do
    ...
    lines := lines.push s!"{rel}\t{AtlasBuilder.Core.Sha256.hexDigest contents}"
  pure (AtlasBuilder.Core.Sha256.hexDigest
    (String.toUTF8 (String.intercalate "\n" lines.toList)))

end AtlasBuilder.Io.Fs

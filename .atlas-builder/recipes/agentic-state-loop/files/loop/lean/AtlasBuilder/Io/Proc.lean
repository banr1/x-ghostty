...
/-!
`AtlasBuilder.Io.Proc` — 外部プロセス観測の薄い IO シェル。
...
namespace AtlasBuilder.Io.Proc
...
def parentPid? (pid : Int) : IO (Option Int) := do
  ...
  | some out => pure (AtlasBuilder.State.Model.pyIntOfString? out.stdout)
  | none => pure none

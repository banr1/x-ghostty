...
/-!
`AtlasBuilder.Cli.AslUtil` — レシピ scripts のシェル内インライン Python の置換先
(`asl-loop util ...`)。live 側 `atlas-builder util` 群(R-7)と同じ収容方針の
...
namespace AtlasBuilder.Cli.AslUtil

open AtlasBuilder
open AtlasBuilder.Core.Json (Value)

private def usage : String :=
  "usage: asl-loop util {stop-message|max-session-cycles|protected-match <rel>} [--loop-root <path>]"

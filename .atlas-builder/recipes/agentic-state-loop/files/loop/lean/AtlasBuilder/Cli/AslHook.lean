...
/-!
`AtlasBuilder.Cli.AslHook` — レシピ agentic-state-loop の vendored Python hooks 3 本
(`pre_tool_guard.py` / `session_start_guard.py` / `post_tool_audit.py`)の
IO シェル(`asl-loop hook pre-tool|post-tool|session-start`)。

§3.3-3 の 3 手だけ(読む → 純粋核 `AtlasBuilder.Asl.Hooks` を呼ぶ → 出力する)で、
...
namespace AtlasBuilder.Cli.AslHook
...
private def usage : String :=
  "usage: asl-loop hook <pre-tool|post-tool|session-start> [--loop-root <path>]"
...
def run (args : List String) : IO UInt32 := do
  match args with
  | event :: rest =>
    if event != "pre-tool" && event != "post-tool" && event != "session-start" then do
      IO.eprintln usage
      return 2

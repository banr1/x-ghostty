#!/usr/bin/env bash
# atlas-builder-triage.sh — human-only interactive triage of the current stop gates
# (META.md §13.6).
#
...
# the exact invocation and runs atlas-builder-resume.sh ONLY after an explicit y on
...
#
# Usage: cd ./.atlas-builder && bash scripts/atlas-builder-triage.sh --project ../PROJECT_TITLE
...
CLAUDE_RC=0
"${SANITIZED_CLAUDE_ENV[@]}" CLAUDE_CODE_SKIP_PROMPT_HISTORY=1 ATLAS_BUILDER_SESSION_MODE=triage ATLAS_BUILDER_SESSION_PROJECT_ROOT="${PROJECT_ROOT}" claude "/triage --project ${PROJECT_ARG}" \
...
ANSWER=""
read -e -r -p "[atlas-builder] Run \`just resume\` with this note now? [y/N] " ANSWER || ANSWER=""
case "${ANSWER}" in
  y | Y | yes | YES)
    ...
    if bash "${CONTROL_ROOT}/scripts/atlas-builder-resume.sh" --project "${PROJECT_ARG}" \

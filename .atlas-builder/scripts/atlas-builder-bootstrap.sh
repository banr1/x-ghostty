#!/usr/bin/env bash
# atlas-builder-bootstrap.sh — initialize a project for Atlas Builder (META.md §18.1, §26).
#
...
# the canonical state via atlas-builder state ensure, then validates.
# The target's CLAUDE.md / .claude/** are the project's own content and are
# never seeded by Atlas Builder (META.md §5.0, §17).
#
# Usage: cd ./.atlas-builder && bash scripts/atlas-builder-bootstrap.sh --project ../PROJECT_TITLE
...
assert_control_root
resolve_project "$@"
# Seeding runs through `atlas-builder util seed`; fail closed before touching
# anything (§5.4).
...
# foreground child (seed, atlas-builder state) still runs; trapping defers the signal
...
seed() {
  ...
  if [[ "${mode}" == "title" ]]; then
    "${ATLAS_BUILDER_BIN}" util seed "${src}" "${dst}" --replace "PROJECT_TITLE=${PROJECT_TITLE}"
  else
    "${ATLAS_BUILDER_BIN}" util seed "${src}" "${dst}"
  fi
  log "seeded: ${rel}"
}
...
log "Next: ensure ${PROJECT_ROOT}/ESSENCE.md is the real human-written Essence, review \`just status\`, commit the initial Atlas Builder state from the workspace root, then run: bash scripts/atlas-builder-loop.sh --project ${PROJECT_ARG}"

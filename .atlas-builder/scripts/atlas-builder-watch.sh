#!/usr/bin/env bash
# atlas-builder-watch.sh — live read-only TUI over the running loop (META.md §15.3).
#
...
# predicates read — I-018 lock liveness, the §19.1-7 drain flag, the §19.1-8
...
#
# The frame is computed by `atlas-builder state watch-render` (status-shaped: the
...
#
# Usage: cd ./.atlas-builder && bash scripts/atlas-builder-watch.sh --project ../PROJECT_TITLE
#
...
# atlas-builder-stop.sh): a silently dropped flag would watch something other than
...
while ((i < ${#args[@]})); do
  case "${args[$i]}" in
    ...
    *)
      ...
      err "usage: bash scripts/atlas-builder-watch.sh --project ../PROJECT_TITLE"
      exit 2
      ;;
  esac
done
...
while :; do
  ...
  if [[ -f "${AUDIT_LOG}" ]]; then
    ...
    SID="$(tail -n 1 "${AUDIT_LOG}" 2>/dev/null |
      "${ATLAS_BUILDER_BIN}" util json-get session_id --lenient)"
  ...
  # not run.
  FRAME="$("${ATLAS_BUILDER_BIN}" state watch-render --project "${PROJECT_ARG}" \

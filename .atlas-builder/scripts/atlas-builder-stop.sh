#!/usr/bin/env bash
# atlas-builder-stop.sh — ask the RUNNING loop to stop cleanly at the next cycle
...
#
# Usage: cd ./.atlas-builder && bash scripts/atlas-builder-stop.sh --project ../PROJECT_TITLE [--cancel]
#
...
# action than the human asked for (same rule as atlas-builder-resume.sh).
CANCEL=0
args=("$@")
i=0
while ((i < ${#args[@]})); do
  case "${args[$i]}" in
    ...
    *)
      ...
      err "usage: bash scripts/atlas-builder-stop.sh --project ../PROJECT_TITLE [--cancel]"
      exit 2
      ;;
  esac
done
...
case "${OWNER_CMD}" in
  *atlas-builder-loop.sh*) ;; # the holder is the loop — register below
  "")

#!/usr/bin/env bash
# atlas-builder-loop.sh — the continuous autonomous Atlas Builder loop (META.md §19.1).
#
...
# via `atlas-builder state should-stop` / `should-complete`, not via this exit code.
#
# Usage: cd ./.atlas-builder && bash scripts/atlas-builder-loop.sh --project ../PROJECT_TITLE \
...
PROMPT_TEMPLATE="${CONTROL_ROOT}/.agent/prompts/atlas-builder-cycle.md"
[[ -f "${PROMPT_TEMPLATE}" ]] || {
...
# §19.1-8: this invocation's own start time, stamped into every cycle's
...
report_complete_and_exit() {
  ...
  printf '%s\n' "${complete_json}"
  completion_scope="$("${ATLAS_BUILDER_BIN}" util json-get completion_scope <<<"${complete_json}")"
...
while ((CYCLE < MAX_CYCLES)); do
  CYCLE=$((CYCLE + 1))
  log "=== Atlas Builder cycle ${CYCLE}/${MAX_CYCLES} (${PROJECT_TITLE}) ==="
  ...
  # §19.1-8: display-only heartbeat for `just watch`, written once SESSION_MODE
  ...
  # declarative baseline for non-scrubbed launches). atlas-builder-doctor.sh
  # asserts both headless forms (§28.5).
  CLAUDE_CMD=("${SANITIZED_CLAUDE_ENV[@]}" "ATLAS_BUILDER_LOCK_TOKEN=${ATLAS_BUILDER_LOCK_TOKEN}" claude -p "${PROMPT}" --setting-sources project --strict-mcp-config)
  if [[ "${SESSION_MODE}" == "fresh" ]]; then
    CYCLES_SINCE_FRESH=1
  else
    CLAUDE_CMD=("${SANITIZED_CLAUDE_ENV[@]}" "ATLAS_BUILDER_LOCK_TOKEN=${ATLAS_BUILDER_LOCK_TOKEN}" claude -c -p "${PROMPT}" --setting-sources project --strict-mcp-config)
    CYCLES_SINCE_FRESH=$((CYCLES_SINCE_FRESH + 1))
  fi
  # §16.1: the per-invocation model override applies to every cycle of this
  # loop run; without ATLAS_BUILDER_MODEL the array is empty and settings.json decides.
  ...
  if [[ "${RUN_STATUS}" != "ok" ]]; then
    ...
    if [[ "${RUN_CLASS}" == "infra" ]]; then
      ...
      log "If it is a plan/model usage limit, wait for the reset or re-run with ATLAS_BUILDER_MODEL=<tier> to continue on another model (§16.1)."
    fi

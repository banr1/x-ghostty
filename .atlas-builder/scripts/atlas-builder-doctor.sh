#!/usr/bin/env bash
# atlas-builder-doctor.sh — inspect CWD, layout, Claude Code availability, settings,
# hooks, and canonical state health (META.md §18.1, §28.5).
#
# Usage: cd ./.atlas-builder && bash scripts/atlas-builder-doctor.sh --project ../PROJECT_TITLE
...
if command -v claude >/dev/null 2>&1; then
  ...
  if grep -q "dontAsk" <<<"${CLAUDE_PERMISSION_BLOCK}"; then
    ...
    fail "claude CLI does not advertise required permission mode dontAsk; update Atlas Builder/Claude before launching sessions (§28.5)"
  fi
  if grep -q "manual" <<<"${CLAUDE_PERMISSION_BLOCK}"; then
    ...
    fail "claude CLI advertises neither manual nor default ask-before-edits mode; update Atlas Builder/Claude before launching sessions (§28.5)"
  fi
  # §16.1: the model is deployment policy, not a safety boundary, so a CLI
  # without --model only costs the ATLAS_BUILDER_MODEL escape hatch (settings.json
  # still decides) — informational, never a failure.
  if grep -q -- "--model" <<<"${CLAUDE_HELP}"; then
    ok "claude CLI supports --model; ATLAS_BUILDER_MODEL=<alias|model-id> can override the settings.json model per run (§16.1)"
  else
    note "claude CLI does not advertise --model; ATLAS_BUILDER_MODEL overrides are unavailable and settings.json decides the model (§16.1)"
  fi
  ...
  LOOP_SRC="$(cat scripts/atlas-builder-loop.sh 2>/dev/null || true)"
  if grep -q -- 'claude -p ' <<<"${LOOP_SRC}" && grep -q -- 'claude -c -p ' <<<"${LOOP_SRC}"; then
    ...
    fail "atlas-builder-loop.sh must launch both fresh and continued cycles headlessly ('claude -p' / 'claude -c -p'): without it, scrubbed default mode could prompt instead of denying (§19.1-5, §28.5)"
  fi
else
...
# binary at CONTROL_ROOT/bin/atlas-builder. The binary is the control plane's only
...
LEAN_BIN="${CONTROL_ROOT}/bin/atlas-builder"
if [[ -x "${LEAN_BIN}" ]]; then
  ok "CONTROL_ROOT/bin/atlas-builder present and executable"
  LEAN_BIN_VERSION_RC=0
  LEAN_BIN_VERSION_OUTPUT="$("${LEAN_BIN}" version 2>&1)" || LEAN_BIN_VERSION_RC=$?
  if ((LEAN_BIN_VERSION_RC == 0)) && [[ "${LEAN_BIN_VERSION_OUTPUT}" == "atlas-builder "* ]]; then
    ok "bin/atlas-builder version: ${LEAN_BIN_VERSION_OUTPUT}"
  else
    fail "bin/atlas-builder version failed (exit ${LEAN_BIN_VERSION_RC}): ${LEAN_BIN_VERSION_OUTPUT}"
  fi
else
  fail "CONTROL_ROOT/bin/atlas-builder not found; nothing can enforce or transition state without it — run: just build"
fi
  ...
  if [[ -n "${CONTROL_GIT_ROOT}" && -n "${PROJECT_GIT_ROOT}" ]]; then
    if [[ "${CONTROL_GIT_ROOT}" == "${PROJECT_GIT_ROOT}" ]]; then
      ...
      if [[ -z "$(git -C "${CONTROL_GIT_ROOT}" status --porcelain --untracked-files=all)" ]]; then
        ...
        note "git worktree is dirty; atlas-builder-loop.sh will refuse to start a cycle until it is clean"
      fi
    else
...
# launch workspace is trusted. Atlas Builder launches only the Over-Project Agent;
...
if [[ "${CLAUDE_AVAILABLE}" -eq 1 ]]; then
  check_claude_trust() {
    ...
    local output
    if [[ ! -x "${LEAN_BIN}" ]]; then
      fail "Claude Code trust check for ${label} requires bin/atlas-builder; run: just build"
      return 0
    fi
...
# --- CONTROL_ROOT layout ---------------------------------------------------
for f in CLAUDE.md .claude/settings.json scripts/atlas-builder-loop.sh scripts/atlas-builder-supervise.sh; do
...
# All five hook events run on `bin/atlas-builder hook <event>` (guard cutover,
...
# The structural checks live in the pure Lean core (AtlasBuilder/Core/SettingsDoctor);
# the launcher source-fragment checks below are plain grep and stay here.
if [[ ! -x "${LEAN_BIN}" ]]; then
  fail ".claude/settings.json safety-wiring check requires bin/atlas-builder; run: just build"
else
  ...
  WIRING_ISSUES="$(
    ...
    emit_missing_fragments scripts/atlas-builder-loop.sh \
      'claude -p "${PROMPT}"' 'claude -c -p "${PROMPT}"' \
      build_sanitized_claude_env ATLAS_BUILDER_LOCK_TOKEN \
      build_bound_session_settings '"${SESSION_SETTINGS_ARGS[@]+' \
      '--setting-sources project' --strict-mcp-config
    # shellcheck disable=SC2016  # the fragments are literal source text, not expansions
    emit_missing_fragments scripts/atlas-builder-triage.sh \
      ATLAS_BUILDER_SESSION_MODE=triage ATLAS_BUILDER_SESSION_PROJECT_ROOT \
    ...
    # shellcheck disable=SC2016  # the fragments are literal source text, not expansions
    emit_missing_fragments scripts/atlas-builder-essence.sh \
      ATLAS_BUILDER_SESSION_MODE=essence ATLAS_BUILDER_SESSION_PROJECT_ROOT \
      CLAUDE_CODE_SKIP_PROMPT_HISTORY=1 resolve_interactive_permission_mode \
      build_sanitized_claude_env build_read_only_session_settings \
      '--add-dir "${PROJECT_ROOT}"' \
      --settings '--setting-sources project' --strict-mcp-config
    # shellcheck disable=SC2016  # the fragments are literal source text, not expansions
    emit_missing_fragments scripts/atlas-builder-supervise.sh \
      CLAUDE_CODE_SKIP_PROMPT_HISTORY=1 ATLAS_BUILDER_LOCK_TOKEN \
    ...
    if [[ ! -f scripts/_lib.sh ]]; then
      ...
      # (AtlasBuilder/Core/Settings.lean); what _lib.sh must still prove here is
      # the wiring: sanitized env -i launches, advisory sessions fed from
      # atlas-builder util read-only-settings, and no CLAUDE_CONFIG_DIR forwarded
      # into Agent sessions.
      ...
      if ! grep -qF -- 'util read-only-settings' scripts/_lib.sh; then
        ...
        printf 'scripts/_lib.sh must build advisory-session settings via %s (read-only target roots)\n' \
          '`atlas-builder util read-only-settings`'
      fi
      if ! grep -qF -- 'util bound-settings' scripts/_lib.sh; then
        ...
        printf 'scripts/_lib.sh must build the bound session overlay via %s (per-launch path rules and sandbox)\n' \
          '`atlas-builder util bound-settings`'
      fi
...
if [[ ! -x "${LEAN_BIN}" ]]; then
  fail "hook smoke tests require bin/atlas-builder; run: just build"
else
  HOOK_ISSUES="$(
    ...
    # §31.2-42). Drop ATLAS_BUILDER_GUARD_SHADOW from the environment: an inherited
    ...
    if pre_out="$(printf '{"cwd":"%s","session_id":"doctor","tool_name":"Edit","tool_input":{"file_path":"%s/ESSENCE.md"}}' \
      ...
      env -u ATLAS_BUILDER_GUARD_SHADOW "${LEAN_BIN}" hook pre-tool 2>/dev/null)"; then
...
if [[ -f "${PROJECT_ROOT}/ESSENCE.md" ]]; then
  ...
  if grep -q "ATLAS-BUILDER-TEMPLATE-PLACEHOLDER" "${PROJECT_ROOT}/ESSENCE.md" 2>/dev/null; then
...
# content. Atlas Builder neither seeds nor trusts them, and never starts them in the
...
if [[ -f "${PROJECT_ROOT}/.claude/settings.json" ]]; then
  if [[ ! -x "${LEAN_BIN}" ]]; then
    fail "PROJECT_ROOT/.claude/settings.json JSON check requires bin/atlas-builder; run: just build"
...
if [[ -d "${PROJECT_STATE_ROOT}/state" ]]; then
  ...
  note "missing ${PROJECT_STATE_ROOT}/state (run atlas-builder-bootstrap.sh)"
fi
...
if [[ ! -f "${INDEX_PATH}" ]]; then
  ...
  fail "project_index binding check requires bin/atlas-builder; run: just build"
...
# (AtlasBuilder/Core/StaticConfig); the runtime never mutates it. A deployed plane
...
if [[ ! -f "${WORKSPACE_JSON}" ]]; then
  ...
  fail "workspace.json drift check requires bin/atlas-builder; run: just build"
  ...
  note "workspace.json drifted from its Lean definition (AtlasBuilder/Core/StaticConfig); review the local edit, then restore it (git restore .agent/state/workspace.json) — the framework maintainer regenerates it with 'just render-static' (§8.1)"
fi
...
if [[ -d "${PROJECT_STATE_ROOT}/state" ]]; then
  ...
  if ((VALIDATE_RC == 0)); then
    ok "atlas-builder state validate passed"
  elif ((VALIDATE_RC == 1)); then
    fail "atlas-builder state validate reported errors (see validation.json)"
  else
    fail "atlas-builder state validate failed with exit ${VALIDATE_RC}; canonical state or the state engine itself is broken (I-021)"
  fi
fi
...
if [[ -d "${LOOP_LOCK_PATH}" ]]; then
  ...
  if [[ -n "${LOCK_PID}" ]] && lock_pid_is_alive "${LOCK_PID}"; then
    note "an Atlas Builder mutating transition appears to be running right now (pid ${LOCK_PID}, I-018)"
  else
...
if [[ -f "${RUNS_JSONL}" ]] && [[ ! -x "${LEAN_BIN}" ]]; then
  fail "runs.jsonl in-flight check requires bin/atlas-builder; run: just build"
  ...
  # itself lives in the pure Lean core (AtlasBuilder/Core/Runs.lean).
...
# §12.2 / §20.3-3: workspace-root sessions load no Atlas Builder settings, so no
...
log "Atlas Builder also never starts an embedded target agent directly in the live PROJECT_ROOT; use a project-defined isolated runner (§10)."

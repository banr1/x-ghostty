...
— G-T2), so atlas-builder-essence.sh pre-dumps every Excel asset under
...
Usage: atlas-builder-xlsx-dump.py <workbook.xlsx> <out.txt> [<display-label>]
...
def main(argv):
    if len(argv) < 3:
        print(
            "usage: atlas-builder-xlsx-dump.py <workbook.xlsx> <out.txt> [<label>]",
            file=sys.stderr,
        )
        return 2

...
/-!
`Mercator.Core.Json` — 挿入順序を保持する純粋 JSON コア。

Lean 標準の `Lean.Data.Json` はオブジェクトをキー整列の RBNode で持ち挿入順序を
失うため(LEAN_MIGRATION_PLAN.md §4.4)、Mercator は入出力とも本モジュールの
...
namespace Mercator.Core.Json

open Mercator.Core.Prim (hexDigit digitsToNat padLeftZeros)

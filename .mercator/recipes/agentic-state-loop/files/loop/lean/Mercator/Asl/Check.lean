...
/-!
`Mercator.Asl.Check` — agentic-state-loop レシピの schema 駆動ミニバリデータ
...
- `pattern` キーワードは Python regex(`re.search`)ではなく §29.6 の制限
  グロブ方言(`Mercator.Core.Glob`、search 意味論)で判定する。
...
namespace Mercator.Asl.Check

open Mercator.Core
open Mercator.Core.Prim (hex2)
open Mercator.State.Model (pyGE? numEq)

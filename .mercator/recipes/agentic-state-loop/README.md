...
Mercator が自分自身の運用に使っている状態管理法の汎用コアを、対象プロダクトが内包できる形に
...
instance は自己完結である: 対象リポジトリ単体を clone すれば動き、Mercator を知らないし
参照もしない。唯一の例外は `<loop_dir>/lean/` の同梱エンジンソース — live runtime と
共通判定コアを共有する忠実複製(バイト同一)であり、モジュール名前空間 `Mercator.*` を
保持する(META.md §29.3 の規範整理)。参照はコード内の名前だけで、CONTROL_ROOT への
ビルド・実行時参照は無い。運用面(scripts / prompts / schema / settings)に Mercator の
語彙は現れない。
...
5. `recipe.lock.json` を `<loop_dir>/` 直下に置く(`recipe.json` の `lock_format`。
   `source_sha256` は `bin/mercator recipe hash agentic-state-loop` の出力)。
...
- **cycle commit は対象が属する git リポジトリに積まれる。** Mercator ワークスペース内で
  開発中の対象で本ループを回すと、その commit はワークスペースのリポジトリ履歴に入る。
  Mercator の cycle 内から挙動確認するときは通常 `once.sh` を使い、Mercator の single-flight

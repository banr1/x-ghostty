...
Mercator の概念で、META.md §29 が正本である。)
...
`mercator state validate` はポインタと instance(`recipe.lock.json`)の突き合わせを warning で可視化する
...
`bin/mercator recipe hash <name>` が計算する。
...
3. Todo 実行で `files/` を対象へ写し、置換トークンを埋め、authored パラメータを Essence から
   導出して書き、`recipe.lock.json` を置く(`source_sha256` は `bin/mercator recipe hash <name>`)。

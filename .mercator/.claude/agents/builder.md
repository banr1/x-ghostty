...
2. Stay inside the files the task names or clearly implies, under the bound
   project. Never touch: `ESSENCE.md`, `.mercator/state/**`, dependency
...
3. Never run `bin/mercator state`, `git add`, `git commit`, or any
   `mercator-*.sh` script. Canonical state and checkpoints belong to the

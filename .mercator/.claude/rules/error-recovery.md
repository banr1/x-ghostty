...
The loop's `mercator state should-stop` predicate will stop until the human resolves it.
...
- If you cannot select any runnable Todo (`pending`/`in_progress` with
  executor `mercator`) while must-priority Todos remain, record a
...
- The loop itself materializes `no_runnable_todos` / `idle_cycles` /
  ...
  Recommendations via `mercator state raise-loop-gates`
  when you fail to; treat a
  loop-raised gate (`raised_by: "mercator-loop"`) in the state as a signal
...
`mercator state resume` / `mercator-resume.sh` yourself, and

...
(`mercator state validate` warns), project it as a **High-Risk Todo**
...
3. Write `recipe.lock.json` per `recipe.json`'s `lock_format`;
   `source_sha256` comes from `bin/mercator recipe hash <name>`.
...
- The instance must stay self-contained and Mercator-free: never write
  Mercator's vocabulary into it, never make it reference CONTROL_ROOT (§6.2,

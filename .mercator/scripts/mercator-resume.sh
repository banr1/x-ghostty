#!/usr/bin/env bash
# mercator-resume.sh — the human-only intervention transition (META.md §13.3).
#
...
#
# Human-only: the pre-tool guard hook denies this script and `mercator state resume` to
# agents so the loop can never release its own stop conditions (I-011).
#
# Usage: cd ./.mercator && bash scripts/mercator-resume.sh --project ../PROJECT_TITLE \
...
while ((i < ${#args[@]})); do
  case "${args[$i]}" in
    ...
    *)
      ...
      err "Usage: bash scripts/mercator-resume.sh --project ../PROJECT_TITLE [--note \"...\"] [--resolve ID]... [--retract-approval ID]... [--approve-should|--close-at-must] [--steer-only] [--force]"
      exit 2
      ;;
  esac
done
...
# child (mercator state resume, git) is still mutating state. Trapping defers the
...
if ((RESUME_RC == 1)); then
  ...
  if ((GUIDANCE_RC == 0)); then
    ...
    warn "mercator state should-stop failed; run \`just status\` once the state is readable (I-021)."
  fi
  exit 3
elif ((RESUME_RC != 0)); then
  [[ -n "${RESUME_JSON}" ]] && printf '%s\n' "${RESUME_JSON}"
  err "mercator state resume failed unexpectedly (exit ${RESUME_RC}); the transition may be half-applied."
...
MODE="$("${MERCATOR_BIN}" util json-get mode <<<"${RESUME_JSON}")"
CLOSED_RUNS="$("${MERCATOR_BIN}" util json-get closed_runs --join , <<<"${RESUME_JSON}")"
REMAINING_SHOULD_STOP="$("${MERCATOR_BIN}" util json-get \
  should_stop_after_resume.should_stop <<<"${RESUME_JSON}")"
REMAINING_STOP=""
if [[ "${REMAINING_SHOULD_STOP}" == "true" ]]; then
  REMAINING_STOP="$("${MERCATOR_BIN}" util json-get \

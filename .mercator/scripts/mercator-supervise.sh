#!/usr/bin/env bash
# mercator-supervise.sh — human-supervised session for an approved High-Risk
...
# discipline mercator-resume.sh / mercator-init.sh / mercator-bootstrap.sh use.
...
APPROVED_TARGET="$("${MERCATOR_BIN}" util json-get target <<<"${PREFLIGHT_JSON}")"
...
CLAUDE_RC=0
"${SANITIZED_CLAUDE_ENV[@]}" CLAUDE_CODE_SKIP_PROMPT_HISTORY=1 MERCATOR_LOCK_TOKEN="${MERCATOR_LOCK_TOKEN}" claude \
  "${SUPERVISE_PROMPT}" \
  "${SESSION_MODEL_ARGS[@]+"${SESSION_MODEL_ARGS[@]}"}" \
  --permission-mode "${INTERACTIVE_PERMISSION_MODE}" \
  --setting-sources project \
  --strict-mcp-config || CLAUDE_RC=$?

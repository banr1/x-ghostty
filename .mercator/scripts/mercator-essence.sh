#!/usr/bin/env bash
# mercator-essence.sh — human-only interactive authoring/updating of
...
# non-contradictory musts, won't fences — §2.1.1..§2.1.3) lives in Mercator.
...
# dir as a text cell listing (xlsx/<relpath>.txt, via mercator-xlsx-dump.py)
...
#
# Usage: cd ./.mercator && bash scripts/mercator-essence.sh --project ../PROJECT_TITLE --mode new|update
...
if [[ -d "${ESSENCES_DIR}" ]]; then
  while IFS= read -r -d '' WORKBOOK; do
    ...
    if python3 "${CONTROL_ROOT}/scripts/mercator-xlsx-dump.py" \
...
CLAUDE_RC=0
"${SANITIZED_CLAUDE_ENV[@]}" CLAUDE_CODE_SKIP_PROMPT_HISTORY=1 MERCATOR_SESSION_MODE=essence MERCATOR_SESSION_ESSENCE_MODE="${MODE}" MERCATOR_SESSION_PROJECT_ROOT="${PROJECT_ROOT}" claude "/essence --project ${PROJECT_ARG} --mode ${MODE}" \
...
ANSWER=""
read -e -r -p "[mercator] Install this draft as ${PROJECT_TITLE}/ESSENCE.md now? [y/N] " ANSWER || ANSWER=""
case "${ANSWER}" in
  y | Y | yes | YES)
    ...
    if [[ -f "${PROJECT_STATE_ROOT}/state/project.json" ]]; then
      ...
      case "${GUIDANCE_RC}" in
        ...
        *) warn "mercator state should-stop failed; run \`just status\` for the exact \`just resume …\` form (I-021)." ;;
      esac
    else

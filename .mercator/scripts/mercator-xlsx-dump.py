...
— G-T2), so mercator-essence.sh pre-dumps every Excel asset under
...
Usage: mercator-xlsx-dump.py <workbook.xlsx> <out.txt> [<display-label>]
...
def main(argv):
    if len(argv) < 3:
        print(
            "usage: mercator-xlsx-dump.py <workbook.xlsx> <out.txt> [<label>]",
            file=sys.stderr,
        )
        return 2

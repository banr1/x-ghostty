...
#
# This intentionally does not match the agent-allowed mercator-*.sh pattern:
...
# Mercator never launches Claude from live PROJECT_ROOT (§10, I-003), so do
...
case "${MODE}" in
  apply)
    log "Ensuring Claude Code trust for Mercator launch roots..."
    "${MERCATOR_BIN}" trust ensure "${CLAUDE_TRUST_CANDIDATES[@]}"
    log "Claude Code trust is ready. Re-run: just doctor"
    ;;
  check)
    "${MERCATOR_BIN}" trust status "${CLAUDE_TRUST_CANDIDATES[@]}"
    ;;
esac

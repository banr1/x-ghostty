...
/-!
`Mercator.Core.Classify.ReadOnly` — read-only セッション(I-022 triage /
...
5. **triage の state 読み取り形状**(`triageStateShape?` /
   ...
   <args...>`(argv 3 個以上、`bin/mercator state` 綴り)
   から binary 生パスと唯一の `--project` 生値を取る。バイナリ実体照合
   (CONTROL_ROOT/bin/mercator への解決一致)と project 解決照合は IO。
...
open Mercator.Core.Text (isPySpace splitLinesBash pyStrip)
...
(CONTROL_ROOT/bin/mercator への解決一致)は IO 側の表引きが行う。 -/
...
-- triageStateShape?
#guard triageStateShape? ["bin/mercator", "state", "--project", "p", "status"]
  == some ("bin/mercator", "p")
#guard triageStateShape? ["bin/mercator", "state", "--project=p", "status"]
  == some ("bin/mercator", "p")
#guard triageStateShape? ["/cr/bin/mercator", "state", "--project", "p", "status"]
  == some ("/cr/bin/mercator", "p")
#guard triageStateShape? ["bin/mercator", "status", "--project", "p"] == none
#guard triageStateShape? ["bin/mercator", "state"] == none
#guard triageStateShape? ["bin/mercator", "state", "status"] == none
...
#guard triageStateSubcommandsOk "bin/mercator state --project p should-stop"
  == true
#guard triageStateSubcommandsOk "bin/mercator state --project p status" == true
#guard triageStateSubcommandsOk "bin/mercator state --project p validate"
  == false
#guard triageStateSubcommandsOk "bin/mercator state --project p" == false
#guard triageStateSubcommandsOk
  "bin/mercator state status; bin/mercator state should-stop" == true

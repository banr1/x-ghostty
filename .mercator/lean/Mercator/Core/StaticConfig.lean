...
/-!
`Mercator.Core.StaticConfig` — 制御プレーン静的設定の SSOT(META.md §8.1)。
...
あり、`mercator util static-config <name>` がその出力面を提供する。乖離は
...
namespace Mercator.Core.StaticConfig

open Mercator.Core.Json (Value)

/-! ## framework の自己記述(META.md §8.1 workspace.json) -/

def frameworkName : String := "Mercator"
def frameworkVersion : String := "2.0.0"
...
def schemaVersion : String := "2.0.0"
...
def overprojectAgentCwd : String := "./.mercator"
...
束縛後の index は `mercator state ensure` / init が変異するランタイム正本で
...
end Mercator.Core.StaticConfig

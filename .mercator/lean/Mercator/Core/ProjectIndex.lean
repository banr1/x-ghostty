...
/-!
`Mercator.Core.ProjectIndex` — §8.1 registry(project_index.json の単一
`project` オブジェクト)の形状判定の純粋核。

置換元は `mercator-init.sh` の 2 つのインライン Python:
...
namespace Mercator.Core.ProjectIndex
...
end Mercator.Core.ProjectIndex

/-!
`Mercator.Core.Path` — POSIX パス文字列の純粋正規化(Python `posixpath` 互換)。
...
引くのに対し常に無展開で返す(意図差: Mercator の trust 候補は絶対パスのみ
なので実行上到達しない)。 -/
...
#guard relpath "/ws/.mercator" "/ws/my-project" == "../my-project"

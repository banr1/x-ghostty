...
/-!
`Mercator.Hook.StopCheck` — Stop hook(`mercator hook stop`)の純粋決定核。

移行元: `.claude/hooks/stop_check.py`(移行完了まで凍結)。Mercator のターン終了時、
ディスク状態がクリーンな停止と矛盾していれば(validation.json がまだ error を報告
していれば)、停止をブロックせずに警告だけ出す。

IO シェル(`Mercator.Cli.Hook`)は project_index.json と validation.json を読んで
...
namespace Mercator.Hook.StopCheck
...
def outputLine (warnings : List String) : Option String :=
  match warnings with
  | [] => none
  | ws =>
    let message := "[Mercator stop_check] " ++ String.intercalate " / " ws
    some (Value.render (.obj [("systemMessage", .str message)]))

/-! ## コンパイル時テスト -/

private def parsed (s : String) : Option Value := (Mercator.Core.Json.parse s).toOption

#guard projectOf none == none
#guard projectOf (parsed "{\"schema_version\": \"2.0.0\", \"project\": null}") == none
...
#guard outputLine [] == none
#guard outputLine ["X: validation.json still reports errors (2)."]
    == some "{\"systemMessage\": \"[Mercator stop_check] X: validation.json still reports errors (2).\"}"

end Mercator.Hook.StopCheck

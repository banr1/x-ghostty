...
/-!
`Mercator.Hook.PreCompact` — PreCompact hook(`mercator hook pre-compact`)の
純粋決定核(META.md §14.1)。
...
namespace Mercator.Hook.PreCompact
...
def noteText : String :=
  "Context compacted. Recover from disk: read project_index.json, then the active project's .mercator/state/context.json and validation.json."

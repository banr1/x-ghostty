...
/-!
`Mercator.Hook.PostTool` — PostToolUse hook(`mercator hook post-tool`)の
...
namespace Mercator.Hook.PostTool

open Mercator.Core.Json (Value)
open Mercator.Core.Text (splitLinesPy)
open Mercator.Core.Classify (isControlPlaneHighRisk isHighRiskPath
...
export Mercator.Core.PathAlg (childPath)
...
def stateRootCandidates (targetStr controlRoot : String) :
  ...
  (selfAndParents targetStr).filterMap fun parent =>
    ...
    else some (parent, childPath parent ".mercator/state")
...
private def pendingBeforeAux (targetStr : String) (sessionId : Value) :
  ...
  | [] => .ok (.str "UNKNOWN")
  | line :: rest =>
    match Mercator.Core.Json.parse line with
...
#guard stateRootCandidates "/ws/proj/hooks/h.py" "/ws/.mercator"
  == [("/ws/proj/hooks/h.py", "/ws/proj/hooks/h.py/.mercator/state"),
      ("/ws/proj/hooks", "/ws/proj/hooks/.mercator/state"),
      ("/ws/proj", "/ws/proj/.mercator/state"),
      ("/ws", "/ws/.mercator/state"),
      ("/", "/.mercator/state")]
#guard stateRootCandidates "/cr/scripts/x" "/cr"
  == [("/cr/scripts/x", "/cr/scripts/x/.mercator/state"),
      ("/cr/scripts", "/cr/scripts/.mercator/state"),
      ("/", "/.mercator/state")]
...
#guard (Mercator.Core.Json.Value.render <$> auditRecord "T"

...
/-!
`Mercator.Io.Lock` — I-018 の単一実行スロット(canonical write window)。
...
- Python は取得時に `MERCATOR_LOCK_TOKEN` を os.environ へ輸出するが、
  Lean 版の ensure / validate は mutating transition を子プロセスとして
  起動しないため輸出しない(継承側 = 環境変数とロックファイルの照合は
  同一挙動)。
-/

namespace Mercator.Io.Lock

open Mercator.State.Model (pyIntOfString?)
...
private def randomHex (n : Nat) : IO String := Mercator.Io.Rand.randomHex n
...
private def heldMessage (owner : Int) : String :=
  s!"another Mercator mutating transition holds the single-flight lock (pid {owner}); state mutation refused."

/-- 取得試行ループ(Python の `for _ in range(3)`)。`.ok true` = 取得
(release 必要)。 -/
private def tryAcquire (lockDir : System.FilePath) :
    ...
    | some e =>
      ...
      | some owner =>
        if ← Proc.pidAlive owner then
          ...
          return .error (heldMessage owner)
        -- 死亡 → stale 再取得へ
      | none =>
        ...
        let age? ← try
            ...
            pure none
        match age? with
        | none => return ← tryAcquire lockDir n
        | some age =>
          if age ≤ 10 then
            return .error ("another Mercator mutating transition is acquiring "
...
def withSlot (controlRoot : System.FilePath) (onErr : String → IO UInt32)
    (act : IO UInt32) : IO UInt32 := do
  match ← acquire controlRoot with
  | .error msg => onErr msg
  | .ok false => act
  | .ok true =>
    try

...
/-!
`Mercator.Io.Fs` — ファイル読み取りの薄い IO シェル(信頼境界)。
...
def readJson? (path : System.FilePath) : IO (Option Mercator.Core.Json.Value) := do
  pure ((← readFile? path).bind fun s => (Mercator.Core.Json.parse s).toOption)
...
def writeJsonPretty (path : System.FilePath) (payload : Mercator.Core.Json.Value) :
    IO Unit := do
...
def appendJsonl (path : System.FilePath) (record : Mercator.Core.Json.Value) :
...
def sha256OrAbsent (path : System.FilePath) : IO (Except String String) := do
  ...
  if !isFile then
    return .ok "ABSENT"
  try
    pure (.ok (Mercator.Core.Sha256.hexDigest (← IO.FS.readBinFile path)))
  catch e =>
    pure (.error (toString e))
...
def sha256File (path : System.FilePath) : IO String := do
  pure (Mercator.Core.Sha256.hexDigest (← IO.FS.readBinFile path))
...
`Cli/Recipe`(`mercator recipe hash`)と `mercator state validate` の
...
def recipeSourceHash (recipeDir : System.FilePath) : IO String := do
  ...
  for rel in sorted do
    ...
    lines := lines.push s!"{rel}\t{Mercator.Core.Sha256.hexDigest contents}"
  pure (Mercator.Core.Sha256.hexDigest

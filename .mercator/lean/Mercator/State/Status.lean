...
/-!
`Mercator.State.Status` — state.py `cmd_status`(L3077–3156)の純粋核
。
...
namespace Mercator.State.Status

open Mercator.Core
open Mercator.Core.Prim (hex2)
...
private def cleanStatus : StatusInputs := {
  ...
  stop := {
    ...
    todo := .text ("{\"items\": [{\"id\": \"T-001\", \"status\": \"pending\", "
      ++ "\"priority\": \"must\", \"executor\": {\"mode\": \"mercator\"}}], "
      ++ "\"active_batch\": []}")
...
private def assetStatus : StatusInputs := cleanWith fun s => { s with
  ...
  project := .text ("{\"essence_sha256\": \""
    ++ Mercator.Core.Sha256.hexDigest (String.toUTF8 assetEssence)
...
private def successStatus : StatusInputs := cleanWith fun s => { s with
  ...
  project := .text ("{\"essence_sha256\": \""
    ++ Mercator.Core.Sha256.hexDigest (String.toUTF8 essenceWithSuccess) ++ "\"}")

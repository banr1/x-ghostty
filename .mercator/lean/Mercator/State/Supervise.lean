...
/-!
`Mercator.State.Supervise` — human-supervised High-Risk session の read-only
preflight。
...
namespace Mercator.State.Supervise
...
def preflight (i : Inputs) : Except String Json.Value := do
  ...
  if (todo.get? "executor").bind (·.get? "mode") != some (.str "mercator") then
    throw s!"Todo {i.todoId} must have executor.mode=mercator"

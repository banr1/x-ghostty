/-!
`Mercator.Guard.Rules` — PreToolUse guard の判定理由文言(META.md §16・§23)。
...
namespace Mercator.Guard.Rules

/-- WRITE_TOOLS の DENY_PATTERNS 一致(I-004・§2.1.5・§13.1-10 アンカー)。 -/
def writeDenyReason (target : String) : String :=
  s!"Mercator invariant: '{target}' is human-only, secret, or attestation-anchoring (ESSENCE.md / essences/ / .env / secrets / essence_attestations.jsonl / .mercator/state/project.json are never directly agent-editable)."
...
def controlPlaneDenyReason (target : String) : String :=
  s!"Mercator control-plane file (META.md §11.3): '{target}'. A target-project Agent session never self-modifies its enforcement plane; framework work belongs to the separate maintainer plane."
...
def commitDenyReason : String :=
  "Cycle commits are owned by mercator-loop.sh; agents may inspect git status/diff but must not stage or commit directly."

/-- Bash: HUMAN_ONLY_BASH / `resume` サブコマンド(I-011、§13.3・§18.2)。 -/
def humanOnlyDenyReason : String :=
  "Human-only Mercator transition (I-011, META.md §13.3, §18.2): resume / loop / once / init / trust / triage / supervise / new-essence / update-essence are operated by the human. Agents never release their own stop conditions, spawn their own loop / triage / essence-interview sessions, re-bind the control plane, grant trust, or author ESSENCE.md."

/-- Bash: LOOP_ONLY_BASH / loop-only サブコマンド(§19.1)。 -/
def loopOnlyDenyReason : String :=
  "Loop-owned Mercator transition (META.md §19.1): start-run / end-run / record-progress / reset-context / raise-loop-gates are invoked by mercator-loop.sh at the cycle boundary. Running them mid-cycle corrupts run records and the idle-progress signal; record your findings in canonical state instead."
...
def bootstrapAskReason : String :=
  "Mercator maintenance transition requires explicit human approval (bootstrap re-seeds project files outside the normal cycle flow)."

/-- Bash: 制御プレーン面のシェル変異(§11.3)。 -/
def controlSurfaceBashDenyReason : String :=
  "Bash command appears to modify a Mercator control-plane file (scripts / bind templates / recipes / prompts / META.md / README.md / justfile); target-project Agent sessions never self-modify the framework (META.md §11.3)."

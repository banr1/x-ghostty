...
/-!
`Mercator.Asl.Hooks` — agentic-state-loop レシピの hooks 3 本
(`asl-loop hook pre-tool|post-tool|session-start` の判定部)の純粋決定核。

判定はすべて本モジュールの純粋関数が行い、外部状態(stdin payload・解決済み
パス・schema.json・現在時刻)は `Mercator.Cli.AslHook`(IO シェル)が注入する
(§3.3-3 の 3 手)。live 版 guard(`Guard.*`)とは別系統 — レシピ hooks は
Mercator 語彙を持たず、`LOOP_DIR` 名を実行時に導出する自己完結面である
(§6.2・§17・§29)。
...
namespace Mercator.Asl.Hooks

open Mercator.Core.Json (Value)
open Mercator.Core.Prim (dropLit?)
open Mercator.Core.Text (isPySpace)
...
def isProtectedWriteTarget (loopDir : String) (command : String) : Bool :=
  Mercator.Core.Text.containsSub command (loopDir ++ "/state/")
    || search
        (lit ".env"
          >=> alts [fun cs => if cs.isEmpty then some [] else none,
                    cls fun c => c == '.' || isPySpace c])
        command
    || Mercator.Core.Text.containsSub command "secrets/"
    || Mercator.Core.Text.containsSub command "config/credentials.json"
...
private def segmentTokens (segment : String) : List String :=
  match Mercator.Core.ShellLex.split segment with
  | .ok toks => toks
  | .error _ => Mercator.Core.Text.pySplitWs segment
...
def pathCandidates (raw : String) (resolved? : Option String) (targetRoot : String) :
  ...
  if raw.isEmpty then []
    ...
    let cand1 := Mercator.Core.PathAlg.candidate raw
    match resolved? with
    | none => [cand1]
    | some r =>
      let cand2 := backToSlash r
      let cand3? :=
        if Mercator.Core.PathAlg.isRelativeToStr r targetRoot then
          Mercator.Core.PathAlg.relativeToStr? r targetRoot
        else none
...
def decideWrite (loopDir target : String) (cands : List String)
  ...
  if cands.any (fun c => matchesBuiltinDeny loopDir c)
      || denyPats.any (fun p => cands.any (Mercator.Core.Glob.accepts p)) then
...
def decideBash (loopDir command : String) (denyBashPats : List String) :
  ...
  if isProtectedWriteTarget loopDir command && hasWriteRedirectOrMutator command then
    ...
    | none =>
      match denyBashPats.find? (fun p => Mercator.Core.Glob.accepts p command) with
...
#guard matchesAsk ".loop" ".loop/lean/Mercator/Asl/Engine.lean" == true

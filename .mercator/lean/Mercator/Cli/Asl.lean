...
/-!
`Mercator.Cli.Asl` — agentic-state-loop レシピの schema 駆動 state エンジンの
...
- `LOOP_TZ` は IANA 名からオフセット方言(`+HH:MM`/`Z`/`UTC`)へ
  (MERCATOR_TZ と同じ裁定の適用)。既定は +09:00 固定。タイムスタンプは
  表示専用で判定に使われない。
...
namespace Mercator.Cli.Asl

open Mercator
open Mercator.Asl
open Mercator.Core.Prim (expandEquals)
open Mercator.State.Model (RawFile)
...
private def lockPid? (lockDir : System.FilePath) : IO (Option Int) := do
  try
    pure (Mercator.State.Model.pyIntOfString? (← IO.FS.readFile (lockDir / "pid")))
  catch _ =>
    pure none

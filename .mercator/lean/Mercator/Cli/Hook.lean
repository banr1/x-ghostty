...
/-!
`Mercator.Cli.Hook` — `mercator hook <event>` の IO シェル。

§3.3-3 の規約どおり「読む → 純粋関数を呼ぶ → 出力する」の 3 手だけで、
分岐ロジックは `Mercator.Hook.*` の純粋核に置く。stdin/stdout/exit code の
契約は Python hooks と同一(§2.1): stdout に JSON を 1 行出すか無出力、
exit は常に 0(fail-open)。
-/

namespace Mercator.Cli.Hook
...
/-- 表示タイムゾーン付きの現在時刻(Python `now_local`。`MERCATOR_TZ` は
§4.4 のとおりオフセット形式、既定 +09:00)。 -/
private def nowLocal : IO String := Io.Clock.nowLocal "MERCATOR_TZ"

/-- `mercator hook stop` — stop_check.py と同一契約。
  ...
  let warnings ← do
    ...
    | some root => do
      ...
      | some (projectRoot, title) => do
        let validation? ←
          Io.Fs.readJson? (root / projectRoot / ".mercator" / "state" / "validation.json")
        pure (Hook.StopCheck.warningOf title validation?).toList
  if let some line := Hook.StopCheck.outputLine warnings then
    IO.println line
  pure 0

/-- `mercator hook session-start` — session_start_guard.py と同一契約
  ...
  | some root => do
    ...
    let env : Hook.SessionStart.Env :=
      { controlRoot := controlRoot.toString
        resolvedCwd?
        claimedCwd
        sessionMode := (← IO.getEnv "MERCATOR_SESSION_MODE").getD "" }
...
/-- `mercator hook pre-compact` — pre_compact_snapshot.py と同一契約
...
def runPreCompact : IO UInt32 := do
  ...
  let mode := (← IO.getEnv "MERCATOR_SESSION_MODE").getD ""
...
/-! ## `mercator hook post-tool`(post_tool_audit.py、META.md §20.4) -/
...
/-- `mercator hook post-tool` — post_tool_audit.py と同一契約(META.md §20.4)。
...
def runPostTool : IO UInt32 := do
  ...
  let mode ← IO.getEnv "MERCATOR_SESSION_MODE"
...
/-! ## `mercator hook pre-tool`(pre_tool_guard.py、META.md §16・§23)
...
**併走モード**: `MERCATOR_GUARD_SHADOW` が非空のとき、
...
/-- `mercator hook pre-tool` — pre_tool_guard.py と同一契約(§2.1)。
...
def runPreTool : IO UInt32 := do
  ...
  | some rootPath => do
    ...
    let sessionMode := Core.Text.pyStrip
      ((← IO.getEnv "MERCATOR_SESSION_MODE").getD "")
    ...
    let expectedProjectRaw := (← IO.getEnv "MERCATOR_SESSION_PROJECT_ROOT").getD ""
    ...
    let essenceMode := Core.Text.pyStrip
      ((← IO.getEnv "MERCATOR_SESSION_ESSENCE_MODE").getD "")
    let stateScript ← if sessionMode == "triage" then do
        pure (← Io.Fs.resolveNonStrict
          ⟨Hook.PostTool.childPath controlRoot "bin/mercator"⟩).toString
      else pure ""

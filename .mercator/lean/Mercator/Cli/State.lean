...
/-!
`mercator state <command> --project <p>` — state エンジンの CLI(IO シェル)。
...
namespace Mercator.Cli.State

open Mercator.Core
open Mercator.Core.Prim (expandEquals)
open Mercator.State
open Mercator.State.Model (RawFile EssenceRead)
...
private def die (msg : String) : IO UInt32 := do
  IO.eprintln s!"[mercator state] ERROR: {msg}"
  pure 2
...
private partial def scanEssencesLevel (dir : System.FilePath) (prefix? : String)
  ...
  | .inl entries =>
    ...
    for name in names do
      ...
      if depth > Mercator.Core.Text.maxAssetDepth then
        tooDeep := tooDeep ++ [rel]
      else
        match t? with
        | some .dir =>
...
private def resolveCtx (projectArg : String) : IO (Except String Ctx) := do
  ...
  if pyParent root != pyParent controlRoot then
    return .error ("PROJECT_ROOT must be a sibling of CONTROL_ROOT "
      ++ "(<workspace>/.mercator and <workspace>/<project>, META.md §5.1).")
  let title := root.fileName.getD ""
  pure (.ok {
    projectRoot := root
    title
    stateRoot := root / ".mercator"
    stateDir := root / ".mercator" / "state"
...
private def stopInputs (ctx : Ctx) : IO Predicates.StopInputs := do
...
private def compactNow : IO String := do
  let offsetMinutes := (((← IO.getEnv "MERCATOR_TZ").getD "")
...
def run (args : List String) : IO UInt32 := do
  ...
  | .error e =>
    die s!"usage: mercator state <command> --project <PROJECT_ROOT> ({e})"
  | .ok parsed =>

...
/-!
`Mercator.Cli.AslHook` — レシピ agentic-state-loop の vendored Python hooks 3 本
(`pre_tool_guard.py` / `session_start_guard.py` / `post_tool_audit.py`)の
IO シェル(`asl-loop hook pre-tool|post-tool|session-start`)。

§3.3-3 の 3 手だけ(読む → 純粋核 `Mercator.Asl.Hooks` を呼ぶ → 出力する)で、
...
namespace Mercator.Cli.AslHook

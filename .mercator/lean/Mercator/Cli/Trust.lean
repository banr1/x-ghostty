...
/-!
`Mercator.Cli.Trust` — `mercator trust status|ensure [--quiet] <paths...>` の
...
namespace Mercator.Cli.Trust

open Mercator

/-- usage(`prog` はマルチコールバイナリ名 — live `mercator` / レシピ
`asl-loop` の両方から共有される)。 -/
...
def run (args : List String) (prog : String := "mercator") : IO UInt32 := do

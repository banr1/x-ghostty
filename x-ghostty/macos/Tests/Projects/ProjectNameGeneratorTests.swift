...
/// Tests for `ProjectNameGenerator` (`SPEC.md` §8, §19.1 "random project name
/// uniqueness"). Randomness is injected via a deterministic generator so the
/// tests are reproducible.
struct ProjectNameGeneratorTests {
    ...
    private static var allCombinations: Set<String> {
        ...
        for adjective in ProjectNameGenerator.adjectives {
            for noun in ProjectNameGenerator.nouns {
    ...
    @Test func makeProducesAdjectiveNounFromWordLists() {
        ...
        let name = ProjectNameGenerator.make(existing: [], using: &generator)

        let parts = name.split(separator: "-", maxSplits: 1).map(String.init)
        #expect(parts.count == 2)
        #expect(ProjectNameGenerator.adjectives.contains(parts[0]))
        #expect(ProjectNameGenerator.nouns.contains(parts[1]))
    }
    ...
    @Test func makeNeverReturnsAnExistingName() {
        ...
        for _ in 0..<80 {
            let name = ProjectNameGenerator.make(existing: existing, using: &generator)

...
/// The name-header band drawn across a project's top edge (`SPEC.md` §7.1).
///
/// `ProjectView` stacks this above the terminal in a `VStack`, so the band takes
...
///
/// Focused projects are emphasized (full-opacity text, medium weight); unfocused
/// projects recede (text dimmed to ~0.4) but stay legible. The band background is
...
/// `WorkspaceModel.renamingProject`, which drives `isRenaming` here, so they share
...
/// The header shows `"{ordinal}. {name}"` (e.g. `3. calm-river`) so the project's
/// `goto_project:<N>` / Cmd+N number is visible where the project is. The ordinal is
...
struct ProjectLabel: View {
    ...
    /// The project's bare name — what rename edits and `show_project:<name>` matches.
    let name: String

    /// The project's 1-based display number, or `nil` when it has none (it is not
    /// a visible project). Rendered as a prefix, never merged into `name`.
    let ordinal: Int?

    let isFocused: Bool
    let isRenaming: Bool

    /// The project's priority, shown as a subtle trailing mark; `nil` (unset)
    /// shows nothing (SPEC §24).
    let priority: ProjectPriority?

    /// The project's deadline, shown as trailing `YYYY-MM-DD` text; `nil`
    ...
    /// Open this project's note editor (the trailing note-glyph click).
    let onOpenNote: () -> Void
    ...
    var body: some View {
        HStack(spacing: 6) {
            content
                // Focused projects are emphasized; unfocused projects stay visible
            ...
            if !isRenaming {
                ...
                ProjectPriorityDeadlineMeta(
                    ...
                    // Recede with the band on unfocused projects, but less than
    ...
    /// Focused projects use a slightly heavier weight for emphasis.
    private var headerFont: Font {
        .system(size: 11, weight: isFocused ? .medium : .regular, design: .monospaced)
    }

    /// The displayed header text: the project's number, then its name. Falls back
    /// to the bare name when the project has no number.
    ...
    /// project's note editor (`SPEC.md` §21.2). It recedes further than the
    ...
    /// above so the affordance stays discoverable on unfocused projects.
    private var noteButton: some View {
        Image(systemName: "square.and.pencil")
            ...
            .accessibilityLabel(Text("Edit note for project \(name)"))
            .help("Edit note")
    }
    ...
    private var label: some View {
        Text(title)
            ...
            .accessibilityAddTraits(.isButton)
            .accessibilityLabel(Text("Project \(title)"))
    }
...
/// no extra stages, no animation. Judgment stays in the model (`ProjectPriority`
/// / `ProjectDeadline` / the overdue predicate); this view only renders values
/// it is handed.
struct ProjectPriorityDeadlineMeta: View {
    let priority: ProjectPriority?
    let deadline: ProjectDeadline?
    let isOverdue: Bool
...
private extension ProjectPriority {

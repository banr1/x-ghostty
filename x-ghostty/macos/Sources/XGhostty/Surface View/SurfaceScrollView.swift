...
/// Wraps a XGhostty surface view in an NSScrollView to provide native macOS scrollbar support.
///
/// ## Coordinate System
...
/// - `surfaceView`: The actual XGhostty renderer, positioned to fill the visible rect
class SurfaceScrollView: NSView {
    private let scrollView: NSScrollView
    private let documentView: NSView
    private let surfaceView: XGhostty.SurfaceView
    ...
    init(contentSize: CGSize, surfaceView: XGhostty.SurfaceView) {
        ...
        // so that our primary XGhostty renderer only needs to render the viewport.
        ...
        // Listen for scrollbar updates from XGhostty
        observers.append(NotificationCenter.default.addObserver(
    ...
    private func handleScrollbarUpdate(_ notification: Notification) {
        guard let scrollbar = notification.userInfo?[SwiftUI.Notification.Name.ScrollbarKey] as? XGhostty.Action.Scrollbar else {
            return
        }

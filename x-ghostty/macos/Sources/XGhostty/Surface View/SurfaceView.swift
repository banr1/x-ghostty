...
extension XGhostty {
    ...
    struct SurfaceForApp<Content: View>: View {
        ...
        init(_ app: xghostty_app_t, @ViewBuilder content: @escaping ((SurfaceView) -> Content)) {
    ...
    struct SurfaceResizeOverlay: View {
        let geoSize: CGSize
        let size: xghostty_surface_size_s
        let overlay: XGhostty.Config.ResizeOverlay
        let position: XGhostty.Config.ResizeOverlayPosition
        let duration: UInt
    ...
    /// A surface is terminology in XGhostty for a terminal surface, or a place where a terminal is actually drawn
    ...
    /// libghostty, usually from the XGhostty configuration.
    struct SurfaceConfiguration {
        ...
        var context: xghostty_surface_context_e = XGHOSTTY_SURFACE_CONTEXT_PROJECT

        init() {}

        init(from config: xghostty_surface_config_s) {
        ...
        func withCValue<T>(view: SurfaceView, _ body: (inout xghostty_surface_config_s) throws -> T) rethrows -> T {
            var config = xghostty_surface_config_new()
            config.userdata = Unmanaged.passUnretained(view).toOpaque()
#if os(macOS)
            config.platform_tag = XGHOSTTY_PLATFORM_MACOS
            config.platform = xghostty_platform_u(macos: xghostty_platform_macos_s(
                nsview: Unmanaged.passUnretained(view).toOpaque()
            ))
            config.scale_factor = NSScreen.main!.backingScaleFactor
#elseif os(iOS)
            config.platform_tag = XGHOSTTY_PLATFORM_IOS
            config.platform = xghostty_platform_u(ios: xghostty_platform_ios_s(
                    ...
                    return try initialInput.withCString { cInput in
                        ...
                        return try keys.withCStrings { keyCStrings in
                            return try values.withCStrings { valueCStrings in
                                // Create array of xghostty_env_var_s
                                var envVars = [xghostty_env_var_s]()
                                envVars.reserveCapacity(environmentVariables.count)
                                for i in 0..<environmentVariables.count {
                                    envVars.append(xghostty_env_var_s(
                                        key: keyCStrings[i],
...
private struct GhosttySurfaceViewKey: EnvironmentKey {
    static let defaultValue: XGhostty.SurfaceView? = nil
}
...
private struct GhosttyLastFocusedSurfaceKey: EnvironmentKey {
    ...
    static let defaultValue: Weak<XGhostty.SurfaceView>? = nil
}

extension EnvironmentValues {
    var ghosttySurfaceView: XGhostty.SurfaceView? {
        get { self[GhosttySurfaceViewKey.self] }
        set { self[GhosttySurfaceViewKey.self] = newValue }
    }

    var ghosttyLastFocusedSurface: Weak<XGhostty.SurfaceView>? {
...
extension View {
    func ghosttySurfaceView(_ surfaceView: XGhostty.SurfaceView?) -> some View {
    ...
    func ghosttyLastFocusedSurface(_ surfaceView: Weak<XGhostty.SurfaceView>?) -> some View {
...
extension FocusedValues {
    ...
    struct FocusedGhosttySurface: FocusedValueKey {
        typealias Value = XGhostty.SurfaceView
    }

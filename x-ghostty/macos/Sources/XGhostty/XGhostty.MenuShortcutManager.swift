...
extension XGhostty {
    /// The manager that's responsible for updating shortcuts of XGhostty's app menu
    @MainActor
    class MenuShortcutManager {

        /// XGhostty menu items indexed by their normalized shortcut. This avoids traversing
        ...
        /// action string used for the XGhostty configuration.
        func syncMenuShortcut(_ config: XGhostty.Config, action: String?, menuItem: NSMenuItem?) {
        ...
        /// XGhostty keybind actions. This is important because it lets our surface dispatch
        ...
        func performGhosttyBindingMenuKeyEquivalent(with event: NSEvent) -> Bool {
            ...
            // If we don't have an entry for this key combo, no XGhostty-owned
...
private extension XGhostty.MenuShortcutManager {
    /// Syncs a single menu shortcut for the given action. The action string is the same
    /// action string used for the XGhostty configuration.
    ///
    /// - Returns: Whether the menu item is updated and saved in ``menuItemsByShortcut``
    func updateMenuShortcut(_ config: XGhostty.Config, action: String?, menuItem menu: NSMenuItem) -> Bool {
        guard
            let action,
...
extension XGhostty.MenuShortcutManager {
    ...
    struct MenuShortcutKey: Hashable {
        ...
        init?(_ shortcut: KeyboardShortcut) {
            // XGhostty configured shortcuts are already normalized
            // in `XGhostty.keyboardShortcut(for:)`, see also gh-#12039

...
fn initPager(alloc: Allocator) ?std.process.Child {
    ...
    // Resolve the pager command: $XGHOSTTY_PAGER > $PAGER > `less`.
    // An empty value for either env var disables paging.
    const xghostty_var = internal_os.getenv(alloc, "XGHOSTTY_PAGER") catch null;
    defer if (xghostty_var) |v| v.deinit(alloc);
    const pager_var = internal_os.getenv(alloc, "PAGER") catch null;
    defer if (pager_var) |v| v.deinit(alloc);

    const cmd: ?[]const u8 = cmd: {
        if (xghostty_var) |v| break :cmd if (v.value.len > 0) v.value else null;

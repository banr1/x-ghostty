...
- Any functions must be updated all the way through from here to
  ...
  in `include/xghostty/vt.h`. Specifically:
  ...
  4. Declare it in the corresponding header under `include/xghostty/vt/`.
- In `include/xghostty/vt.h`, always sort the header contents by:
...
- Structs:
  ...
  - May use the "sized struct" pattern: an `extern struct` with
    ...
    callers use `XGHOSTTY_INIT_SIZED` from `types.h` to zero-initialize and
    set the size.

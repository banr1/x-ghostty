...
let result = xghostty_terminal_new(nil, &terminal, opts)
guard result == XGHOSTTY_SUCCESS, let terminal else {
...
text.withCString { ptr in
    xghostty_terminal_vt_write(terminal, ptr, strlen(ptr))
}
...
var len: Int = 0
let allocResult = xghostty_formatter_format_alloc(formatter, nil, &buf, &len)
guard allocResult == XGHOSTTY_SUCCESS, let buf else {

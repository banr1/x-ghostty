# Mercator Workspace — x-ghostty

Mercator v2.0(Essence-Mapped Agentic Coding Framework)のワークスペースです。設計の正本は [.mercator/META.md](.mercator/META.md) を参照してください。

## 構造

```text
<workspace>/
├── .mercator/                    # CONTROL_ROOT — Over-Project Agent の実行ルート(META.md はここ)
└── x-ghostty/   # PROJECT_ROOT — 対象プロジェクト(In-Project Agent を含み得るが Mercator は live 起動しない)
```
...
# (または対話起草: cd ./.mercator && just trust && just new-essence — セッションの境界強制に
...
このワークスペースは `just init ../x-ghostty` で束縛・初期化済みです。別プロジェクトへ束縛し直すには `.mercator/` 内で `just init ../<other> --force` を実行します。

`just` が入っていない場合、人間は `.mercator/` 内で対応する `bash scripts/mercator-*.sh --project ../x-ghostty` を直接実行できます。ただし loop / once / init / resume / triage / supervise / essence / trust / trust-check / doctor は入口の綴りによらず人間専用で、Agent session からの直接実行も hook が拒否します。Agent が実行できるのは settings と hook が明示許可する status/validation/state 操作だけです(META.md §18.2)。

詳細な運用手順は [.mercator/README.md](.mercator/README.md) を参照してください。

...
/// Represents the XGhostty `quick-terminal-size` configuration. See the documentation for
...
struct QuickTerminalSize {
    ...
    init(from cStruct: xghostty_config_quick_terminal_size_s) {
    ...
    enum Size {
        ...
        init?(from cStruct: xghostty_quick_terminal_size_s) {
            switch cStruct.tag {
            case XGHOSTTY_QUICK_TERMINAL_SIZE_NONE:
                return nil
            case XGHOSTTY_QUICK_TERMINAL_SIZE_PERCENTAGE:
                self = .percentage(cStruct.value.percentage)
            case XGHOSTTY_QUICK_TERMINAL_SIZE_PIXELS:

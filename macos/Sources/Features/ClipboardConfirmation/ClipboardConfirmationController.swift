...
class ClipboardConfirmationController: NSWindowController {
    ...
    let surface: xghostty_surface_t
    let contents: String
    let request: XGhostty.ClipboardRequest
    let state: UnsafeMutableRawPointer?
    weak private var delegate: ClipboardConfirmationViewDelegate?

    init(surface: xghostty_surface_t, contents: String, request: XGhostty.ClipboardRequest, state: UnsafeMutableRawPointer?, delegate: ClipboardConfirmationViewDelegate) {

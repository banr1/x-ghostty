...
extension NSPasteboard {
    /// The pasteboard to used for XGhostty selection.
    static var ghosttySelection: NSPasteboard = {
        NSPasteboard(name: .init("com.mitchellh.xghostty.selection"))
    }()
    ...
    func getOpinionatedStringContents() -> String? {
        if let urls = readObjects(forClasses: [NSURL.self]) as? [URL],
           urls.count > 0 {
            return urls
                .map { $0.isFileURL ? XGhostty.Shell.escape($0.path) : $0.absoluteString }
                .joined(separator: " ")
        }
    ...
    /// The pasteboard for the XGhostty enum type.
    static func ghostty(_ clipboard: xghostty_clipboard_e) -> NSPasteboard? {
        switch clipboard {
        case XGHOSTTY_CLIPBOARD_STANDARD:
            return Self.general

        case XGHOSTTY_CLIPBOARD_SELECTION:

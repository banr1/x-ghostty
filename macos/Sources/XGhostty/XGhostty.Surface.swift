...
extension XGhostty {
    /// Represents a single surface within XGhostty.
    ///
    /// NOTE(mitchellh): This is a work-in-progress class as part of a general refactor
    /// of our XGhostty data model. At the time of writing there's still a ton of surface
    /// functionality that is not encapsulated in this class. It is planned to migrate that
    /// all over.
    ///
    /// Wraps a `xghostty_surface_t`
    final class Surface: Sendable {
        private let surface: xghostty_surface_t
        ...
        var unsafeCValue: xghostty_surface_t {
            surface
        }

        /// Initialize from the C structure.
        init(cSurface: xghostty_surface_t) {
        ...
        func sendKeyEvent(_ event: Input.KeyEvent) {
            event.withCValue { cEvent in
                xghostty_surface_key(surface, cEvent)
            }
        }
        ...
        @MainActor
        func keyIsBinding(_ event: xghostty_input_key_s) -> Input.BindingFlags? {
            var flags = xghostty_binding_flags_e(0)
            guard xghostty_surface_key_is_binding(surface, event, &flags) else { return nil }
            return Input.BindingFlags(cFlags: flags)
        }

        /// See `keyIsBinding(_ event: xghostty_input_key_s)`.
        @MainActor
        ...
        @MainActor
        var ttyName: String? {
            let ttyName = AllocatedString(xghostty_surface_tty_name(surface)).string

...
extension XGhostty {
    ...
    class SurfaceView: OSSurfaceView, Codable, Identifiable {
                ...
                if let searchState {
                    ...
                    searchNeedleCancellable = searchState.$needle
                        ...
                        .sink { [weak self] needle in
                            ...
                            xghostty_surface_binding_action(surface, action, UInt(action.lengthOfBytes(using: .utf8)))
                        }
        ...
        // detected with the set_password_input_cb on the XGhostty state.
        ...
        var needsConfirmQuit: Bool {
            ...
            return xghostty_surface_needs_confirm_quit(surface)
        }

        // Returns true if the process in this surface has exited.
        var processExited: Bool {
            guard let surface = self.surface else { return true }
            return xghostty_surface_process_exited(surface)
        }

        // Returns the inspector instance for this surface, or nil if the
        // surface has been closed or no inspector is active.
        var inspector: XGhostty.Inspector? {
            guard let surface = self.surface else { return nil }
            guard let cInspector = xghostty_surface_inspector(surface) else { return nil }
            return XGhostty.Inspector(cInspector: cInspector)
        }
        ...
        @Published var inspectorVisible: Bool = false {
            didSet {
                if oldValue && !inspectorVisible {
                    guard let surface = self.surface else { return }
                    xghostty_inspector_free(surface)
                }
            }
        }
        ...
        private(set) var surfaceModel: XGhostty.Surface?

        /// Returns the underlying C value for the surface. See "note" on surfaceModel.
        override var surface: xghostty_surface_t? {
        ...
        var scrollbar: XGhostty.Action.Scrollbar?
        ...
        init(_ app: xghostty_app_t, baseConfig: SurfaceConfiguration? = nil, uuid: UUID? = nil) {
            ...
            cachedScreenContents = .init(duration: .milliseconds(500)) { [weak self] in
                ...
                guard let surface = self.surface else { return "" }
                var text = xghostty_text_s()
                let sel = xghostty_selection_s(
                    top_left: xghostty_point_s(
                        tag: XGHOSTTY_POINT_SCREEN,
                        coord: XGHOSTTY_POINT_COORD_TOP_LEFT,
                        x: 0,
                        y: 0),
                    bottom_right: xghostty_point_s(
                        tag: XGHOSTTY_POINT_SCREEN,
                        coord: XGHOSTTY_POINT_COORD_BOTTOM_RIGHT,
                        x: 0,
                        y: 0),
                    rectangle: false)
                guard xghostty_surface_read_text(surface, sel, &text) else { return "" }
                defer { xghostty_surface_free_text(surface, &text) }
            ...
            center.addObserver(
                ...
                name: XGhostty.Notification.didUpdateRendererHealth,
                object: self)
            center.addObserver(
                self,
                selector: #selector(ghosttyDidContinueKeySequence),
                name: XGhostty.Notification.didContinueKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(ghosttyDidEndKeySequence),
                name: XGhostty.Notification.didEndKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(ghosttyDidChangeKeyTable),
                name: XGhostty.Notification.didChangeKeyTable,
                object: self)
            center.addObserver(
                self,
            ...
            let surface = surface_cfg.withCValue(view: self) { surface_cfg_c in
                xghostty_surface_new(app, &surface_cfg_c)
            }
            guard let surface = surface else {
                self.error = XGhostty.Error.apiFailed
                return
            }
            self.surfaceModel = XGhostty.Surface(cSurface: surface)
        ...
        override func focusDidChange(_ focused: Bool) {
            ...
            // Notify libghostty
            xghostty_surface_set_focus(surface, focused)
        ...
        override func sizeDidChange(_ size: CGSize) {
            // XGhostty wants to know the actual framebuffer size... It is very important
        ...
        private func setSurfaceSize(width: UInt32, height: UInt32) {
            ...
            xghostty_surface_set_size(surface, width, height)

            // Update our cached size metrics
            let size = xghostty_surface_size(surface)
        ...
        func setCursorShape(_ shape: xghostty_action_mouse_shape_e) {
            switch shape {
            case XGHOSTTY_MOUSE_SHAPE_DEFAULT:
                pointerStyle = .default

            case XGHOSTTY_MOUSE_SHAPE_TEXT:
                pointerStyle = .horizontalText

            case XGHOSTTY_MOUSE_SHAPE_GRAB:
                pointerStyle = .grabIdle

            case XGHOSTTY_MOUSE_SHAPE_GRABBING:
                pointerStyle = .grabActive

            case XGHOSTTY_MOUSE_SHAPE_POINTER:
                pointerStyle = .link

            case XGHOSTTY_MOUSE_SHAPE_W_RESIZE:
                pointerStyle = .resizeLeft

            case XGHOSTTY_MOUSE_SHAPE_E_RESIZE:
                pointerStyle = .resizeRight

            case XGHOSTTY_MOUSE_SHAPE_N_RESIZE:
                pointerStyle = .resizeUp

            case XGHOSTTY_MOUSE_SHAPE_S_RESIZE:
                pointerStyle = .resizeDown

            case XGHOSTTY_MOUSE_SHAPE_NS_RESIZE:
                pointerStyle = .resizeUpDown

            case XGHOSTTY_MOUSE_SHAPE_EW_RESIZE:
                pointerStyle = .resizeLeftRight

            case XGHOSTTY_MOUSE_SHAPE_VERTICAL_TEXT:
                pointerStyle = .verticalText

            case XGHOSTTY_MOUSE_SHAPE_CONTEXT_MENU:
                pointerStyle = .contextMenu

            case XGHOSTTY_MOUSE_SHAPE_CROSSHAIR:
                pointerStyle = .crosshair

            case XGHOSTTY_MOUSE_SHAPE_NOT_ALLOWED:
        ...
        @objc private func onUpdateRendererHealth(notification: SwiftUI.Notification) {
            ...
            guard let health = healthAny as? xghostty_action_renderer_health_e else { return }
            DispatchQueue.main.async { [weak self] in
                self?.healthy = health == XGHOSTTY_RENDERER_HEALTH_HEALTHY
            }
        }

        @objc private func ghosttyDidContinueKeySequence(notification: SwiftUI.Notification) {
            guard let keyAny = notification.userInfo?[XGhostty.Notification.KeySequenceKey] else { return }
        ...
        @objc private func ghosttyDidChangeKeyTable(notification: SwiftUI.Notification) {
            guard let action = notification.userInfo?[XGhostty.Notification.KeyTableKey] as? XGhostty.Action.KeyTable else { return }
        ...
        @objc private func ghosttyColorDidChange(_ notification: SwiftUI.Notification) {
            ...
            ] as? XGhostty.Action.ColorChange else { return }
        ...
        @objc private func windowDidChangeScreen(notification: SwiftUI.Notification) {
            ...
            xghostty_surface_set_display_id(surface, screen.displayID ?? 0)
        ...
        override func viewDidChangeBackingProperties() {
            ...
            xghostty_surface_set_content_scale(surface, xScale, yScale)
        ...
        override func mouseDown(with event: NSEvent) {
            guard let surface = self.surface else { return }
            let mods = XGhostty.ghosttyMods(event.modifierFlags)
            xghostty_surface_mouse_button(surface, XGHOSTTY_MOUSE_PRESS, XGHOSTTY_MOUSE_LEFT, mods)
        }
        ...
        override func mouseUp(with event: NSEvent) {
            ...
            guard let surface = self.surface else { return }
            let mods = XGhostty.ghosttyMods(event.modifierFlags)
            xghostty_surface_mouse_button(surface, XGHOSTTY_MOUSE_RELEASE, XGHOSTTY_MOUSE_LEFT, mods)

            // Release pressure
            xghostty_surface_mouse_pressure(surface, 0, 0)
        }

        override func otherMouseDown(with event: NSEvent) {
            guard let surface = self.surface else { return }
            let mods = XGhostty.ghosttyMods(event.modifierFlags)
            let button = XGhostty.Input.MouseButton(fromNSEventButtonNumber: event.buttonNumber)
            xghostty_surface_mouse_button(surface, XGHOSTTY_MOUSE_PRESS, button.cMouseButton, mods)
        }
        ...
        override func otherMouseUp(with event: NSEvent) {
            ...
            let mods = XGhostty.ghosttyMods(event.modifierFlags)
            let button = XGhostty.Input.MouseButton(fromNSEventButtonNumber: event.buttonNumber)
            xghostty_surface_mouse_button(surface, XGHOSTTY_MOUSE_RELEASE, button.cMouseButton, mods)
        }
        ...
        override func rightMouseDown(with event: NSEvent) {
            ...
            let mods = XGhostty.ghosttyMods(event.modifierFlags)
            if xghostty_surface_mouse_button(
                surface,
        ...
        override func mouseEntered(with event: NSEvent) {
            ...
            let mouseEvent = XGhostty.Input.MousePosEvent(
                x: pos.x,
        ...
        override func pressureChange(with event: NSEvent) {
            ...
            // Notify XGhostty first. We do this because this will let XGhostty handle
            // state setup that we'll need for later pressure handling (such as
            // QuickLook)
            xghostty_surface_mouse_pressure(surface, UInt32(event.stage), Double(event.pressure))
        ...
        override func keyDown(with event: NSEvent) {
            ...
            let translationModsGhostty = XGhostty.eventModifierFlags(
                mods: xghostty_surface_key_translation_mods(
                    surface,
                    XGhostty.ghosttyMods(event.modifierFlags)
                )
            )
            ...
            let action = event.isARepeat ? XGHOSTTY_ACTION_REPEAT : XGHOSTTY_ACTION_PRESS
            ...
            if markedTextBefore,
               ...
               list.count > 0 {
                for text in list {
                    if XGhostty.SurfaceView.shouldSuppressComposingControlInput(
                        text,
                        composing: composing
                    ) {
                        continue
        ...
        override func keyUp(with event: NSEvent) {
            _ = keyAction(XGHOSTTY_ACTION_RELEASE, event: event)
        }
        ...
        override func flagsChanged(with event: NSEvent) {
            ...
            case 0x39: mod = XGHOSTTY_MODS_CAPS.rawValue
            case 0x38, 0x3C: mod = XGHOSTTY_MODS_SHIFT.rawValue
            case 0x3B, 0x3E: mod = XGHOSTTY_MODS_CTRL.rawValue
            case 0x3A, 0x3D: mod = XGHOSTTY_MODS_ALT.rawValue
            case 0x37, 0x36: mod = XGHOSTTY_MODS_SUPER.rawValue
            default: return
            }
            ...
            var action = XGHOSTTY_ACTION_RELEASE
        ...
        private func keyAction(
            ...
            // character. Control characters are encoded by XGhostty itself.
            // Without this, `ctrl+enter` does the wrong thing.
            if let text, text.count > 0,
               let codepoint = text.utf8.first, codepoint >= 0x20 {
                return text.withCString { ptr in
                    key_ev.text = ptr
                    return xghostty_surface_key(surface, key_ev)
                }
            } else {
                return xghostty_surface_key(surface, key_ev)
            }
        ...
        private func shouldReplayCommittedPreeditKey(_ event: NSEvent) -> Bool {
            guard let key = XGhostty.Input.Key(keyCode: event.keyCode) else { return false }
            switch key {
        ...
        private func committedPreeditTextAction(
            ...
            var key_ev = xghostty_input_key_s()
            key_ev.action = action
            key_ev.keycode = 0
            key_ev.text = nil
            key_ev.composing = false
            key_ev.mods = XGHOSTTY_MODS_NONE
            key_ev.consumed_mods = XGHOSTTY_MODS_NONE
        ...
        override func quickLook(with event: NSEvent) {
            ...
            var text = xghostty_text_s()
            guard xghostty_surface_quicklook_word(surface, &text) else { return super.quickLook(with: event) }
            defer { xghostty_surface_free_text(surface, &text) }
            ...
            if let fontRaw = xghostty_surface_quicklook_font(surface) {
                // Memory management here is wonky: xghostty_surface_quicklook_font
            ...
            // XGhostty coordinate system is top-left, convert to bottom-left for AppKit
        ...
        @IBAction func splitRight(_ sender: Any) {
            guard let surface = self.surface else { return }
            xghostty_surface_split(surface, XGHOSTTY_SPLIT_DIRECTION_RIGHT)
        }

        @IBAction func splitLeft(_ sender: Any) {
            guard let surface = self.surface else { return }
            xghostty_surface_split(surface, XGHOSTTY_SPLIT_DIRECTION_LEFT)
        }

        @IBAction func splitDown(_ sender: Any) {
            guard let surface = self.surface else { return }
            xghostty_surface_split(surface, XGHOSTTY_SPLIT_DIRECTION_DOWN)
        }

        @IBAction func splitUp(_ sender: Any) {
            guard let surface = self.surface else { return }
            xghostty_surface_split(surface, XGHOSTTY_SPLIT_DIRECTION_UP)
        }
        ...
        func showUserNotification(title: String, body: String, requireFocus: Bool = true) {
            ...
            content.categoryIdentifier = XGhostty.userNotificationCategory
            content.userInfo = [
        ...
        struct DerivedConfig {
            ...
            let scrollbar: XGhostty.Config.Scrollbar
        ...
        required convenience init(from decoder: Decoder) throws {
            // Decoding uses the global XGhostty app
...
extension XGhostty.SurfaceView: NSTextInputClient {
    ...
    func selectedRange() -> NSRange {
        ...
        // Get our range from the XGhostty API. There is a race condition between getting the
        ...
        var text = xghostty_text_s()
        guard xghostty_surface_read_selection(surface, &text) else { return NSRange() }
        defer { xghostty_surface_free_text(surface, &text) }
    ...
    func attributedSubstring(forProposedRange range: NSRange, actualRange: NSRangePointer?) -> NSAttributedString? {
        // XGhostty.logger.warning("pressure substring range=\(range) selectedRange=\(self.selectedRange())")
        guard let surface = self.surface else { return nil }
        ...
        var text = xghostty_text_s()
        guard xghostty_surface_read_selection(surface, &text) else { return nil }
        defer { xghostty_surface_free_text(surface, &text) }
    ...
    func firstRect(forCharacterRange range: NSRange, actualRange: NSRangePointer?) -> NSRect {
        ...
        // XGhostty will tell us where it thinks an IME keyboard should render.
        var x: Double = 0
        var y: Double = 0
        ...
        if range.length > 0 && range != self.selectedRange() {
            // QuickLook
            var text = xghostty_text_s()
            if xghostty_surface_read_selection(surface, &text) {
                ...
                // Free our text
                xghostty_surface_free_text(surface, &text)
            } else {
                xghostty_surface_ime_point(surface, &x, &y, &width, &height)
            }
        } else {
            xghostty_surface_ime_point(surface, &x, &y, &width, &height)
        }
        ...
        // XGhostty coordinates are in top-left (0, 0) so we have to convert to
...
extension XGhostty.SurfaceView: NSServicesMenuRequestor {
    override func validRequestor(
        ...
        if (returnType == nil || receivable.contains(returnType!)) &&
            ...
            // performance hits because `xghostty_surface_has_selection` isn't free.
            if let sendType, sendableRequiresSelection.contains(sendType) {
                if surface == nil || !xghostty_surface_has_selection(surface) {
    ...
    func writeSelection(
        ...
        // Read the selection
        var text = xghostty_text_s()
        guard xghostty_surface_read_selection(surface, &text) else { return false }
        defer { xghostty_surface_free_text(surface, &text) }
    ...
    func readSelection(from pboard: NSPasteboard) -> Bool {
        ...
        str.withCString { ptr in
            ...
            xghostty_surface_text(surface, ptr, UInt(len - 1))
        }
...
extension XGhostty.SurfaceView: NSMenuItemValidation {
...
extension XGhostty.SurfaceView {
    ...
    override func performDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        ...
        if let url = pb.string(forType: .URL) {
            ...
            content = XGhostty.Shell.escape(url)
           ...
           urls.count > 0 {
            ...
            content = urls
                .map { XGhostty.Shell.escape($0.path) }
                .joined(separator: " ")
...
extension XGhostty.SurfaceView {
    ...
    /// this to copy styling information as well but we need to augment XGhostty core to
    /// expose that.
    ///

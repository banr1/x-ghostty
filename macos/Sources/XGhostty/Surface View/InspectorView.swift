...
extension XGhostty {
    ...
    struct InspectableSurface: View {
        ...
        var body: some View {
            ...
            .onChange(of: surfaceView.inspectorVisible) { inspectorVisible in
                ...
                if inspectorVisible {
                    ...
                    XGhostty.moveFocus(to: surfaceView)
                }
            }
        }
        ...
        private func onControlInspector(_ notification: SwiftUI.Notification) {
            ...
            guard let mode = modeAny as? xghostty_action_inspector_e else { return }

            switch mode {
            case XGHOSTTY_INSPECTOR_TOGGLE:
                surfaceView.inspectorVisible = !surfaceView.inspectorVisible

            case XGHOSTTY_INSPECTOR_SHOW:
                surfaceView.inspectorVisible = true

            case XGHOSTTY_INSPECTOR_HIDE:
    ...
    class InspectorView: MTKView, NSTextInputClient {
        ...
        private var inspector: XGhostty.Inspector? {
        ...
        override func mouseDown(with event: NSEvent) {
            ...
            inspector.mouseButton(XGHOSTTY_MOUSE_PRESS, button: XGHOSTTY_MOUSE_LEFT, mods: mods)
        }

        override func mouseUp(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = XGhostty.ghosttyMods(event.modifierFlags)
            inspector.mouseButton(XGHOSTTY_MOUSE_RELEASE, button: XGHOSTTY_MOUSE_LEFT, mods: mods)
        }

        override func rightMouseDown(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = XGhostty.ghosttyMods(event.modifierFlags)
            inspector.mouseButton(XGHOSTTY_MOUSE_PRESS, button: XGHOSTTY_MOUSE_RIGHT, mods: mods)
        }

        override func rightMouseUp(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = XGhostty.ghosttyMods(event.modifierFlags)
            inspector.mouseButton(XGHOSTTY_MOUSE_RELEASE, button: XGHOSTTY_MOUSE_RIGHT, mods: mods)
        }
        ...
        override func scrollWheel(with event: NSEvent) {
            ...
            var momentum: xghostty_input_mouse_momentum_e = XGHOSTTY_MOUSE_MOMENTUM_NONE
            switch event.momentumPhase {
            case .began:
                momentum = XGHOSTTY_MOUSE_MOMENTUM_BEGAN
            case .stationary:
                momentum = XGHOSTTY_MOUSE_MOMENTUM_STATIONARY
            case .changed:
                momentum = XGHOSTTY_MOUSE_MOMENTUM_CHANGED
            case .ended:
                momentum = XGHOSTTY_MOUSE_MOMENTUM_ENDED
            case .cancelled:
                momentum = XGHOSTTY_MOUSE_MOMENTUM_CANCELLED
            case .mayBegin:
                momentum = XGHOSTTY_MOUSE_MOMENTUM_MAY_BEGIN
            default:
                break
            }
        ...
        override func keyUp(with event: NSEvent) {
            keyAction(XGHOSTTY_ACTION_RELEASE, event: event)
        }
        ...
        override func flagsChanged(with event: NSEvent) {
            ...
            if mods.rawValue & mod != 0 { action = XGHOSTTY_ACTION_PRESS }

            keyAction(action, event: event)
        }

        private func keyAction(_ action: xghostty_input_action_e, event: NSEvent) {
            guard let inspector = self.inspector else { return }
            guard let key = XGhostty.Input.Key(keyCode: event.keyCode) else { return }
            let mods = XGhostty.ghosttyMods(event.modifierFlags)

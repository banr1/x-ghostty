...
extension XGhostty {
    class OSSurfaceView: OSView, ObservableObject {
        ...
        @Published var surfaceSize: xghostty_surface_size_s?
        ...
        /// A message sent from `xghostty_surface_t` when a child process exited
        @Published private(set) var childExitedMessage: ChildExitedMessage?

        var surface: xghostty_surface_t? {
            nil
        }
...
extension XGhostty.OSSurfaceView {
    @MainActor class SearchState: ObservableObject {
        ...
        init(
            from startSearch: XGhostty.Action.StartSearch,

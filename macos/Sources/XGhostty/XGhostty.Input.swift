...
extension XGhostty {
    ...
    /// because XGhostty input triggers are a superset of what can be represented by a macOS
    ...
    /// handling for XGhostty is handled at a lower level (usually). This function should generally only
    /// be used for things like NSMenu that only support keyboard shortcuts anyways.
    static func keyboardShortcut(for trigger: xghostty_input_trigger_s) -> KeyboardShortcut? {
        let key: KeyEquivalent
        switch trigger.tag {
        case XGHOSTTY_TRIGGER_PHYSICAL:
        ...
        case XGHOSTTY_TRIGGER_CATCH_ALL:
        ...
        return KeyboardShortcut(
            key,
            modifiers: EventModifiers(nsFlags: XGhostty.eventModifierFlags(mods: trigger.mods)))
    }

    // MARK: Mods

    /// Returns the event modifier flags set for the XGhostty mods enum.
    static func eventModifierFlags(mods: xghostty_input_mods_e) -> NSEvent.ModifierFlags {
        var flags = NSEvent.ModifierFlags(rawValue: 0)
        if mods.rawValue & XGHOSTTY_MODS_SHIFT.rawValue != 0 { flags.insert(.shift) }
        if mods.rawValue & XGHOSTTY_MODS_CTRL.rawValue != 0 { flags.insert(.control) }
        if mods.rawValue & XGHOSTTY_MODS_ALT.rawValue != 0 { flags.insert(.option) }
        if mods.rawValue & XGHOSTTY_MODS_SUPER.rawValue != 0 { flags.insert(.command) }
        return flags
    }

    /// Translate event modifier flags to a ghostty mods enum.
    static func ghosttyMods(_ flags: NSEvent.ModifierFlags) -> xghostty_input_mods_e {
        var mods: UInt32 = XGHOSTTY_MODS_NONE.rawValue

        if flags.contains(.shift) { mods |= XGHOSTTY_MODS_SHIFT.rawValue }
        if flags.contains(.control) { mods |= XGHOSTTY_MODS_CTRL.rawValue }
        if flags.contains(.option) { mods |= XGHOSTTY_MODS_ALT.rawValue }
        if flags.contains(.command) { mods |= XGHOSTTY_MODS_SUPER.rawValue }
        if flags.contains(.capsLock) { mods |= XGHOSTTY_MODS_CAPS.rawValue }

        // Handle sided input. We can't tell that both are pressed in the
        // XGhostty structure but that's okay -- we don't use that information.
        let rawFlags = flags.rawValue
        if rawFlags & UInt(NX_DEVICERSHIFTKEYMASK) != 0 { mods |= XGHOSTTY_MODS_SHIFT_RIGHT.rawValue }
        if rawFlags & UInt(NX_DEVICERCTLKEYMASK) != 0 { mods |= XGHOSTTY_MODS_CTRL_RIGHT.rawValue }
        if rawFlags & UInt(NX_DEVICERALTKEYMASK) != 0 { mods |= XGHOSTTY_MODS_ALT_RIGHT.rawValue }
        if rawFlags & UInt(NX_DEVICERCMDKEYMASK) != 0 { mods |= XGHOSTTY_MODS_SUPER_RIGHT.rawValue }

        return xghostty_input_mods_e(mods)
    }

    /// A map from the XGhostty key enum to the keyEquivalent string for shortcuts. Note that
    /// not all ghostty key enum values are represented here because not all of them can be
    /// mapped to a KeyEquivalent.
    static let keyToEquivalent: [xghostty_input_key_e: KeyEquivalent] = [
        // Function keys
        XGHOSTTY_KEY_ARROW_UP: .upArrow,
        XGHOSTTY_KEY_ARROW_DOWN: .downArrow,
        XGHOSTTY_KEY_ARROW_LEFT: .leftArrow,
        XGHOSTTY_KEY_ARROW_RIGHT: .rightArrow,
        XGHOSTTY_KEY_HOME: .home,
        XGHOSTTY_KEY_END: .end,
        XGHOSTTY_KEY_DELETE: .deleteForward,
        XGHOSTTY_KEY_PAGE_UP: .pageUp,
        XGHOSTTY_KEY_PAGE_DOWN: .pageDown,
        XGHOSTTY_KEY_ESCAPE: .escape,
        XGHOSTTY_KEY_ENTER: .return,
        XGHOSTTY_KEY_TAB: .tab,
        XGHOSTTY_KEY_BACKSPACE: .delete,
        XGHOSTTY_KEY_SPACE: .space,
    ]
}

// MARK: XGhostty.Input.BindingFlags
...
extension XGhostty.Input {
    ...
    struct BindingFlags: OptionSet, Sendable {
        ...
        static let consumed = BindingFlags(rawValue: XGHOSTTY_BINDING_FLAGS_CONSUMED.rawValue)
        static let all = BindingFlags(rawValue: XGHOSTTY_BINDING_FLAGS_ALL.rawValue)
        static let global = BindingFlags(rawValue: XGHOSTTY_BINDING_FLAGS_GLOBAL.rawValue)
        static let performable = BindingFlags(rawValue: XGHOSTTY_BINDING_FLAGS_PERFORMABLE.rawValue)

        init(rawValue: UInt32) {
            self.rawValue = rawValue
        }

        init(cFlags: xghostty_binding_flags_e) {
            self.rawValue = cFlags.rawValue
        }

        var cFlags: xghostty_binding_flags_e {
            xghostty_binding_flags_e(rawValue)
        }
    }
}

// MARK: XGhostty.Input.KeyEvent
...
extension XGhostty.Input {
    ...
    struct KeyEvent {
        ...
        init?(cValue: xghostty_input_key_s) {
            // Convert action
            switch cValue.action {
            case XGHOSTTY_ACTION_PRESS: self.action = .press
            case XGHOSTTY_ACTION_RELEASE: self.action = .release
            case XGHOSTTY_ACTION_REPEAT: self.action = .repeat
        ...
        ///
        /// This method safely converts the Swift KeyEntity to a C `xghostty_input_key_s` struct
        ...
        @discardableResult
        func withCValue<T>(execute: (xghostty_input_key_s) -> T) -> T {
            var keyEvent = xghostty_input_key_s()
...
extension XGhostty.Input {
    ...
    enum Action: String, CaseIterable {
        ...
        var cAction: xghostty_input_action_e {
            switch self {
            case .release: XGHOSTTY_ACTION_RELEASE
            case .press: XGHOSTTY_ACTION_PRESS
            case .repeat: XGHOSTTY_ACTION_REPEAT
            }
        }
    }
}
...
extension XGhostty.Input {
    ...
    struct MouseButtonEvent {
        ...
        init?(state: xghostty_input_mouse_state_e, button: xghostty_input_mouse_button_e, mods: xghostty_input_mods_e) {
            // Convert state
            switch state {
            case XGHOSTTY_MOUSE_RELEASE: self.action = .release
            case XGHOSTTY_MOUSE_PRESS: self.action = .press
            default: return nil
            }

            // Convert button
            switch button {
            case XGHOSTTY_MOUSE_UNKNOWN: self.button = .unknown
            case XGHOSTTY_MOUSE_LEFT: self.button = .left
            case XGHOSTTY_MOUSE_RIGHT: self.button = .right
            case XGHOSTTY_MOUSE_MIDDLE: self.button = .middle
            default: return nil
            }
...
// MARK: XGhostty.Input.MouseState
...
extension XGhostty.Input {
    ...
    enum MouseState: String, CaseIterable {
        ...
        var cMouseState: xghostty_input_mouse_state_e {
            switch self {
            case .release: XGHOSTTY_MOUSE_RELEASE
            case .press: XGHOSTTY_MOUSE_PRESS
            }
        }
    }
}
...
extension XGhostty.Input {
    ...
    enum MouseButton: String, CaseIterable {
        ...
        var cMouseButton: xghostty_input_mouse_button_e {
            switch self {
            case .unknown: XGHOSTTY_MOUSE_UNKNOWN
            case .left: XGHOSTTY_MOUSE_LEFT
            case .right: XGHOSTTY_MOUSE_RIGHT
            case .middle: XGHOSTTY_MOUSE_MIDDLE
            case .four: XGHOSTTY_MOUSE_FOUR
            case .five: XGHOSTTY_MOUSE_FIVE
            case .six: XGHOSTTY_MOUSE_SIX
            case .seven: XGHOSTTY_MOUSE_SEVEN
            case .eight: XGHOSTTY_MOUSE_EIGHT
            case .nine: XGHOSTTY_MOUSE_NINE
            case .ten: XGHOSTTY_MOUSE_TEN
            case .eleven: XGHOSTTY_MOUSE_ELEVEN
            }
        }
...
extension XGhostty.Input {
    /// `xghostty_input_scroll_mods_t` - Scroll event modifiers
    ///
    ...
    struct ScrollMods {
        ...
        var cScrollMods: xghostty_input_scroll_mods_t {
            rawValue
        }
    }
}

// MARK: XGhostty.Input.Momentum

extension XGhostty.Input {
    /// `xghostty_input_mouse_momentum_e` - Momentum phase for scroll events
    enum Momentum: UInt8, CaseIterable {
        ...
        var cMomentum: xghostty_input_mouse_momentum_e {
            switch self {
            case .none: XGHOSTTY_MOUSE_MOMENTUM_NONE
            case .began: XGHOSTTY_MOUSE_MOMENTUM_BEGAN
            case .stationary: XGHOSTTY_MOUSE_MOMENTUM_STATIONARY
            case .changed: XGHOSTTY_MOUSE_MOMENTUM_CHANGED
            case .ended: XGHOSTTY_MOUSE_MOMENTUM_ENDED
            case .cancelled: XGHOSTTY_MOUSE_MOMENTUM_CANCELLED
            case .mayBegin: XGHOSTTY_MOUSE_MOMENTUM_MAY_BEGIN
            }
        }
    }
}
...
extension XGhostty.Input.Momentum {
...
extension XGhostty.Input {
    ...
    struct Mods: OptionSet {
        ...
        static let none = Mods(rawValue: XGHOSTTY_MODS_NONE.rawValue)
        static let shift = Mods(rawValue: XGHOSTTY_MODS_SHIFT.rawValue)
        static let ctrl = Mods(rawValue: XGHOSTTY_MODS_CTRL.rawValue)
        static let alt = Mods(rawValue: XGHOSTTY_MODS_ALT.rawValue)
        static let `super` = Mods(rawValue: XGHOSTTY_MODS_SUPER.rawValue)
        static let caps = Mods(rawValue: XGHOSTTY_MODS_CAPS.rawValue)
        static let shiftRight = Mods(rawValue: XGHOSTTY_MODS_SHIFT_RIGHT.rawValue)
        static let ctrlRight = Mods(rawValue: XGHOSTTY_MODS_CTRL_RIGHT.rawValue)
        static let altRight = Mods(rawValue: XGHOSTTY_MODS_ALT_RIGHT.rawValue)
        static let superRight = Mods(rawValue: XGHOSTTY_MODS_SUPER_RIGHT.rawValue)

        var cMods: xghostty_input_mods_e {
            xghostty_input_mods_e(rawValue)
        }
        ...
        init(cMods: xghostty_input_mods_e) {
            self.rawValue = cMods.rawValue
        }

        init(nsFlags: NSEvent.ModifierFlags) {
            self.init(cMods: XGhostty.ghosttyMods(nsFlags))
        }

        var nsFlags: NSEvent.ModifierFlags {
            XGhostty.eventModifierFlags(mods: cMods)
        }
    }
}

// MARK: XGhostty.Input.Key
...
extension XGhostty.Input {
    ...
    enum Key: String {
        ...
        var cKey: xghostty_input_key_e {
            switch self {
            // Writing System Keys
            case .backquote: XGHOSTTY_KEY_BACKQUOTE
            case .backslash: XGHOSTTY_KEY_BACKSLASH
            case .bracketLeft: XGHOSTTY_KEY_BRACKET_LEFT
            case .bracketRight: XGHOSTTY_KEY_BRACKET_RIGHT
            case .comma: XGHOSTTY_KEY_COMMA
            case .digit0: XGHOSTTY_KEY_DIGIT_0
            case .digit1: XGHOSTTY_KEY_DIGIT_1
            case .digit2: XGHOSTTY_KEY_DIGIT_2
            case .digit3: XGHOSTTY_KEY_DIGIT_3
            case .digit4: XGHOSTTY_KEY_DIGIT_4
            case .digit5: XGHOSTTY_KEY_DIGIT_5
            case .digit6: XGHOSTTY_KEY_DIGIT_6
            case .digit7: XGHOSTTY_KEY_DIGIT_7
            case .digit8: XGHOSTTY_KEY_DIGIT_8
            case .digit9: XGHOSTTY_KEY_DIGIT_9
            case .equal: XGHOSTTY_KEY_EQUAL
            case .intlBackslash: XGHOSTTY_KEY_INTL_BACKSLASH
            case .intlRo: XGHOSTTY_KEY_INTL_RO
            case .intlYen: XGHOSTTY_KEY_INTL_YEN
            case .a: XGHOSTTY_KEY_A
            case .b: XGHOSTTY_KEY_B
            case .c: XGHOSTTY_KEY_C
            case .d: XGHOSTTY_KEY_D
            case .e: XGHOSTTY_KEY_E
            case .f: XGHOSTTY_KEY_F
            case .g: XGHOSTTY_KEY_G
            case .h: XGHOSTTY_KEY_H
            case .i: XGHOSTTY_KEY_I
            case .j: XGHOSTTY_KEY_J
            case .k: XGHOSTTY_KEY_K
            case .l: XGHOSTTY_KEY_L
            case .m: XGHOSTTY_KEY_M
            case .n: XGHOSTTY_KEY_N
            case .o: XGHOSTTY_KEY_O
            case .p: XGHOSTTY_KEY_P
            case .q: XGHOSTTY_KEY_Q
            case .r: XGHOSTTY_KEY_R
            case .s: XGHOSTTY_KEY_S
            case .t: XGHOSTTY_KEY_T
            case .u: XGHOSTTY_KEY_U
            case .v: XGHOSTTY_KEY_V
            case .w: XGHOSTTY_KEY_W
            case .x: XGHOSTTY_KEY_X
            case .y: XGHOSTTY_KEY_Y
            case .z: XGHOSTTY_KEY_Z
            case .minus: XGHOSTTY_KEY_MINUS
            case .period: XGHOSTTY_KEY_PERIOD
            case .quote: XGHOSTTY_KEY_QUOTE
            case .semicolon: XGHOSTTY_KEY_SEMICOLON
            case .slash: XGHOSTTY_KEY_SLASH

            // Functional Keys
            case .altLeft: XGHOSTTY_KEY_ALT_LEFT
            case .altRight: XGHOSTTY_KEY_ALT_RIGHT
            case .backspace: XGHOSTTY_KEY_BACKSPACE
            case .capsLock: XGHOSTTY_KEY_CAPS_LOCK
            case .contextMenu: XGHOSTTY_KEY_CONTEXT_MENU
            case .controlLeft: XGHOSTTY_KEY_CONTROL_LEFT
            case .controlRight: XGHOSTTY_KEY_CONTROL_RIGHT
            case .enter: XGHOSTTY_KEY_ENTER
            case .metaLeft: XGHOSTTY_KEY_META_LEFT
            case .metaRight: XGHOSTTY_KEY_META_RIGHT
            case .shiftLeft: XGHOSTTY_KEY_SHIFT_LEFT
            case .shiftRight: XGHOSTTY_KEY_SHIFT_RIGHT
            case .space: XGHOSTTY_KEY_SPACE
            case .tab: XGHOSTTY_KEY_TAB
            case .convert: XGHOSTTY_KEY_CONVERT
            case .kanaMode: XGHOSTTY_KEY_KANA_MODE
            case .nonConvert: XGHOSTTY_KEY_NON_CONVERT

            // Control Pad Section
            case .delete: XGHOSTTY_KEY_DELETE
            case .end: XGHOSTTY_KEY_END
            case .help: XGHOSTTY_KEY_HELP
            case .home: XGHOSTTY_KEY_HOME
            case .insert: XGHOSTTY_KEY_INSERT
            case .pageDown: XGHOSTTY_KEY_PAGE_DOWN
            case .pageUp: XGHOSTTY_KEY_PAGE_UP

            // Arrow Pad Section
            case .arrowDown: XGHOSTTY_KEY_ARROW_DOWN
            case .arrowLeft: XGHOSTTY_KEY_ARROW_LEFT
            case .arrowRight: XGHOSTTY_KEY_ARROW_RIGHT
            case .arrowUp: XGHOSTTY_KEY_ARROW_UP

            // Numpad Section
            case .numLock: XGHOSTTY_KEY_NUM_LOCK
            case .numpad0: XGHOSTTY_KEY_NUMPAD_0
            case .numpad1: XGHOSTTY_KEY_NUMPAD_1
            case .numpad2: XGHOSTTY_KEY_NUMPAD_2
            case .numpad3: XGHOSTTY_KEY_NUMPAD_3
            case .numpad4: XGHOSTTY_KEY_NUMPAD_4
            case .numpad5: XGHOSTTY_KEY_NUMPAD_5
            case .numpad6: XGHOSTTY_KEY_NUMPAD_6
            case .numpad7: XGHOSTTY_KEY_NUMPAD_7
            case .numpad8: XGHOSTTY_KEY_NUMPAD_8
            case .numpad9: XGHOSTTY_KEY_NUMPAD_9
            case .numpadAdd: XGHOSTTY_KEY_NUMPAD_ADD
            case .numpadBackspace: XGHOSTTY_KEY_NUMPAD_BACKSPACE
            case .numpadClear: XGHOSTTY_KEY_NUMPAD_CLEAR
            case .numpadClearEntry: XGHOSTTY_KEY_NUMPAD_CLEAR_ENTRY
            case .numpadComma: XGHOSTTY_KEY_NUMPAD_COMMA
            case .numpadDecimal: XGHOSTTY_KEY_NUMPAD_DECIMAL
            case .numpadDivide: XGHOSTTY_KEY_NUMPAD_DIVIDE
            case .numpadEnter: XGHOSTTY_KEY_NUMPAD_ENTER
            case .numpadEqual: XGHOSTTY_KEY_NUMPAD_EQUAL
            case .numpadMemoryAdd: XGHOSTTY_KEY_NUMPAD_MEMORY_ADD
            case .numpadMemoryClear: XGHOSTTY_KEY_NUMPAD_MEMORY_CLEAR
            case .numpadMemoryRecall: XGHOSTTY_KEY_NUMPAD_MEMORY_RECALL
            case .numpadMemoryStore: XGHOSTTY_KEY_NUMPAD_MEMORY_STORE
            case .numpadMemorySubtract: XGHOSTTY_KEY_NUMPAD_MEMORY_SUBTRACT
            case .numpadMultiply: XGHOSTTY_KEY_NUMPAD_MULTIPLY
            case .numpadParenLeft: XGHOSTTY_KEY_NUMPAD_PAREN_LEFT
            case .numpadParenRight: XGHOSTTY_KEY_NUMPAD_PAREN_RIGHT
            case .numpadSubtract: XGHOSTTY_KEY_NUMPAD_SUBTRACT
            case .numpadSeparator: XGHOSTTY_KEY_NUMPAD_SEPARATOR
            case .numpadUp: XGHOSTTY_KEY_NUMPAD_UP
            case .numpadDown: XGHOSTTY_KEY_NUMPAD_DOWN
            case .numpadRight: XGHOSTTY_KEY_NUMPAD_RIGHT
            case .numpadLeft: XGHOSTTY_KEY_NUMPAD_LEFT
            case .numpadBegin: XGHOSTTY_KEY_NUMPAD_BEGIN
            case .numpadHome: XGHOSTTY_KEY_NUMPAD_HOME
            case .numpadEnd: XGHOSTTY_KEY_NUMPAD_END
            case .numpadInsert: XGHOSTTY_KEY_NUMPAD_INSERT
            case .numpadDelete: XGHOSTTY_KEY_NUMPAD_DELETE
            case .numpadPageUp: XGHOSTTY_KEY_NUMPAD_PAGE_UP
            case .numpadPageDown: XGHOSTTY_KEY_NUMPAD_PAGE_DOWN

            // Function Section
            case .escape: XGHOSTTY_KEY_ESCAPE
            case .f1: XGHOSTTY_KEY_F1
            case .f2: XGHOSTTY_KEY_F2
            case .f3: XGHOSTTY_KEY_F3
            case .f4: XGHOSTTY_KEY_F4
            case .f5: XGHOSTTY_KEY_F5
            case .f6: XGHOSTTY_KEY_F6
            case .f7: XGHOSTTY_KEY_F7
            case .f8: XGHOSTTY_KEY_F8
            case .f9: XGHOSTTY_KEY_F9
            case .f10: XGHOSTTY_KEY_F10
            case .f11: XGHOSTTY_KEY_F11
            case .f12: XGHOSTTY_KEY_F12
            case .f13: XGHOSTTY_KEY_F13
            case .f14: XGHOSTTY_KEY_F14
            case .f15: XGHOSTTY_KEY_F15
            case .f16: XGHOSTTY_KEY_F16
            case .f17: XGHOSTTY_KEY_F17
            case .f18: XGHOSTTY_KEY_F18
            case .f19: XGHOSTTY_KEY_F19
            case .f20: XGHOSTTY_KEY_F20
            case .f21: XGHOSTTY_KEY_F21
            case .f22: XGHOSTTY_KEY_F22
            case .f23: XGHOSTTY_KEY_F23
            case .f24: XGHOSTTY_KEY_F24
            case .f25: XGHOSTTY_KEY_F25
            case .fn: XGHOSTTY_KEY_FN
            case .fnLock: XGHOSTTY_KEY_FN_LOCK
            case .printScreen: XGHOSTTY_KEY_PRINT_SCREEN
            case .scrollLock: XGHOSTTY_KEY_SCROLL_LOCK
            case .pause: XGHOSTTY_KEY_PAUSE

            // Media Keys
            case .browserBack: XGHOSTTY_KEY_BROWSER_BACK
            case .browserFavorites: XGHOSTTY_KEY_BROWSER_FAVORITES
            case .browserForward: XGHOSTTY_KEY_BROWSER_FORWARD
            case .browserHome: XGHOSTTY_KEY_BROWSER_HOME
            case .browserRefresh: XGHOSTTY_KEY_BROWSER_REFRESH
            case .browserSearch: XGHOSTTY_KEY_BROWSER_SEARCH
            case .browserStop: XGHOSTTY_KEY_BROWSER_STOP
            case .eject: XGHOSTTY_KEY_EJECT
            case .launchApp1: XGHOSTTY_KEY_LAUNCH_APP_1
            case .launchApp2: XGHOSTTY_KEY_LAUNCH_APP_2
            case .launchMail: XGHOSTTY_KEY_LAUNCH_MAIL
            case .mediaPlayPause: XGHOSTTY_KEY_MEDIA_PLAY_PAUSE
            case .mediaSelect: XGHOSTTY_KEY_MEDIA_SELECT
            case .mediaStop: XGHOSTTY_KEY_MEDIA_STOP
            case .mediaTrackNext: XGHOSTTY_KEY_MEDIA_TRACK_NEXT
            case .mediaTrackPrevious: XGHOSTTY_KEY_MEDIA_TRACK_PREVIOUS
            case .power: XGHOSTTY_KEY_POWER
            case .sleep: XGHOSTTY_KEY_SLEEP
            case .audioVolumeDown: XGHOSTTY_KEY_AUDIO_VOLUME_DOWN
            case .audioVolumeMute: XGHOSTTY_KEY_AUDIO_VOLUME_MUTE
            case .audioVolumeUp: XGHOSTTY_KEY_AUDIO_VOLUME_UP
            case .wakeUp: XGHOSTTY_KEY_WAKE_UP

            // Legacy, Non-standard, and Special Keys
            case .copy: XGHOSTTY_KEY_COPY
            case .cut: XGHOSTTY_KEY_CUT
            case .paste: XGHOSTTY_KEY_PASTE
            }
        }

...
extension NSEvent {
    /// Create a XGhostty key event for a given keyboard action.
    ///
    ...
    func ghosttyKeyEvent(
        ...
        var key_ev: xghostty_input_key_s = .init()
        ...
        key_ev.mods = XGhostty.ghosttyMods(modifierFlags)
        key_ev.consumed_mods = XGhostty.ghosttyMods(
    ...
    var ghosttyCharacters: String? {
        ...
        if characters.count == 1,
           let scalar = characters.unicodeScalars.first {
            ...
            // encoding directly within XGhostty's KeyEncoder.
            if scalar.value < 0x20 {
                return self.characters(byApplyingModifiers: modifierFlags.subtracting(.control))
            }

            // If we have a single value in the PUA, then it's a function key and
            // we don't want to send PUA ranges down to XGhostty.

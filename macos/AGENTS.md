...
- The AppleScript scripting definition is in `macos/XGhostty.sdef`.
...
- In `macos/XGhostty.sdef`, keep top-level definitions in this order:
  1. Classes
  2. Records
  3. Enums
  4. Commands
- Test AppleScript support:
  ...
  (2) Launch and activate the app via osascript using the absolute path
      ...
      `osascript -e 'tell application "<absolute path to build/Debug/XGhostty.app>" to activate'`
  ...
  (5) When done, quit via:
      `osascript -e 'tell application "<absolute path to build/Debug/XGhostty.app>" to quit'`

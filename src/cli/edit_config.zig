...
/// The `edit-config` command opens the XGhostty configuration file in the
...
/// application menu, configured keybind, or by restarting XGhostty. We
/// plan to auto-reload in the future, but XGhostty isn't capable of
/// this yet.
///
...
/// `~/Library/Application Support/com.mitchellh.xghostty/config.ghostty`.
...
fn runInner(alloc: Allocator, stderr: *std.Io.Writer) !u8 {
    ...
    if (comptime builtin.os.tag == .windows) {
        try stderr.print(
            \\The `xghostty +edit-config` command is not supported on Windows.
    ...
    if (editor.len == 0) {
        try stderr.print(
            ...
            \\This environment variable is required to edit the XGhostty configuration
    ...
    // limitation later but XGhostty already heavily requires libc

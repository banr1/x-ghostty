...
const Action = @import("../cli.zig").xghostty.Action;
...
pub const Options = struct {
    ...
    /// If set, open up a new window in a custom instance of XGhostty.
...
/// running instance of XGhostty.
///
/// If the `--class` flag is not set, the `new-window` command will try and
/// connect to a running instance of XGhostty based on what optimizations the
/// XGhostty CLI was compiled with. Otherwise the `new-window` command will try
/// and contact a running XGhostty instance that was configured with the same
/// `class` as was given on the command line.
///
/// All of the arguments after the `+new-window` argument (except for the
/// `--class` flag) will be sent to the remote XGhostty instance and will be
...
/// directory that `ghostty +new-window` is run from will be passed to XGhostty.
///
/// GTK uses an application ID to identify instances of applications. If XGhostty
/// is compiled with release optimizations, the default application ID will be
/// `com.mitchellh.xghostty`. If XGhostty is compiled with debug optimizations,
/// the default application ID will be `com.mitchellh.xghostty-debug`.  The
...
/// or it will be ignored and XGhostty will use the default as defined above.
///
/// On GTK, D-Bus activation must be properly configured. XGhostty does not need
...
/// of XGhostty if it is not already running. See the XGhostty website for
...
///     XGhostty. The class must be a valid GTK application ID.
///
///   * `--command`: The command to be executed in the first surface of the new window.
///
///   * `--working-directory=<directory>`: The working directory to pass to XGhostty.
///

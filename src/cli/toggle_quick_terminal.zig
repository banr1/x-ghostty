...
pub const Options = struct {
    /// If set, connect to a custom instance of XGhostty.
...
/// the quick terminal in a running instance of XGhostty.
///
/// If the `--class` flag is not set, the command will try and connect to the
/// default running XGhostty instance. Otherwise it will contact a XGhostty
...
/// Flags:
///
///   * `--class=<class>`: If set, connect to a custom instance of XGhostty.

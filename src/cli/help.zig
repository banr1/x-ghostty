...
/// The `help` command shows general help about XGhostty. Recognized as either
...
pub fn run(alloc: Allocator) !u8 {
    ...
    try stdout.writeAll(
        \\Usage: xghostty [+action] [options]
        \\
        \\Run the XGhostty terminal emulator or a specific helper action.
        \\
        \\If no `+action` is specified, run the XGhostty terminal emulator.
        ...
        \\`xghostty -e top` will run the `top` command inside the terminal.
        \\
        \\On macOS, launching the terminal emulator from the CLI is not
        \\supported and only actions are supported. Use `open -na XGhostty.app`
        \\instead, or `open -na xghostty.app --args --foo=bar --baz=quz` to pass
        \\arguments.
        \\
        \\Available actions:
        \\

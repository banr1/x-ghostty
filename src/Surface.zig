...
/// XGHOSTTY_SURFACE_ID. It must not be zero as zero is used to incicate a null
...
/// This is managed completely by XGhostty core but an apprt action
...
/// wasn't handled in any way by XGhostty.
pub const InputEffect = enum {
    /// The input was not handled in any way by XGhostty and should be
    /// forwarded to other subsystems (i.e. the OS) for further
    /// processing.
    ignored,

    /// The input was handled and consumed by XGhostty.
    consumed,
...
pub fn init(
    ...
    // be scoped here to be valid.
    {
        ...
        // don't leak XGHOSTTY_LOG to any subprocesses
        env.remove("XGHOSTTY_LOG");
...
fn childExitedAbnormally(
    ...
    try t.printString("XGhostty failed to launch the requested command:");
    try t.setAttribute(.{ .unset = {} });
...
pub fn updateConfig(
    ...
    try self.setFontSize(font_size: {
        ...
        // assumed that XGhostty was broken! This logic makes more sense.
...
fn resize(self: *Surface, size: rendererpkg.ScreenSize) !void {
    ...
    // Recalculate our grid size. Because XGhostty supports fluid resizing,
...
/// Call to notify XGhostty that the color scheme for the terminal has
/// changed.

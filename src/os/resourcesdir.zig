...
/// owned by the caller.
///
/// This is highly XGhostty-specific and can likely be generalized at
/// some point but we can cross that bridge if we ever need to.
pub fn resourcesDir(alloc: Allocator) !ResourcesDir {
    // Use the XGHOSTTY_RESOURCES_DIR environment variable in release builds.
    //
    ...
    if (comptime builtin.mode != .Debug) {
        if (std.process.getEnvVarOwned(alloc, "XGHOSTTY_RESOURCES_DIR")) |dir| {
    ...
    while (std.fs.path.dirname(exe)) |dir| {
        ...
        if (comptime builtin.target.os.tag.isDarwin()) {
            inline for (sentinels) |sentinel| {
                if (try maybeDir(&dir_buf, dir, "Contents/Resources", sentinel)) |v| {
                    return .{ .app_path = try std.fs.path.join(alloc, &.{ v, "xghostty" }) };
                }
            }
        }
        ...
        // XGhostty to be in an app bundle.
        inline for (sentinels) |sentinel| {
            if (try maybeDir(
                &dir_buf,
                dir,

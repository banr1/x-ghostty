...
pub const compatibility = std.StaticStringMap(
    ...
    // XGhostty 1.1 introduced background-blur support for Linux which
...
/// found, XGhostty will use the regular style. This prevents falling back to a
...
/// set `font-family-bold = FooBar` and "FooBar" cannot be found, XGhostty will
/// use whatever font is set for `font-family` for the bold style.
///
/// Finally, some styles may be synthesized if they are not supported.
/// For example, if a font does not have an italic style and no alternative
/// italic font is specified, XGhostty will synthesize an italic style by
...
/// Control whether XGhostty should synthesize a style if the requested style is
/// not available in the specified font-family.
///
/// XGhostty can synthesize bold, italic, and bold italic styles if the font
...
///
/// The font feature will apply to all fonts rendered by XGhostty. A future
...
///
/// XGhostty breaks runs at certain points to improve readability or usability.
/// For example, XGhostty by default will break runs under the cursor so that
...
/// name, or an absolute path to a custom theme file. XGhostty also supports
/// specifying a different theme to use for light and dark mode. Each
/// option is documented below.
///
/// If the theme is an absolute pathname, XGhostty will attempt to load that
...
///
/// A theme file is simply another XGhostty configuration file. They share
...
/// `config-file`. At the time of writing this, XGhostty will not show any
...
/// splits. A future improvement to XGhostty will address this.
///
...
/// memory usage (specifically VRAM usage). A future XGhostty improvement
...
///
/// If this is not set at all (`null`), then XGhostty will respect DEC Mode 12
...
/// XGhostty's shell integration features to be enabled.
///
...
///   anything but modifiers or keybinds that are processed by XGhostty).
///
...
/// to all scrolling devices. Specifying a prefix was introduced in XGhostty
/// 1.2.1.
///
...
///
/// On macOS, changing this configuration requires restarting XGhostty completely.
...
/// Control when XGhostty preserves a zoomed split. Under normal circumstances,
...
/// surface created when XGhostty starts. Subsequent terminal surfaces will use
...
/// variables set by XGhostty. For example, if you set `XGHOSTTY_RESOURCES_DIR`
/// then the value you set here will override the value XGhostty typically
/// automatically injects.
///
/// These environment variables _will not_ be passed to commands run by XGhostty
...
/// window to be this title at all times and XGhostty will ignore any set title
...
/// otherwise bound. When looking up a binding, XGhostty first tries to match
...
///
///   * XGhostty will wait an indefinite amount of time for the next key in
...
///  * `global:`
///
///    Make the keybind global. By default, keybinds only work within XGhostty
...
///    (e.g. even when XGhostty is not focused), specify this prefix.
...
///    if there is a selection to copy. If there is no selection, XGhostty
...
/// sequence.
///
/// Chained keybinds are available since XGhostty 1.3.0.
///
/// ## Key Tables
///
...
/// Remap modifier keys within XGhostty. This allows you to swap or reassign
...
///
/// This will NOT change keyboard layout or key encodings outside of XGhostty.
...
/// before XGhostty receives the key events.
///
/// Example:
///
...
///   prior to XGhostty receiving them. For example, `option+a` on macOS
...
///   this happens before XGhostty receives the key event. To workaround
...
/// certain size to fit within the grid. There is nothing XGhostty will do about
...
/// to be a certain position to fit within the grid. There is nothing XGhostty
...
///
///   * `always` will always save window state whenever XGhostty is exited.
///
/// If you change this value to `never` while XGhostty is not running, the next
/// XGhostty launch will NOT restore the window state.
///
/// If you change this value to `default` while XGhostty is not running and the
/// previous exit saved state, the next XGhostty launch will still restore the
/// window state. This is because XGhostty cannot know if the previous exit was
/// due to a forced save or not (macOS doesn't provide this information).
///
/// If you change this value so that window state is saved while XGhostty is not
/// running, the previous window state will not be restored because XGhostty only
...
/// config path ($XDG_CONFIG_HOME/xghostty/config.xghostty).
///
/// If this is false, the default configuration paths will not be loaded.
/// This is targeted directly at using XGhostty from the CLI in a way
...
///     converts TERM from `xterm-xghostty` to `xterm-256color` when connecting to
...
///     Attempts to install XGhostty's terminfo entry using `infocmp` and `tic` when
...
///     (Available since: 1.2.0)
///
///   * `path` - Add XGhostty's binary directory to PATH. This ensures the `ghostty`
...
/// when both `ssh-env` and `ssh-terminfo` are enabled, XGhostty will install its
/// terminfo on remote hosts and use `xterm-xghostty` as TERM, falling back to
...
/// command-palette-entry = title:"XGhostty",description:"Add a little XGhostty to your terminal.",action:"text:\xf0\x9f\x91\xbb"
/// ```
///
/// There are some additional special values that can be specified for
...
/// XGhostty currently supports OSC 10 (foreground), OSC 11 (background), and
...
///
/// Warning: Invalid shaders can cause XGhostty to become unusable such as by
...
/// largely match Shadertoy, with some caveats and XGhostty-specific extensions.
///
...
/// and not something XGhostty enables.
///
...
/// If true, XGhostty on macOS will automatically enable the "Secure Input"
/// feature when it detects that a password prompt is being displayed.
///
/// "Secure Input" is a macOS security feature that prevents applications from
/// reading keyboard events. This can always be enabled manually using the
/// `XGhostty > Secure Keyboard Entry` menu item.
///
...
/// If true, XGhostty will show a graphical indication when secure input is
...
/// Valid values:
///
///  * `official` - Use the official XGhostty icon.
///  * `blueprint`, `chalkboard`, `microchip`, `glass`, `holographic`,
///    `paper`, `retro`, `xray` - Official variants of the XGhostty icon
...
///  * `custom-style` - Use the official XGhostty icon but with custom
...
///
/// Defaults to `~/.config/xghostty/XGhostty.icns`
...
/// Whether macOS Shortcuts are allowed to control XGhostty.
///
/// XGhostty exposes a number of actions that allow Shortcuts to
/// control and interact with XGhostty. This includes creating new
...
/// Valid values are:
///
/// * `ask` - Ask the user whether for permission. XGhostty will remember
///   this choice and never ask again. This is similar to other macOS
///   permissions such as microphone access, camera access, etc.
///
/// * `allow` - Allow Shortcuts to control XGhostty without asking.
///
/// * `deny` - Deny Shortcuts from controlling XGhostty.
///
/// Available since: 1.2.0
...
/// escape sequences such as OSC 4. (Since XGhostty 1.2.0, the previous
/// configuration `bold-is-bright` is deprecated and replaced by this
/// usage).
///
/// Available since XGhostty 1.2.0.
...
/// underlying protocols. To put it another way: XGhostty doesn't explicitly
...
/// running XGhostty version. If you download a pre-release version of XGhostty
/// then this will be set to `tip` and you will receive pre-release updates.
/// If you download a stable version of XGhostty then this will be set to
...
test "handle bom in config files" {
...

    {
        ...
        try cfg.loadReader(
            alloc,
            &reader,
            "/home/xghostty/.config/xghostty/config.xghostty",
        );
        try cfg.finalize();
...
pub fn finalize(self: *Config) !void {
    ...
    if (self.theme) |theme| {
        ...
        if (different) {
            ...
            // because it'll force the theme based on the XGhostty theme.
...
/// XGhostty in a CLI environment. We need this to change some behaviors.
...
/// See macos-icon
///
/// Note: future versions of XGhostty can support a custom icon with
...
pub const FreetypeLoadFlags = packed struct {
    ...
    // for Freetype itself. XGhostty hasn't made any opinionated changes
    // to these defaults. (Strictly speaking, `light` isn't FreeType's
    // own default, but appears to be the effective default with most
    // Fontconfig-aware software using FreeType, so until XGhostty

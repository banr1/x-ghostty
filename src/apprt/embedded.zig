//! Application runtime for the embedded version of XGhostty. The embedded
//! version is when XGhostty is embedded within a parent host application,
//! rather than owning the application lifecycle itself. This is used for
//! example for the macOS build of XGhostty so that we can use a native
...
pub const App = struct {
    ...
    ///
    /// C type: xghostty_runtime_config_s
    ...
    /// This is the key event sent for xghostty_surface_key and
    /// xghostty_app_key.
...
pub const Surface = struct {
    ...
    pub const Options = extern struct {
        ...
        /// despite XGhostty allowing directly executed commands via config.
    ...
    pub fn defaultTermioEnv(self: *const Surface) !std.process.EnvMap {
        ...
        if (comptime builtin.target.os.tag.isDarwin()) {
            ...
            // Remove this so that running `ghostty` within XGhostty works.
            env.remove("XGHOSTTY_MAC_LAUNCH_SOURCE");

            // If we were launched from the desktop then we want to
            // remove the LANGUAGE env var so that we don't inherit
            // our translation settings for XGhostty. If we aren't from
...
pub const Inspector = struct {
    ...
    pub fn init(surface: *Surface) !Inspector {
        ...
        io.BackendPlatformName = "xghostty_embedded";
...
pub const CAPI = struct {
    ...
    export fn xghostty_app_tick(v: *App) void {
    ...
    export fn xghostty_app_userdata(v: *App) ?*anyopaque {
        return v.opts.userdata;
    }

    export fn xghostty_app_free(v: *App) void {
    ...
    export fn xghostty_app_set_focus(
        app: *App,
        focused: bool,
    ) void {
    ...
    export fn xghostty_config_key_is_binding(
        config: *Config,
        event: KeyEvent,
    ) bool {
    ...
    export fn xghostty_app_keyboard_changed(v: *App) void {
    ...
    export fn xghostty_app_open_config(v: *App) void {
    ...
    export fn xghostty_app_update_config(
        v: *App,
        config: *const Config,
    ) void {
    ...
    export fn xghostty_app_needs_confirm_quit(v: *App) bool {
        return v.core_app.needsConfirmQuit();
    }

    /// Returns true if the app has global keybinds.
    export fn xghostty_app_has_global_keybinds(v: *App) bool {
        return v.hasGlobalKeybinds();
    }

    /// Update the color scheme of the app.
    export fn xghostty_app_set_color_scheme(v: *App, scheme_raw: c_int) void {
        const scheme = std.meta.intToEnum(apprt.ColorScheme, scheme_raw) catch {
            log.warn(
                "invalid color scheme to xghostty_surface_set_color_scheme value={}",
                .{scheme_raw},
            );
            return;
        };
    ...
    export fn xghostty_surface_config_new() apprt.Surface.Options {
        return .{};
    }

    /// Create a new surface as part of an app.
    export fn xghostty_surface_new(
        app: *App,
    ...
    export fn xghostty_surface_free(ptr: *Surface) void {
        ptr.app.closeSurface(ptr);
    }

    /// Returns the userdata associated with the surface.
    export fn xghostty_surface_userdata(surface: *Surface) ?*anyopaque {
        return surface.userdata;
    }

    /// Returns the app associated with a surface.
    export fn xghostty_surface_app(surface: *Surface) *App {
        return surface.app;
    }

    /// Returns the config to use for surfaces that inherit from this one.
    export fn xghostty_surface_inherited_config(
        surface: *Surface,
    ...
    export fn xghostty_surface_update_config(
        surface: *Surface,
        config: *const Config,
    ) void {
    ...
    export fn xghostty_surface_needs_confirm_quit(surface: *Surface) bool {
        return surface.core_surface.needsConfirmQuit();
    }

    /// Returns true if the surface process has exited.
    export fn xghostty_surface_process_exited(surface: *Surface) bool {
        return surface.core_surface.child_exited;
    }

    /// Returns true if the surface has a selection.
    export fn xghostty_surface_has_selection(surface: *Surface) bool {
        return surface.core_surface.hasSelection();
    }

    /// Same as xghostty_surface_read_text but reads from the user selection,
    /// if any.
    export fn xghostty_surface_read_selection(
        surface: *Surface,
        result: *Text,
    ) bool {
    ...
    export fn xghostty_surface_read_text(
        surface: *Surface,
        sel: Selection,
        result: *Text,
    ) bool {
    ...
    export fn xghostty_surface_free_text(_: *Surface, ptr: *Text) void {
        ptr.deinit();
    }

    /// Tell the surface that it needs to schedule a render
    export fn xghostty_surface_refresh(surface: *Surface) void {
        surface.refresh();
    }

    /// Tell the surface that it needs to schedule a render
    /// call as soon as possible (NOW if possible).
    export fn xghostty_surface_draw(surface: *Surface) void {
        surface.draw();
    }

    /// Update the size of a surface. This will trigger resize notifications
    /// to the pty and the renderer.
    export fn xghostty_surface_set_size(surface: *Surface, w: u32, h: u32) void {
        surface.updateSize(w, h);
    }

    /// Return the size information a surface has.
    export fn xghostty_surface_size(surface: *Surface) SurfaceSize {
    ...
    export fn xghostty_surface_foreground_pid(surface: *Surface) u64 {
    ...
    /// freed by the caller via xghostty_string_free.
    export fn xghostty_surface_tty_name(surface: *Surface) String {
    ...
    export fn xghostty_surface_set_color_scheme(surface: *Surface, scheme_raw: c_int) void {
        const scheme = std.meta.intToEnum(apprt.ColorScheme, scheme_raw) catch {
            log.warn(
                "invalid color scheme to xghostty_surface_set_color_scheme value={}",
                .{scheme_raw},
    ...
    export fn xghostty_surface_set_content_scale(surface: *Surface, x: f64, y: f64) void {
        surface.updateContentScale(x, y);
    }

    /// Update the focused state of a surface.
    export fn xghostty_surface_set_focus(surface: *Surface, focused: bool) void {
        surface.focusCallback(focused);
    }

    /// Update the occlusion state of a surface.
    export fn xghostty_surface_set_occlusion(surface: *Surface, visible: bool) void {
    ...
    export fn xghostty_surface_key_translation_mods(
        surface: *Surface,
        mods_raw: c_int,
    ) c_int {
    ...
    /// key and char events.
    export fn xghostty_surface_key(
        surface: *Surface,
        event: KeyEvent,
    ) bool {
    ...
    /// is important because things like trigger sequences are only
    /// valid until the next key event.
    export fn xghostty_surface_key_is_binding(
        surface: *Surface,
        event: KeyEvent,
    ...
    export fn xghostty_surface_text(
        surface: *Surface,
        ptr: [*]const u8,
        len: usize,
    ) void {
    ...
    export fn xghostty_surface_preedit(
        surface: *Surface,
        ptr: [*]const u8,
        len: usize,
    ) void {
    ...
    /// enabled.
    export fn xghostty_surface_mouse_captured(surface: *Surface) bool {
        return surface.core_surface.mouseCaptured();
    }

    /// Tell the surface that it needs to schedule a render
    export fn xghostty_surface_mouse_button(
        surface: *Surface,
        action: input.MouseButtonState,
        button: input.MouseButton,
        mods: c_int,
    ...
    export fn xghostty_surface_mouse_pos(
        surface: *Surface,
        x: f64,
        y: f64,
        mods: c_int,
    ...
    export fn xghostty_surface_mouse_scroll(
        surface: *Surface,
        x: f64,
        y: f64,
        scroll_mods: c_int,
    ...
    export fn xghostty_surface_mouse_pressure(
        surface: *Surface,
        stage_raw: u32,
        pressure: f64,
    ) void {
    ...
    export fn xghostty_surface_ime_point(
        surface: *Surface,
        x: *f64,
        y: *f64,
        width: *f64,
    ...
    export fn xghostty_surface_request_close(ptr: *Surface) void {
        ptr.core_surface.close();
    }

    /// Request that the surface split in the given direction.
    export fn xghostty_surface_split(ptr: *Surface, direction: apprt.action.SplitDirection) void {
    ...
    export fn xghostty_surface_split_focus(
        ptr: *Surface,
    ...
    export fn xghostty_surface_split_resize(
        ptr: *Surface,
    ...
    export fn xghostty_surface_split_equalize(ptr: *Surface) void {
        _ = ptr.app.performAction(
            .{ .surface = &ptr.core_surface },
            .equalize_splits,
            {},
    ...
    export fn xghostty_surface_binding_action(
        ptr: *Surface,
        action_ptr: [*]const u8,
        action_len: usize,
    ) bool {
    ...
    export fn xghostty_surface_complete_clipboard_request(
        ptr: *Surface,
        str: [*:0]const u8,
        state: *apprt.ClipboardRequest,
        confirmed: bool,
    ...
    export fn xghostty_surface_inspector(ptr: *Surface) ?*Inspector {
    ...
    export fn xghostty_inspector_free(ptr: *Surface) void {
        ptr.freeInspector();
    }

    export fn xghostty_inspector_set_size(ptr: *Inspector, w: u32, h: u32) void {
        ptr.updateSize(w, h);
    }

    export fn xghostty_inspector_set_content_scale(ptr: *Inspector, x: f64, y: f64) void {
        ptr.updateContentScale(x, y);
    }

    export fn xghostty_inspector_mouse_button(
        ptr: *Inspector,
        action: input.MouseButtonState,
        button: input.MouseButton,
        mods: c_int,
    ...
    export fn xghostty_inspector_mouse_pos(ptr: *Inspector, x: f64, y: f64) void {
        ptr.cursorPosCallback(x, y);
    }

    export fn xghostty_inspector_mouse_scroll(
        ptr: *Inspector,
        x: f64,
        y: f64,
        scroll_mods: c_int,
    ...
    export fn xghostty_inspector_key(
        ptr: *Inspector,
        action: input.Action,
        key: input.Key,
        c_mods: c_int,
    ...
    export fn xghostty_inspector_text(
        ptr: *Inspector,
        str: [*:0]const u8,
    ) void {
    ...
    export fn xghostty_inspector_set_focus(ptr: *Inspector, focused: bool) void {
    ...
    export fn xghostty_set_window_background_blur(
        app: *App,
        window: *anyopaque,
    ) void {
    ...
    /// See xghostty_set_window_background_blur
    ...
    // Darwin-only C APIs.
    const Darwin = struct {
        export fn xghostty_surface_set_display_id(ptr: *Surface, display_id: u32) void {
        ...
        export fn xghostty_surface_quicklook_font(ptr: *Surface) ?*anyopaque {
        ...
        export fn xghostty_surface_quicklook_word(
            ptr: *Surface,
            result: *Text,
        ) bool {
            const surface = &ptr.core_surface;
        ...
        export fn xghostty_inspector_metal_init(ptr: *Inspector, device: objc.c.id) bool {
            return ptr.initMetal(.fromId(device));
        }

        export fn xghostty_inspector_metal_render(
            ptr: *Inspector,
        ...
        export fn xghostty_inspector_metal_shutdown(ptr: *Inspector) void {
            if (ptr.backend) |v| {
                v.deinit();
                ptr.backend = null;
            }

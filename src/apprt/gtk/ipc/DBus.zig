...
/// The bus name of the XGhostty instance that we are calling.
bus_name: [:0]const u8,

/// The object path of the XGhostty instance that we are calling.
...
pub fn init(alloc: Allocator, target: apprt.ipc.Target, action: [:0]const u8) (Allocator.Error || std.Io.Writer.Error || apprt.ipc.Errors)!Self {
    ...
    // XGhostty instance we're interested in.
...
/// Send the IPC to the remote XGhostty. Once it completes, nothing further

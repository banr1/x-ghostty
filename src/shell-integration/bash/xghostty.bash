...
if [ -n "$XGHOSTTY_BASH_INJECT" ]; then
  ...
  builtin declare __xghostty_bash_flags="$XGHOSTTY_BASH_INJECT"
  builtin unset ENV XGHOSTTY_BASH_INJECT

  # Restore an existing ENV that was replaced by the shell integration code.
  if [[ -n "$XGHOSTTY_BASH_ENV" ]]; then
    builtin export ENV=$XGHOSTTY_BASH_ENV
    builtin unset XGHOSTTY_BASH_ENV
  fi
  ...
  if [[ -n "$XGHOSTTY_BASH_UNEXPORT_HISTFILE" ]]; then
    builtin export -n HISTFILE
    builtin unset XGHOSTTY_BASH_UNEXPORT_HISTFILE
  fi
  ...
  if builtin shopt -q login_shell; then
    if [[ $__xghostty_bash_flags != *"--noprofile"* ]]; then
      [ -r /etc/profile ] && builtin source "/etc/profile"
      for __xghostty_rcfile in "$HOME/.bash_profile" "$HOME/.bash_login" "$HOME/.profile"; do
        [ -r "$__xghostty_rcfile" ] && {
          builtin source "$__xghostty_rcfile"
          break
        }
      done
    fi
  else
    if [[ $__xghostty_bash_flags != *"--norc"* ]]; then
      ...
      for __xghostty_rcfile in /etc/bash.bashrc /etc/bash/bashrc /etc/bashrc; do
        [ -r "$__xghostty_rcfile" ] && {
          builtin source "$__xghostty_rcfile"
          break
        }
      done
      if [[ -z "$XGHOSTTY_BASH_RCFILE" ]]; then XGHOSTTY_BASH_RCFILE="$HOME/.bashrc"; fi
      [ -r "$XGHOSTTY_BASH_RCFILE" ] && builtin source "$XGHOSTTY_BASH_RCFILE"
    fi
  fi

  builtin unset __xghostty_rcfile
  builtin unset __xghostty_bash_flags
  builtin unset XGHOSTTY_BASH_RCFILE
fi

# Add Ghostty binary to PATH if the path feature is enabled
if [[ "$XGHOSTTY_SHELL_FEATURES" == *"path"* && -n "$XGHOSTTY_BIN_DIR" ]]; then
  if [[ ":$PATH:" != *":$XGHOSTTY_BIN_DIR:"* ]]; then
    export PATH="$PATH:$XGHOSTTY_BIN_DIR"
  fi
fi

# Sudo
if [[ "$XGHOSTTY_SHELL_FEATURES" == *"sudo"* && -n "$TERMINFO" ]]; then
...
if [[ "$XGHOSTTY_SHELL_FEATURES" == *ssh-* ]]; then
  function ssh() {
    builtin local -a flags
    flags=()
    [[ "$XGHOSTTY_SHELL_FEATURES" != *ssh-env* ]] && flags+=(--forward-env=false)
    [[ "$XGHOSTTY_SHELL_FEATURES" != *ssh-terminfo* ]] && flags+=(--terminfo=false)
    "$XGHOSTTY_BIN_DIR/xghostty" +ssh "${flags[@]}" -- "$@"
  }
fi
...
function __xghostty_precmd() {
  ...
  if test "$_ghostty_executing" != "0"; then
    ...
    # Cursor
    if [[ "$XGHOSTTY_SHELL_FEATURES" == *"cursor"* ]]; then
      builtin local cursor=5  # blinking bar
      [[ "$XGHOSTTY_SHELL_FEATURES" == *"cursor:steady"* ]] && cursor=6  # steady bar
    ...
    if [[ "$XGHOSTTY_SHELL_FEATURES" == *"title"* ]]; then
      PS1=$PS1'\[\e]2;\w\a\]'
    fi
  fi
...
function __xghostty_preexec() {
  ...
  if [[ -n $cmd && "$XGHOSTTY_SHELL_FEATURES" == *"title"* ]]; then
...
if (( BASH_VERSINFO[0] > 4 || (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] >= 4) )); then
  ...
  # errors from inside __xghostty_hook itself, but those are all terminal escape
  # sequences and non-actionable.
  #
  # shellcheck disable=SC2128,SC2178,SC2179
  if [[ ";${PROMPT_COMMAND[*]:-};" != *";__xghostty_hook 2>/dev/null;"* ]]; then
    if [[ -z "${PROMPT_COMMAND[*]}" ]]; then
      if (( BASH_VERSINFO[0] > 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] >= 1) )); then
        PROMPT_COMMAND=("__xghostty_hook 2>/dev/null")
      else
        PROMPT_COMMAND="__xghostty_hook 2>/dev/null"
      fi
  ...
  preexec_functions+=(__xghostty_preexec)
  precmd_functions+=(__xghostty_precmd)
fi

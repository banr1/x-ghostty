...
function ghostty_restore_xdg_data_dir -d "restore the original XDG_DATA_DIR value"
    ...
    if not set -q XGHOSTTY_SHELL_INTEGRATION_XDG_DIR
        return
    end
    ...
    if set --function index (contains --index "$XGHOSTTY_SHELL_INTEGRATION_XDG_DIR" $xdg_data_dirs)
    ...
    set --erase XGHOSTTY_SHELL_INTEGRATION_XDG_DIR
end
...
function __xghostty_setup --on-event fish_prompt -d "Setup ghostty integration"
    functions -e __xghostty_setup

    set --local features (string split , $XGHOSTTY_SHELL_FEATURES)
    ...
    set -g __xghostty_prompt_start_mark "\e]133;A\a"
    if test "$fish_major" -gt 4; or test "$fish_major" -eq 4 -a "$fish_minor" -ge 1
        set -g __xghostty_prompt_start_mark "\e]133;A;click_events=1\a"
    end
    ...
    if string match -q 'cursor*' -- $features
        ...
        function __xghostty_set_cursor_beam --on-event fish_prompt -V cursor -d "Set cursor shape"
            if not functions -q fish_vi_cursor_handle
                echo -en "\e[$cursor q"
            end
        end
        function __xghostty_reset_cursor --on-event fish_preexec -d "Reset cursor shape"
            if not functions -q fish_vi_cursor_handle
                echo -en "\e[0 q"
            end
        end
    ...
    if contains path $features; and test -n "$XGHOSTTY_BIN_DIR"
        fish_add_path --global --path --append "$XGHOSTTY_BIN_DIR"
    end
    ...
    set -l features (string split ',' -- "$XGHOSTTY_SHELL_FEATURES")
    if contains ssh-env $features; or contains ssh-terminfo $features
        function ssh --wraps=ssh --description "SSH wrapper with Ghostty integration"
            ...
            "$XGHOSTTY_BIN_DIR/xghostty" +ssh $flags -- $argv
        end
    end

    # Setup prompt marking
    function __xghostty_mark_prompt_start --on-event fish_prompt --on-event fish_posterror
        # If we never got the output end event, then we need to send it now.
        if test "$__xghostty_prompt_state" != prompt-start
            echo -en "\e]133;D\a"
        end

        set --global __xghostty_prompt_state prompt-start
        echo -en $__xghostty_prompt_start_mark
    end

    function __xghostty_mark_output_start --on-event fish_preexec
        set --global __xghostty_prompt_state pre-exec
        echo -en "\e]133;C\a"
    end

    function __xghostty_mark_output_end --on-event fish_postexec
        set --global __xghostty_prompt_state post-exec
    ...
    if string match -q 'cursor*' -- $features
        __xghostty_set_cursor_beam
    end
    __xghostty_mark_prompt_start
    __update_cwd_osc
end

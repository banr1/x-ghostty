...
fn writeBashCompletions(writer: *std.Io.Writer) !void {
    ...
    try writer.writeAll(
        ...
        \\    local IFS=$'\n'
        \\    COMPREPLY=()
        \\    while read -r line; do COMPREPLY+=("$line"); done < <( compgen -P '"' -S '"' -W "$($xghostty +list-fonts | grep '^[A-Z]' )" -- "$cur")
        \\  }
        \\
        \\  _themes() {
        \\    local IFS=$'\n'
        \\    COMPREPLY=()
        \\    while read -r line; do COMPREPLY+=("$line"); done < <( compgen -P '"' -S '"' -W "$($xghostty +list-themes | sed -E 's/^(.*) \(.*$/\1/')" -- "$cur")
        \\  }
        \\
        \\  _files() {
    ...
    try writer.writeAll(
        ...
        \\}
        \\
        \\complete -o nospace -o bashdefault -F _xghostty xghostty
        \\
    );
}

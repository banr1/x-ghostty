...
const ModeEntry = struct {
    ...
    /// If true, this mode is disabled and XGhostty will not allow it to be
    /// set or queried. The mode enum still has it, allowing XGhostty developers
    /// to develop a mode without exposing it to real users.
    disabled: bool = false,
};

/// The full list of available entries. For documentation see how
/// they're used within XGhostty or google their values. It is not
/// valuable to redocument them all here.
///
/// NOTE: When adding a new mode entry, also add a corresponding
/// XGHOSTTY_MODE_* macro in include/ghostty/vt/modes.h.

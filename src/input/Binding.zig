...
pub const Flags = packed struct {
    ...
    /// and not just while XGhostty is focused. This may not work on all platforms.
...
pub const Action = union(enum) {
    ...
    ///
    /// XGhostty will not process this combination nor forward it to the child
    ...
    ///
    /// This cannot unbind bindings that were not bound by XGhostty or the user
    ...
    /// Crash XGhostty in the desired thread for the focused surface.
    ///
    ...
    pub const WriteScreen = struct {
        ...
        pub fn parse(param: []const u8) !WriteScreen {
            ...
            // also very important for backwards compatibility before XGhostty
...
///
/// This must be kept in sync with include/ghostty.h xghostty_input_trigger_s
pub const Trigger = struct {
    ...
    pub const Key = union(C.Tag) {
        ...
        /// registered keycode with XGhostty.
        unicode: u21,
    ...
    pub fn parse(input: []const u8) !Trigger {
        ...
        loop: while (rem.len > 0) {
            ...
            // keys with XGhostty 1.1.x. We do this last so its least likely
    ...
    /// The values that are backwards compatible with XGhostty 1.1.x.
    /// XGhostty 1.2+ doesn't support these anymore since we moved to
    /// W3C key codes.
...
// For XGhostty 1.2+ we changed our key names to match the W3C and removed

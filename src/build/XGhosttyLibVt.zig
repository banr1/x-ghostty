...
const GhosttyZig = @import("XGhosttyZig.zig");
...
fn pkgConfigFiles(
    ...
    return .{
        .shared = wf.add("libghostty-vt.pc", b.fmt(
            ...
            \\Description: XGhostty VT library
            \\Version: {f}
            \\Cflags: -I${{includedir}}
        ...
        .static = wf.add("libghostty-vt-static.pc", b.fmt(
            ...
            \\URL: https://github.com/ghostty-org/ghostty
            \\Description: XGhostty VT library (static)
            \\Version: {f}
            \\Cflags: -I${{includedir}}
            \\Libs: ${{libdir}}/{s}
            \\Libs.private: {s}
...
pub fn xcframework(
    ...
    // for XGhosttyKit (the macOS app library).
    const wf = b.addWriteFiles();

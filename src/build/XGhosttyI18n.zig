...
const domain = "com.mitchellh.xghostty";
...
fn createUpdateStep(b: *std.Build) !*std.Build.Step {
    ...
    inline for (gresource.blueprints) |blp| {
        ...
        // a lot of churn because not everyone has the XGhostty code checked out in
...

    {
        ...
        for (gtk_files.items) |item| {
            ...
            // cause a lot of churn because not everyone has the XGhostty code
    ...
    const nautilus_script_path = "dist/linux/xghostty_nautilus.py";
    ...
    const xgettext_merge = b.addSystemCommand(&.{
        ...
        "--copyright-holder=\"Mitchell Hashimoto, XGhostty contributors\"",
        "-o",
        "-",
    });

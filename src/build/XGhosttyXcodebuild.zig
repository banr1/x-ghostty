...
const Docs = @import("XGhosttyDocs.zig");
const I18n = @import("XGhosttyI18n.zig");
const Resources = @import("XGhosttyResources.zig");
const XCFramework = @import("XGhosttyXCFramework.zig");
...
pub fn init(
    ...
    const app_path = b.fmt("macos/build/{s}/XGhostty.app", .{xc_config});

    // Our step to build the XGhostty macOS app.
    const build = build: {
    ...
    // "XGhostty"), ignoring INFOPLIST_KEY_CFBundleName when INFOPLIST_FILE
    ...
    // Our step to open the resulting XGhostty app.
    const open = open: {
        const open = RunStep.create(b, "run XGhostty app");
        open.has_side_effects = true;
        open.cwd = b.path("");
        open.addArgs(&.{b.fmt(
            "{s}/Contents/MacOS/xghostty",
            .{app_path},
        )});
        ...
        open.setEnvironmentVariable("XGHOSTTY_LOG", "stderr,macos");

        // Configure how we're launching
        open.setEnvironmentVariable("XGHOSTTY_MAC_LAUNCH_SOURCE", "zig_run");
...
pub fn install(self: *const XGhostty) void {
    const b = self.copy.step.owner;
    b.getInstallStep().dependOn(&self.copy.step);
}

pub fn installXcframework(self: *const XGhostty) void {

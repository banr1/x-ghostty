...
pub fn init(b: *std.Build, cfg: *const Config, deps: *const SharedDeps) !GhosttyResources {
    ...
    terminfo: {
        ...
        if (cfg.emit_terminfo) {
            const source_install = b.addInstallFile(
                source,
                if (os_tag == .freebsd)
                    "share/site-terminfo/xghostty.terminfo"
                else
                    "share/terminfo/xghostty.terminfo",
            );
        ...
        if (cfg.emit_termcap) {
            ...
            const cap_install = b.addInstallFile(
                out_source,
                if (os_tag == .freebsd)
                    "share/site-terminfo/xghostty.termcap"
                else
                    "share/terminfo/xghostty.termcap",
            );
    ...
    // Shell-integration
    {
        const install_step = b.addInstallDirectory(.{
            ...
            .install_subdir = b.pathJoin(&.{ "xghostty", "shell-integration" }),
    ...
    if (cfg.emit_themes) {
        if (b.lazyDependency("iterm2_themes", .{})) |upstream| {
            const install_step = b.addInstallDirectory(.{
                ...
                .install_dir = .{ .custom = "share" },
                .install_subdir = b.pathJoin(&.{ "xghostty", "themes" }),
                .exclude_extensions = &.{".md"},
            });
            try steps.append(b.allocator, &install_step.step);
        }
    ...
    // bash shell completions
    {
        ...
        run.addArg("+bash");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(), "xghostty.bash");
...
/// Add the resource files needed to make XGhostty a proper
...
fn addLinuxAppResources(
    ...
    const app_id = b.fmt("com.mitchellh.xghostty{s}", .{
        switch (cfg.optimize) {
            .Debug, .ReleaseSafe => "-debug",
            .ReleaseFast, .ReleaseSmall => "",
        },
    ...
    const templates: []const Template = templates: {
        ...
        // within app stores
        try ts.append(b.allocator, .{
            b.path("dist/linux/com.mitchellh.xghostty.metainfo.xml.in"),
    ...
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("dist/linux/xghostty_dolphin.desktop"),
        "share/kio/servicemenus/com.mitchellh.xghostty.desktop",
    ).step);
    ...
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("dist/linux/xghostty_nautilus.py"),
    ...
    try steps.append(b.allocator, &b.addInstallFile(
        ...
        "share/icons/hicolor/16x16/apps/com.mitchellh.xghostty.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/32.png"),
        "share/icons/hicolor/32x32/apps/com.mitchellh.xghostty.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/128.png"),
        "share/icons/hicolor/128x128/apps/com.mitchellh.xghostty.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/256.png"),
        "share/icons/hicolor/256x256/apps/com.mitchellh.xghostty.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/512.png"),
        "share/icons/hicolor/512x512/apps/com.mitchellh.xghostty.png",
    ).step);
    // Flatpaks only support icons up to 512x512.
    if (!cfg.flatpak) {
        try steps.append(b.allocator, &b.addInstallFile(
            b.path("images/gnome/1024.png"),
            "share/icons/hicolor/1024x1024/apps/com.mitchellh.xghostty.png",
        ).step);
    }

    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/32.png"),
        "share/icons/hicolor/16x16@2/apps/com.mitchellh.xghostty.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/64.png"),
        "share/icons/hicolor/32x32@2/apps/com.mitchellh.xghostty.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/256.png"),
        "share/icons/hicolor/128x128@2/apps/com.mitchellh.xghostty.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/512.png"),
        "share/icons/hicolor/256x256@2/apps/com.mitchellh.xghostty.png",
    ).step);
}

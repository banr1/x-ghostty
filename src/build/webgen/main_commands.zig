...
const Action = @import("../../cli/xghostty.zig").Action;
...
pub fn genActions(writer: *std.Io.Writer) !void {
    ...
    try writer.writeAll(
        \\---
        \\title: Reference
        \\description: Reference of all XGhostty action subcommands.
        \\editOnGithubLink: https://github.com/ghostty-org/ghostty/tree/main/src/cli
        \\---
        \\XGhostty includes a number of utility actions that can be accessed as subcommands.
    ...
    inline for (@typeInfo(Action).@"enum".fields) |field| {
        ...
        if (@hasDecl(help_strings.Action, field.name)) {
            ...
            try writer.writeAll("\n```\n");
            switch (action) {
                .help, .version => try writer.writeAll("xghostty --" ++ field.name ++ "\n"),
                else => try writer.writeAll("xghostty +" ++ field.name ++ "\n"),
            }
            try writer.writeAll("```\n\n");
        }
    }

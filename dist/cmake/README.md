# CMake Support for libxghostty-vt

The top-level `CMakeLists.txt` wraps the Zig build system so that CMake
projects can consume libxghostty-vt without invoking `zig build` manually.
...
target_link_libraries(myapp PRIVATE xghostty-vt)
```

This fetches the Ghostty source, builds libxghostty-vt via Zig during your
...
```shell-session
cmake -B build -DFETCHCONTENT_SOURCE_DIR_XGHOSTTY=/path/to/ghostty
cmake --build build
```

## Using find_package (install-based)

Build and install libxghostty-vt first:
...
```cmake
find_package(xghostty-vt REQUIRED)

add_executable(myapp main.c)
target_link_libraries(myapp PRIVATE xghostty-vt::xghostty-vt)
```

## Cross-compilation

For cross-compiling to a different Zig target triple, use
`xghostty_vt_add_target()` after `FetchContent_MakeAvailable`:
...
target platform. `XGhosttyZigCompiler.cmake` provides
`xghostty_zig_compiler()` to set up `zig cc` as the C/CXX compiler for

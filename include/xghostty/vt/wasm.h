...
#ifndef XGHOSTTY_VT_WASM_H
#define XGHOSTTY_VT_WASM_H
 ...
 *
 * XGhostty relies on pointers to various types for ABI compatibility, and
 ...
 *
 * // Create key encoder
 * const encoderPtr = exports.xghostty_wasm_alloc_opaque();
 * exports.xghostty_key_encoder_new(null, encoderPtr);
 * const encoder = view.getUint32(encoder, true);
 *
 * // Configure encoder with Kitty protocol flags
 * const flagsPtr = exports.xghostty_wasm_alloc_u8();
 * view.setUint8(flagsPtr, 0x1F);
 * exports.xghostty_key_encoder_setopt(encoder, 5, flagsPtr);
 *
 * // Allocate output buffer and size pointer
 * const bufferSize = 32;
 * const bufPtr = exports.xghostty_wasm_alloc_u8_array(bufferSize);
 * const writtenPtr = exports.xghostty_wasm_alloc_usize();
 *
 * // Encode the key event
 * exports.xghostty_key_encoder_encode(
 ...
 * @ingroup wasm
 */
XGHOSTTY_API void** xghostty_wasm_alloc_opaque(void);

/**
 * Free an opaque pointer allocated by xghostty_wasm_alloc_opaque().
 *
 * @param ptr Pointer to free, or NULL (NULL is safely ignored)
 * @ingroup wasm
 */
XGHOSTTY_API void xghostty_wasm_free_opaque(void **ptr);
 ...
 * @ingroup wasm
 */
XGHOSTTY_API uint8_t* xghostty_wasm_alloc_u8_array(size_t len);

/**
 * Free an array allocated by xghostty_wasm_alloc_u8_array().
 *
 ...
 * @ingroup wasm
 */
XGHOSTTY_API void xghostty_wasm_free_u8_array(uint8_t *ptr, size_t len);
 ...
 * @ingroup wasm
 */
XGHOSTTY_API uint16_t* xghostty_wasm_alloc_u16_array(size_t len);

/**
 * Free an array allocated by xghostty_wasm_alloc_u16_array().
 *
 * @param ptr Pointer to the array to free, or NULL (NULL is safely ignored)
 * @param len Length of the array (must match the length passed to alloc)
 * @ingroup wasm
 */
XGHOSTTY_API void xghostty_wasm_free_u16_array(uint16_t *ptr, size_t len);
 ...
 * @ingroup wasm
 */
XGHOSTTY_API uint8_t* xghostty_wasm_alloc_u8(void);

/**
 * Free a uint8_t allocated by xghostty_wasm_alloc_u8().
 *
 * @param ptr Pointer to free, or NULL (NULL is safely ignored)
 * @ingroup wasm
 */
XGHOSTTY_API void xghostty_wasm_free_u8(uint8_t *ptr);
 ...
 * @ingroup wasm
 */
XGHOSTTY_API size_t* xghostty_wasm_alloc_usize(void);

/**
 * Free a size_t allocated by xghostty_wasm_alloc_usize().
 *
 * @param ptr Pointer to free, or NULL (NULL is safely ignored)
 * @ingroup wasm
 */
XGHOSTTY_API void xghostty_wasm_free_usize(size_t *ptr);

/** @} */

#endif /* __wasm__ */

#endif /* XGHOSTTY_VT_WASM_H */

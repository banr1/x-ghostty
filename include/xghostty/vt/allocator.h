...
#ifndef XGHOSTTY_VT_ALLOCATOR_H
#define XGHOSTTY_VT_ALLOCATOR_H
 ...
 * ## Alloc/Free Helpers
 *
 * xghostty_alloc() and xghostty_free() provide a simple malloc/free-style
 ...
 *   (e.g. preparing input data for xghostty_terminal_vt_write()).
 * - You need to free a buffer returned by a libghostty-vt function
 *   (e.g. the output of xghostty_formatter_format_alloc()).
 ...
 * The returned buffer must be freed with xghostty_free() using the same
 * allocator.
 *
 ...
 * xghostty_formatter_format_alloc(). Pass the same allocator that was

...
#ifndef XGHOSTTY_VT_GRID_REF_TRACKED_H
#define XGHOSTTY_VT_GRID_REF_TRACKED_H
 ...
 * XGHOSTTY_NO_VALUE. XGHOSTTY_NO_VALUE is also returned when the reference cannot
 ...
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_INVALID_VALUE if ref is invalid,
 *         or XGHOSTTY_NO_VALUE if there is no representable value.
 *
 * @ingroup grid_ref
 */
 ...
 * "no value" state is cleared. On XGHOSTTY_OUT_OF_MEMORY, the original tracked
 ...
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_INVALID_VALUE if ref, terminal,
 *         or point is invalid, or XGHOSTTY_OUT_OF_MEMORY if allocation fails.
 *
 * @ingroup grid_ref
 */
 ...
 * xghostty_grid_ref_graphemes(), xghostty_grid_ref_hyperlink_uri(), or
 * xghostty_grid_ref_style().
 *
 * If the tracked reference no longer has a meaningful value, this returns
 * XGHOSTTY_NO_VALUE. This includes references whose owning terminal has been
 * freed.
 *
 * @param ref Tracked grid reference.
 * @param[out] out_ref On success, receives an untracked snapshot. May be NULL.
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_INVALID_VALUE if ref is invalid,
 *         or XGHOSTTY_NO_VALUE if the tracked location was discarded.
 *
 * @ingroup grid_ref
 */
...
#endif /* XGHOSTTY_VT_GRID_REF_TRACKED_H */

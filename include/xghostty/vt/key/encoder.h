...
#ifndef XGHOSTTY_VT_KEY_ENCODER_H
#define XGHOSTTY_VT_KEY_ENCODER_H
 ...
 * start with `XGHOSTTY_KITTY_KEY_`.
 *
 * @ingroup key
 */
typedef uint8_t XGhosttyKittyKeyFlags;

/** Kitty keyboard protocol disabled (all flags off) */
#define XGHOSTTY_KITTY_KEY_DISABLED 0

/** Disambiguate escape codes */
#define XGHOSTTY_KITTY_KEY_DISAMBIGUATE (1 << 0)

/** Report key press and release events */
#define XGHOSTTY_KITTY_KEY_REPORT_EVENTS (1 << 1)

/** Report alternate key codes */
#define XGHOSTTY_KITTY_KEY_REPORT_ALTERNATES (1 << 2)

/** Report all key events including those normally handled by the terminal */
#define XGHOSTTY_KITTY_KEY_REPORT_ALL (1 << 3)

/** Report associated text with key events */
#define XGHOSTTY_KITTY_KEY_REPORT_ASSOCIATED (1 << 4)

/** All Kitty keyboard protocol flags enabled */
#define XGHOSTTY_KITTY_KEY_ALL (XGHOSTTY_KITTY_KEY_DISAMBIGUATE | XGHOSTTY_KITTY_KEY_REPORT_EVENTS | XGHOSTTY_KITTY_KEY_REPORT_ALTERNATES | XGHOSTTY_KITTY_KEY_REPORT_ALL | XGHOSTTY_KITTY_KEY_REPORT_ASSOCIATED)

/**
 * macOS option key behavior.
 *
 * Determines whether the "option" key on macOS is treated as "alt" or not.
 * See the XGhostty `macos-option-as-alt` configuration option for more details.
 *
 * @ingroup key
 */
typedef enum XGHOSTTY_ENUM_TYPED {
    /** Option key is not treated as alt */
    XGHOSTTY_OPTION_AS_ALT_FALSE = 0,
    /** Option key is treated as alt */
    XGHOSTTY_OPTION_AS_ALT_TRUE = 1,
    /** Only left option key is treated as alt */
    XGHOSTTY_OPTION_AS_ALT_LEFT = 2,
 ...
 *
 * These values are used with xghostty_key_encoder_setopt() to configure
...
typedef enum XGHOSTTY_ENUM_TYPED {
    ...
    XGHOSTTY_KEY_ENCODER_OPT_CURSOR_KEY_APPLICATION = 0,

    /** Terminal DEC mode 66: keypad key application mode (value: bool) */
    XGHOSTTY_KEY_ENCODER_OPT_KEYPAD_KEY_APPLICATION = 1,

    /** Terminal DEC mode 1035: ignore keypad with numlock (value: bool) */
    XGHOSTTY_KEY_ENCODER_OPT_IGNORE_KEYPAD_WITH_NUMLOCK = 2,

    /** Terminal DEC mode 1036: alt sends escape prefix (value: bool) */
    XGHOSTTY_KEY_ENCODER_OPT_ALT_ESC_PREFIX = 3,

    /** xterm modifyOtherKeys mode 2 (value: bool) */
    XGHOSTTY_KEY_ENCODER_OPT_MODIFY_OTHER_KEYS_STATE_2 = 4,
 ...
 * using xghostty_key_encoder_setopt() and must be freed using
 * xghostty_key_encoder_free() when no longer needed.
 *
 ...
 * and associated flags) with xghostty_key_encoder_setopt_from_terminal().
 *
 ...
 * terminal state and is reset to `XGHOSTTY_OPTION_AS_ALT_FALSE` by this
 * call. Use xghostty_key_encoder_setopt() to set it afterward if needed.
 *
 * @param encoder The encoder handle, must not be NULL
 ...
 *
 * If the output buffer is too small, this function returns XGHOSTTY_OUT_OF_SPACE
 ...
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_OUT_OF_SPACE if buffer too small, or other error code
 *
 ...
 * size_t written = 0;
 * result = xghostty_key_encoder_encode(encoder, event, buf, required, &written);
 ...
 * } else if (result == XGHOSTTY_OUT_OF_SPACE) {
 *   // Buffer too small, written contains required size
 *   char *dynamic_buf = malloc(written);
 *   result = xghostty_key_encoder_encode(encoder, event, dynamic_buf, written, &written);
 *   assert(result == XGHOSTTY_SUCCESS);

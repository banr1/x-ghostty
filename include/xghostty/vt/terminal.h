...
#ifndef XGHOSTTY_VT_TERMINAL_H
#define XGHOSTTY_VT_TERMINAL_H
...
#include <xghostty/vt/device.h>
#include <xghostty/vt/modes.h>
#include <xghostty/vt/size_report.h>
#include <xghostty/vt/grid_ref.h>
#include <xghostty/vt/kitty_graphics.h>
#include <xghostty/vt/screen.h>
#include <xghostty/vt/point.h>
#include <xghostty/vt/selection.h>
#include <xghostty/vt/style.h>
 ...
 * to write keyboard input via xghostty_key_encoder_setopt_from_terminal().
 *
 ...
 * ## Effects
 *
 * By default, the terminal sequence processing with xghostty_terminal_vt_write() 
 ...
 * sequences processed during xghostty_terminal_vt_write(). They let the
 ...
 * xghostty_terminal_vt_write(). Callbacks **must not** call
 * xghostty_terminal_vt_write() on the same terminal (no reentrancy).
 ...
 * ### Default Colors
 *
 * Use xghostty_terminal_set() with the color options to configure the
 ...
 * ### Reading colors
 *
 * Use xghostty_terminal_get() to read colors. There are two variants for
...
typedef enum XGHOSTTY_ENUM_TYPED {
  ...
  XGHOSTTY_SCROLL_VIEWPORT_TOP,

  /** Scroll to the bottom (active area). */
  XGHOSTTY_SCROLL_VIEWPORT_BOTTOM,
 ...
 */
typedef union {
  /** Scroll delta (only used with XGHOSTTY_SCROLL_VIEWPORT_DELTA). Up is negative. */
  intptr_t delta;
...
typedef enum XGHOSTTY_ENUM_TYPED {
  ...
  XGHOSTTY_TERMINAL_SCREEN_PRIMARY = 0,
...
typedef enum XGHOSTTY_ENUM_TYPED {
  ...
  XGHOSTTY_TERMINAL_CURSOR_STYLE_BAR = 0,

  /** Block cursor (DECSCUSR 1, 2). */
  XGHOSTTY_TERMINAL_CURSOR_STYLE_BLOCK = 1,

  /** Underline cursor (DECSCUSR 3, 4). */
  XGHOSTTY_TERMINAL_CURSOR_STYLE_UNDERLINE = 2,
 ...
 * @param userdata The userdata pointer set via XGHOSTTY_TERMINAL_OPT_USERDATA
 *
 * @ingroup terminal
 */
 ...
 * OSC 1337 CurrentDir (iTerm2). Use xghostty_terminal_get() with
 * XGHOSTTY_TERMINAL_DATA_PWD inside the callback to read the new value.
 *
 ...
 * OSC 7). In that case XGHOSTTY_TERMINAL_DATA_PWD returns a zero-length
 * string.
 *
 * @param terminal The terminal handle
 * @param userdata The userdata pointer set via XGHOSTTY_TERMINAL_OPT_USERDATA
 ...
 *
 * These values are used with xghostty_terminal_set() to configure
...
typedef enum XGHOSTTY_ENUM_TYPED {
   ...
   * Input type: void*
   */
  XGHOSTTY_TERMINAL_OPT_USERDATA = 0,
   ...
   * Input type: uint64_t*
   */
  XGHOSTTY_TERMINAL_OPT_KITTY_IMAGE_STORAGE_LIMIT = 15,
   ...
   * Input type: bool*
   */
  XGHOSTTY_TERMINAL_OPT_KITTY_IMAGE_MEDIUM_FILE = 16,
   ...
   * Input type: bool*
   */
  XGHOSTTY_TERMINAL_OPT_KITTY_IMAGE_MEDIUM_TEMP_FILE = 17,
   ...
   * Input type: bool*
   */
  XGHOSTTY_TERMINAL_OPT_KITTY_IMAGE_MEDIUM_SHARED_MEM = 18,
   ...
   * Input type: size_t*
   */
  XGHOSTTY_TERMINAL_OPT_APC_MAX_BYTES = 19,
   ...
   * Input type: size_t*
   */
  XGHOSTTY_TERMINAL_OPT_APC_MAX_BYTES_KITTY = 20,
   ...
   * Input type: bool*
   */
  XGHOSTTY_TERMINAL_OPT_DEFAULT_CURSOR_BLINK = 23,
   ...
   * Input type: bool*
   */
  XGHOSTTY_TERMINAL_OPT_GLYPH_PROTOCOL = 24,
 ...
 * using `xghostty_terminal_get`.
 *
 * @ingroup terminal
 */
typedef enum XGHOSTTY_ENUM_TYPED {
  /** Invalid data type. Never results in any data extraction. */
  XGHOSTTY_TERMINAL_DATA_INVALID = 0,
   ...
   * Output type: uint16_t *
   */
  XGHOSTTY_TERMINAL_DATA_CURSOR_X = 3,
   ...
   * Output type: uint16_t *
   */
  XGHOSTTY_TERMINAL_DATA_CURSOR_Y = 4,
   ...
   * Output type: bool *
   */
  XGHOSTTY_TERMINAL_DATA_CURSOR_PENDING_WRAP = 5,
   ...
   * Output type: bool *
   */
  XGHOSTTY_TERMINAL_DATA_CURSOR_VISIBLE = 7,

  /**
   * Current Kitty keyboard protocol flags.
   *
   * Output type: XGhosttyKittyKeyFlags * (uint8_t *)
   */
  XGHOSTTY_TERMINAL_DATA_KITTY_KEYBOARD_FLAGS = 8,
   ...
   * Output type: bool *
   */
  XGHOSTTY_TERMINAL_DATA_MOUSE_TRACKING = 11,
   ...
   * Output type: size_t *
   */
  XGHOSTTY_TERMINAL_DATA_TOTAL_ROWS = 14,
   ...
   * Output type: size_t *
   */
  XGHOSTTY_TERMINAL_DATA_SCROLLBACK_ROWS = 15,

  /**
   * The total width of the terminal in pixels.
   *
   * This is cols * cell_width_px as set by xghostty_terminal_resize().
   *
   * Output type: uint32_t *
   */
  XGHOSTTY_TERMINAL_DATA_WIDTH_PX = 16,

  /**
   * The total height of the terminal in pixels.
   *
   * This is rows * cell_height_px as set by xghostty_terminal_resize().
   *
   * Output type: uint32_t *
   */
  XGHOSTTY_TERMINAL_DATA_HEIGHT_PX = 17,
   ...
   */
  XGHOSTTY_TERMINAL_DATA_KITTY_IMAGE_STORAGE_LIMIT = 26,
   ...
   * Output type: bool *
   */
  XGHOSTTY_TERMINAL_DATA_KITTY_IMAGE_MEDIUM_FILE = 27,
   ...
   * Output type: bool *
   */
  XGHOSTTY_TERMINAL_DATA_KITTY_IMAGE_MEDIUM_TEMP_FILE = 28,
   ...
   * Output type: bool *
   */
  XGHOSTTY_TERMINAL_DATA_KITTY_IMAGE_MEDIUM_SHARED_MEM = 29,
   ...
   * until the next mutating terminal call (e.g. xghostty_terminal_vt_write()
   * or xghostty_terminal_reset()).
   *
   * Returns XGHOSTTY_NO_VALUE when Kitty graphics are disabled at build time.
   *
   * Output type: XGhosttyKittyGraphics *
   */
  XGHOSTTY_TERMINAL_DATA_KITTY_GRAPHICS = 30,
   ...
   * xghostty_terminal_vt_write(), xghostty_terminal_resize(), or
 ...
 * Callbacks must not call xghostty_terminal_vt_write() on the same
 ...
 * xghostty_terminal_set() with XGHOSTTY_TERMINAL_OPT_WRITE_PTY to install
 ...
 * When using XGHOSTTY_SCROLL_VIEWPORT_DELTA, set the delta field in
 ...
 *
 * This is an optimization over calling xghostty_terminal_get()
 ...
 * xghostty_grid_ref_cell() and xghostty_grid_ref_row() to extract the cell
 * and row.
 *
 ...
 *
 * This is the tracked variant of xghostty_terminal_grid_ref(). The returned
 ...
 * XGHOSTTY_INVALID_VALUE and writes NULL to out_ref.
 *
 * The returned handle must be freed with xghostty_tracked_grid_ref_free(). If
 ...
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_INVALID_VALUE if terminal,
 *         point, or out_ref is invalid, or XGHOSTTY_OUT_OF_MEMORY if allocation
 *         fails.
 *
 * @ingroup terminal
 */
 ...
 *
 * This is the inverse of xghostty_terminal_grid_ref(): given a grid reference,
 ...
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_INVALID_VALUE if the terminal
 *         or ref is NULL/invalid, XGHOSTTY_NO_VALUE if the ref falls outside

...
#ifndef XGHOSTTY_VT_RENDER_H
#define XGHOSTTY_VT_RENDER_H
...
typedef enum XGHOSTTY_ENUM_TYPED {
  ...
  XGHOSTTY_RENDER_STATE_DIRTY_FALSE = 0,

  /** Some rows changed; renderer can redraw incrementally. */
  XGHOSTTY_RENDER_STATE_DIRTY_PARTIAL = 1,
...
typedef enum XGHOSTTY_ENUM_TYPED {
  ...
  XGHOSTTY_RENDER_STATE_CURSOR_VISUAL_STYLE_BAR = 0,

  /** Block cursor (DECSCUSR 1, 2). */
  XGHOSTTY_RENDER_STATE_CURSOR_VISUAL_STYLE_BLOCK = 1,

  /** Underline cursor (DECSCUSR 3, 4). */
  XGHOSTTY_RENDER_STATE_CURSOR_VISUAL_STYLE_UNDERLINE = 2,
...
typedef enum XGHOSTTY_ENUM_TYPED {
  ...
  XGHOSTTY_RENDER_STATE_DATA_INVALID = 0,

  /** Viewport width in cells (uint16_t). */
  XGHOSTTY_RENDER_STATE_DATA_COLS = 1,
...
typedef enum XGHOSTTY_ENUM_TYPED {
  /** Invalid / sentinel value. */
  XGHOSTTY_RENDER_STATE_ROW_DATA_INVALID = 0,
 ...
 * @param terminal The terminal handle to read from (NULL returns XGHOSTTY_INVALID_VALUE)
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_INVALID_VALUE if `state` or
 * `terminal` is NULL, XGHOSTTY_OUT_OF_MEMORY if updating the state requires
 ...
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_INVALID_VALUE if `state` is
 ...
 * via xghostty_render_state_get() with
 * XGHOSTTY_RENDER_STATE_DATA_ROW_ITERATOR.
 *
 * @param allocator Pointer to allocator, or NULL to use the default allocator
 ...
 *
 * @param iterator The iterator handle to query (NULL returns XGHOSTTY_INVALID_VALUE)
 * @param data The data kind to query
 * @param[out] out Pointer to receive the queried value
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_INVALID_VALUE if
 ...
 * calling this function.
 *
 * @param iterator The iterator handle to update (NULL returns XGHOSTTY_INVALID_VALUE)
 * @param option The option to set
 * @param[in] value Pointer to the value to set (NULL returns
 *            XGHOSTTY_INVALID_VALUE)
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_INVALID_VALUE if
 ...
 *
 * All fields except the allocator are left undefined until populated
 * via xghostty_render_state_row_get() with
 * XGHOSTTY_RENDER_STATE_ROW_DATA_CELLS.
 *
 * You can reuse this value repeatedly with xghostty_render_state_row_get() to 
...
typedef enum XGHOSTTY_ENUM_TYPED {
  ...
  XGHOSTTY_RENDER_STATE_ROW_CELLS_DATA_GRAPHEMES_LEN = 3,
  ...
  XGHOSTTY_RENDER_STATE_ROW_CELLS_DATA_FG_COLOR = 6,
   ...
   *  XGHOSTTY_RENDER_STATE_ROW_DATA_SELECTION once per row and applying that
   *  range directly, avoiding one C API call per cell for selection state. */
  XGHOSTTY_RENDER_STATE_ROW_CELLS_DATA_SELECTED = 7,
   ...
   * XGHOSTTY_OUT_OF_SPACE without writing any bytes and sets len to the required
 ...
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_INVALID_VALUE if `cells`
 ...
 * calling this function.
 *
 * @param cells The row cells handle to query (NULL returns XGHOSTTY_INVALID_VALUE)
 * @param data The data kind to query
 * @param[out] out Pointer to receive the queried value
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_INVALID_VALUE if
 ...
 * failing key (i.e. the number of values successfully written).
 *
 * @param cells The row cells handle (NULL returns XGHOSTTY_INVALID_VALUE)
 * @param count Number of key/value pairs
 * @param keys Array of data kinds to query
 * @param values Array of output pointers (types must match each key's
 *               documented output type)

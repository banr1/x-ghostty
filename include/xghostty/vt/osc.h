...
#ifndef XGHOSTTY_VT_OSC_H
#define XGHOSTTY_VT_OSC_H
 ...
 * ## Basic Usage
 *
 * 1. Create a parser instance with xghostty_osc_new()
 * 2. Feed bytes to the parser using xghostty_osc_next() 
 * 3. Finalize parsing with xghostty_osc_end() to get the command
 * 4. Query command type and extract data using xghostty_osc_command_type()
 *    and xghostty_osc_command_data()
 * 5. Free the parser with xghostty_osc_free() when done
 *
 * @{
 */
...
typedef enum XGHOSTTY_ENUM_TYPED {
  XGHOSTTY_OSC_COMMAND_INVALID = 0,
  XGHOSTTY_OSC_COMMAND_CHANGE_WINDOW_TITLE = 1,
  XGHOSTTY_OSC_COMMAND_CHANGE_WINDOW_ICON = 2,
  XGHOSTTY_OSC_COMMAND_SEMANTIC_PROMPT = 3,
  XGHOSTTY_OSC_COMMAND_CLIPBOARD_CONTENTS = 4,
  XGHOSTTY_OSC_COMMAND_REPORT_PWD = 5,
  XGHOSTTY_OSC_COMMAND_MOUSE_SHAPE = 6,
  XGHOSTTY_OSC_COMMAND_COLOR_OPERATION = 7,
  XGHOSTTY_OSC_COMMAND_KITTY_COLOR_PROTOCOL = 8,
  XGHOSTTY_OSC_COMMAND_SHOW_DESKTOP_NOTIFICATION = 9,
  XGHOSTTY_OSC_COMMAND_HYPERLINK_START = 10,
  XGHOSTTY_OSC_COMMAND_HYPERLINK_END = 11,
  XGHOSTTY_OSC_COMMAND_CONEMU_SLEEP = 12,
  XGHOSTTY_OSC_COMMAND_CONEMU_SHOW_MESSAGE_BOX = 13,
  XGHOSTTY_OSC_COMMAND_CONEMU_CHANGE_TAB_TITLE = 14,
  XGHOSTTY_OSC_COMMAND_CONEMU_PROGRESS_REPORT = 15,
  XGHOSTTY_OSC_COMMAND_CONEMU_WAIT_INPUT = 16,
  XGHOSTTY_OSC_COMMAND_CONEMU_GUIMACRO = 17,
  XGHOSTTY_OSC_COMMAND_CONEMU_RUN_PROCESS = 18,
  XGHOSTTY_OSC_COMMAND_CONEMU_OUTPUT_ENVIRONMENT_VARIABLE = 19,
  XGHOSTTY_OSC_COMMAND_CONEMU_XTERM_EMULATION = 20,
 ...
 * using `xghostty_osc_command_data`.
 *
 * @ingroup osc
 */
typedef enum XGHOSTTY_ENUM_TYPED {
  /** Invalid data type. Never results in any data extraction. */
  XGHOSTTY_OSC_DATA_INVALID = 0,
  
  /** 
   * Window title string data.
   *
   * Valid for: XGHOSTTY_OSC_COMMAND_CHANGE_WINDOW_TITLE
   *
   * Output type: const char ** (pointer to null-terminated string)
   *
   * Lifetime: Valid until the next call to any xghostty_osc_* function with 
 ...
 * allocator. The parser must be freed using xghostty_vt_osc_free() when
 * no longer needed.
 * 
 ...
 *
 * When finished pumping the parser with bytes, call xghostty_osc_end
 ...
 * using xghostty_osc_next() with the exception of the terminating character
 ...
 * with type XGHOSTTY_OSC_COMMAND_INVALID.
 * 
 ...
 * `xghostty_osc_*` function with the same parser instance with the exception
 * of command introspection functions such as `xghostty_osc_command_type`.
 * 
 * @param parser The parser handle, must not be null.
 ...
 * @return The command type, or XGHOSTTY_OSC_COMMAND_INVALID if command is NULL
 * 
 * @ingroup osc
 */

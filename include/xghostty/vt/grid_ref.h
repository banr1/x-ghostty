...
#ifndef XGHOSTTY_VT_GRID_REF_H
#define XGHOSTTY_VT_GRID_REF_H
 ...
 * terminal. Obtain a grid reference from `xghostty_terminal_grid_ref`
 * for untracked or `xghostty_terminal_grid_ref_track` for tracked. Untracked
 ...
 * xghostty_tracked_grid_ref_has_value() returns false and
 * xghostty_tracked_grid_ref_snapshot() / xghostty_tracked_grid_ref_point() return
 * XGHOSTTY_NO_VALUE. The handle remains valid, and callers may move it to a new
 ...
 * alternate screens. Calling xghostty_tracked_grid_ref_set() resolves the new
 ...
 * xghostty_tracked_grid_ref_free(). If the terminal that created a tracked
 ...
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_INVALID_VALUE if the ref's
 *         node is NULL
 *
 * @ingroup grid_ref
 */
 ...
 * out_len is set to 0 and XGHOSTTY_SUCCESS is returned.
 *
 * If the buffer is too small (or NULL), the function returns
 * XGHOSTTY_OUT_OF_SPACE and writes the required number of codepoints to
 ...
 *             XGHOSTTY_OUT_OF_SPACE, the required buffer size in codepoints.
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_INVALID_VALUE if the ref's
 *         node is NULL, XGHOSTTY_OUT_OF_SPACE if the buffer is too small
 *
 * @ingroup grid_ref
 */
 ...
 * hyperlink, out_len is set to 0 and XGHOSTTY_SUCCESS is returned.
 *
 * If the buffer is too small (or NULL), the function returns
 * XGHOSTTY_OUT_OF_SPACE and writes the required number of bytes to
 * out_len. The caller can then retry with a sufficiently sized buffer.
 *
 * @param ref Pointer to the grid reference
 ...
 *             XGHOSTTY_OUT_OF_SPACE, the required buffer size in bytes.
 * @return XGHOSTTY_SUCCESS on success, XGHOSTTY_INVALID_VALUE if the ref's
 *         node is NULL, XGHOSTTY_OUT_OF_SPACE if the buffer is too small
 *
 * @ingroup grid_ref
